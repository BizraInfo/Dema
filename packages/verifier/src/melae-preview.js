...
export const SAPE_SCORECARD_PREVIEW_SCHEMA =
  "bizra.dema.sape_scorecard_preview.v0.1";
export const DEFAULT_MELAE_FLOOR = 0.95;

export const SAPE_PROBES = Object.freeze([
  { id: "counterfactual", weight: 0.1 },
  { id: "boundary", weight: 0.15 },
  { id: "analogical", weight: 0.1 },
  { id: "formalization", weight: 0.15 },
  { id: "program_sketch", weight: 0.1 },
  { id: "compression", weight: 0.1 },
  { id: "expansion", weight: 0.1 },
  { id: "adversarial", weight: 0.15 },
  { id: "ethical_overlay", weight: 0.05 },
]);

const PROBE_WEIGHTS = new Map(
  SAPE_PROBES.map((probe) => [probe.id, probe.weight]),
);
...
function normalizeProbeScore(entry) {
  ...
  return {
    ...
    passed:
      Boolean(weight) &&
      finiteUnitNumber(score) &&
      score >= DEFAULT_MELAE_FLOOR,
...
function summarizeProbeSet(normalized) {
  ...
  const supplied = new Set(
    normalized.map((probe) => probe.probe_id).filter(Boolean),
  );
  return {
...
export function buildSapeScorecardPreview({
  ...
  const normalized = Array.isArray(probeScores)
    ? probeScores.map(normalizeProbeScore)
    : [];
  ...
  const shapeOk =
    ...
    normalized.every(
      (probe) =>
        probe.probe_id &&
        finiteUnitNumber(probe.score) &&
        finiteUnitNumber(probe.weight),
    );
  ...
  scorecard.scorecard_digest = sha256Canonical({
    ...
    probes: scorecard.probes.map(({ probe_id, score, weight }) => ({
      probe_id,
      score,
      weight,
    })),
...
export function evaluateMelaeGatePreview({
  ...
  return {
    ...
    sape,
    verdict:
      sapeMet && snrMet && ihsanMet ? "PARTIAL_PLACEHOLDER" : "PREVIEW_REJECT",
    checks: [
