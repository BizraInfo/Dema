...
export const EVIDENCE_CHAIN_PREVIEW_SCHEMA =
...
function linkPayloadFromLink(link) {
  return {
    ...
    previous_entry_digest:
      typeof link?.previous_entry_digest === "string"
        ? link.previous_entry_digest
        : null,
    entry_schema:
      typeof link?.entry_schema === "string" ? link.entry_schema : null,
    entry_digest:
      typeof link?.entry_digest === "string" ? link.entry_digest : null,
    entry_input_hash:
      typeof link?.entry_input_hash === "string" ? link.entry_input_hash : null,
    entry_output_hash:
      typeof link?.entry_output_hash === "string"
        ? link.entry_output_hash
        : null,
    entry_policy_hash:
      typeof link?.entry_policy_hash === "string"
        ? link.entry_policy_hash
        : null,
    entry_tool_calls_hash:
      typeof link?.entry_tool_calls_hash === "string"
        ? link.entry_tool_calls_hash
        : null,
  };
}
...
function containsAnyField(value, fields, seen = new Set()) {
  ...
  if (isObject(value) && fields.some((field) => hasOwn(value, field)))
    return true;
...
function isCanonicalReceiptLike(receipt) {
  if (!isObject(receipt) || receipt.schema === EVIDENCE_RECEIPT_PREVIEW_SCHEMA)
    return false;
  return (
...
function boundaryDeclaresNoEffects(boundary, fields) {
  return (
    isObject(boundary) && fields.every((field) => boundary[field] === false)
  );
}
...
function receiptVerificationVerdict(receipt) {
  try {
    return {
      ok: true,
      value: verifyEvidenceReceiptPreview(receipt),
      error: null,
    };
  } catch (error) {
    return {
...
function validateReceipt(receipt, index) {
  ...
  if (!isObject(receipt)) {
    return [
      denial("invalid_receipts", "receipt entry must be an object", index),
    ];
  }
  if (isProofForgeReceiptLike(receipt)) {
    return [
      denial(
        "proof_forge_receipt_rejected",
        "Proof Forge receipts are outside this preview domain",
        index,
      ),
    ];
  }
  if (isCanonicalReceiptLike(receipt)) {
    return [
      denial(
        ...
        "canonical receipt-like shapes are outside this preview domain",
        index,
      ),
    ];
  }
  ...
  if (receipt.mode !== MODE) {
    denials.push(
      denial(
        ...
        "receipt mode must be PREVIEW_ONLY",
        index,
      ),
    );
  }
  if (receipt.certifies !== false) {
    denials.push(
      denial("receipt_certifies", "receipt preview must not certify", index),
    );
  }
  if (receipt.chain_id !== PREVIEW_CHAIN_ID) {
    denials.push(
      denial(
        "receipt_has_chain_identity",
        "receipt preview chain_id must be preview-no-chain",
        index,
      ),
    );
  }
  if (receipt.prev_digest !== null) {
    denials.push(
      denial(
        "receipt_has_prev_digest",
        "receipt preview prev_digest must be null",
        index,
      ),
    );
  }
  if (receipt.producer_identity !== null) {
    denials.push(
      denial(
        "receipt_has_producer_identity",
        "receipt preview must not bind producer identity",
        index,
      ),
    );
  }
  if (hasAnyTopLevelField(receipt, SIGNATURE_AND_IDENTITY_FIELDS)) {
    denials.push(
      denial(
        "receipt_has_signature_fields",
        "signature, key, session, and producer id fields are forbidden",
        index,
      ),
    );
  }
  if (containsAnyField(receipt, POSITION_FIELDS)) {
    denials.push(
      denial(
        "entry_contains_position_claim",
        "position-like fields are forbidden",
        index,
      ),
    );
  }
  if (
    !boundaryDeclaresNoEffects(receipt.boundary, RECEIPT_BOUNDARY_FALSE_FIELDS)
  ) {
    denials.push(
      denial(
        "receipt_boundary_has_effects",
        "receipt boundary must declare every preview effect as false",
        index,
      ),
    );
  }
  ...
  if (!verification.ok) {
    denials.push(
      denial("receipt_verification_failed", verification.error, index),
    );
  } else if (verification.value?.verdict !== "PARTIAL_PLACEHOLDER") {
    denials.push(
      denial(
        "receipt_verification_failed",
        "receipt preview verifier did not return PARTIAL_PLACEHOLDER",
        index,
      ),
    );
  }
...
export function buildEvidenceChainPreview({
  ...
  if (!nonEmptyString(normalizedPurpose)) {
    denials.push(
      denial(
        "invalid_purpose",
        "purpose must be a non-empty string when provided",
      ),
    );
  }
  if (!Array.isArray(receipts)) {
    denials.push(
      denial("invalid_receipts", "receipts must be a non-empty array"),
    );
  } else if (receipts.length === 0) {
    denials.push(
      denial(
        "empty_chain",
        "receipts must contain at least one evidence receipt preview",
      ),
    );
  }
  ...
  if (Array.isArray(receipts)) {
    receipts.forEach((receipt, index) => {
      ...
      if (denials.length === before && seenDigests.has(receipt.self_digest)) {
        denials.push(
          denial(
            "duplicate_entry_digest",
            "receipt self_digest values must be unique",
            index,
          ),
        );
      }
      if (
        denials.length === before &&
...
function verifyLink(link, index, links) {
  ...
  const expectedPrevious =
    index === 0 ? GENESIS_SENTINEL : links[index - 1]?.link_digest;
  ...
  return {
    ...
    entry_preview_verifies:
      payload.entry_schema === EVIDENCE_RECEIPT_PREVIEW_SCHEMA && entryHashesOk,
    previous_entry_digest_ok:
      payload.previous_entry_digest === expectedPrevious,
    link_digest_ok:
      expectedDigest.ok && link?.link_digest === expectedDigest.value,
    signature_ok: null,
    expected_link_digest: expectedDigest.value,
    actual_link_digest:
      typeof link?.link_digest === "string" ? link.link_digest : null,
  };
}
...
export function verifyEvidenceChainPreview(chain) {
  ...
  const actualDigest =
    typeof chain?.preview_chain_digest === "string"
  ...
  const duplicateEntryDigestsOk =
    uniqueEntryDigests.size === entryDigests.length;

  const checks = [
    check(
      "schema_declared_as_evidence_chain_preview",
      chain?.schema === EVIDENCE_CHAIN_PREVIEW_SCHEMA,
      `expected ${EVIDENCE_CHAIN_PREVIEW_SCHEMA}`,
    ),
    check(
      "mode_is_preview_only",
      chain?.mode === MODE,
      "mode must be PREVIEW_ONLY",
    ),
    check(
      "truth_label_declared",
      chain?.truth_label === TRUTH_LABEL,
      "truth_label must be DECLARED",
    ),
    check(
      "certifies_false",
      chain?.certifies === false,
      "preview chain must not certify",
    ),
    check(
      "chain_id_preview_no_chain",
      chain?.chain_id === EVIDENCE_CHAIN_PREVIEW_CHAIN_ID,
      "chain_id must be preview-no-chain",
    ),
    check(
      "producer_identity_absent",
      chain?.producer_identity === null,
      "producer_identity must be null",
    ),
    check(
      "signature_fields_absent",
      !hasAnyTopLevelField(chain ?? {}, SIGNATURE_AND_IDENTITY_FIELDS),
      "signature-like fields are forbidden",
    ),
    check(
      "canonical_chain_fields_absent",
      !hasAnyTopLevelField(chain ?? {}, ["prev_digest", "chain_head"]),
      "canonical chain field names are forbidden",
    ),
    check(
      "policy_matches_preview_contract",
      policyMatchesPreviewContract(chain?.policy),
      "policy must preserve the preview-only contract",
    ),
    check(
      "boundary_declares_no_effects",
      boundaryDeclaresNoEffects(chain?.boundary, CHAIN_BOUNDARY_FALSE_FIELDS),
      "all preview boundary effects must be false",
    ),
    check("links_array", Array.isArray(chain?.links), "links must be an array"),
    check(
      "denials_empty",
      Array.isArray(chain?.denials) && chain.denials.length === 0,
      "valid preview chains must have no denials",
    ),
    check(
      "duplicate_entry_digests_absent",
      duplicateEntryDigestsOk,
      "entry_digest values must be unique",
    ),
    check(
      "preview_chain_digest_well_formed",
      PREFIXED_SHA256_RE.test(actualDigest ?? ""),
      "preview_chain_digest must be sha256:<64 hex>",
    ),
    check(
      "preview_chain_digest_recomputes",
      expectedDigest.ok && actualDigest === expectedDigest.value,
      expectedDigest.ok
        ? "preview_chain_digest matches canonical preview payload"
        : expectedDigest.error,
    ),
  ];
  ...
  for (const entry of entries) {
    checks.push(
      check(
        `entry_${entry.index}_schema_ok`,
        entry.entry_schema_ok,
        "entry_schema must be evidence receipt preview",
      ),
    );
    checks.push(
      check(
        `entry_${entry.index}_preview_hashes_ok`,
        entry.entry_preview_verifies,
        "entry digest and hash fields must be bare sha256 hex",
      ),
    );
    checks.push(
      check(
        `entry_${entry.index}_previous_entry_digest_ok`,
        entry.previous_entry_digest_ok,
        "previous_entry_digest must link to prior preview link digest",
      ),
    );
    checks.push(
      check(
        `entry_${entry.index}_link_digest_ok`,
        entry.link_digest_ok,
        "link_digest must recompute from canonical preview link payload",
      ),
    );
  }
...
export function formatEvidenceChainPreview(chain) {
  ...
  const denialLines =
    denials.length === 0
      ...
      : denials.map(
          (item) =>
            `  - ${item.code}${item.index === null || item.index === undefined ? "" : ` at entry ${item.index}`}`,
        );
