...
export function buildSovereignJourneyPreview({ intent, now = new Date() } = {}) {
...
function appendMission(lines, missionDraft) {
  ...
  lines.push(`  phase: ${missionDraft.mission.current_phase} -> ${missionDraft.phase_gate.next_phase}`);
...
export function formatSovereignJourneyPreview(journey) {
  ...
  lines.push("Boundary: preview-only; no approval; no handoff; no execution; no receipt minted.");
