...
function smallestKnownModel(candidates) {
  return candidates
    .filter((model) => Number.isFinite(model.size_bytes) && model.size_bytes > 0)
    .sort((a, b) => a.size_bytes - b.size_bytes)[0] ?? candidates[0] ?? null;
}

export function buildRoutingRecommendations(providers) {
  const candidates = allModelCandidates(providers);
  const nonEmbeddings = candidates.filter((model) => (
    !/embed|nomic/i.test(model.id) && !isSupportFile(model)
  ));
  const reasoningModel = pick(candidates, /deepseek|r1/i) ?? pick(candidates, /qwen/i);
