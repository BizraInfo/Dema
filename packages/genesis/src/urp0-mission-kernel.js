...
const PROPOSAL_BINDING_FIELDS = Object.freeze([
  "bridge_hash",
  "source_intent_hash",
...
// Optional cross-plane binding. The URP contract remains the authority for the
// live mission, while this exact shape lets DEMA prove which proposal caused the
// human-facing consent card. Unknown fields are refused before hashing.
function normalizeProposalBinding(value) {
  if (value === undefined || value === null) return null;
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new TypeError("proposal_binding_invalid");
  }
  const actual = Object.keys(value).sort();
  const expected = [...PROPOSAL_BINDING_FIELDS].sort();
  if (actual.length !== expected.length || actual.some((key, i) => key !== expected[i])) {
    throw new TypeError("proposal_binding_shape_invalid");
  }
  for (const key of PROPOSAL_BINDING_FIELDS) {
    if (!SHA256_RE.test(value[key])) throw new TypeError(`proposal_binding_hash_invalid:${key}`);
  }
  return Object.freeze(Object.fromEntries(PROPOSAL_BINDING_FIELDS.map((key) => [key, value[key]])));
}
...
export function buildMissionContract({ mission_id = URP0_MISSION_ID, canonical_root, limits, proposal_binding = null } = {}) {
  const binding = normalizeProposalBinding(proposal_binding);
  const body = {
  ...
  if (binding) body.proposal_binding = binding;
...
export function buildConsentContext({ mission_id, canonical_root, contract_hash, nonce, created_at_iso, expires_at_iso, proposal_binding = null }) {
  const binding = normalizeProposalBinding(proposal_binding);
  const context = {
  ...
  if (binding) context.proposal_binding = binding;
