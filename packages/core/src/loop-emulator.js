...
const PAT_CLASSES = [
  ["read_file", 0.20, 1, 0.005, [0.98, 0.01, 0.01], 0.1],
  ["write_journal", 0.15, 1, 0.005, [0.95, 0.04, 0.01], 0.1],
  ["query_memory", 0.15, 2, 0.010, [0.96, 0.02, 0.02], 0.4],
  ["propose_action", 0.12, 3, 0.020, [0.50, 0.40, 0.10], 0.3],
  ["ingest_corpus", 0.08, 4, 0.030, [0.30, 0.55, 0.15], 1.5],
  ["draft_message", 0.08, 3, 0.025, [0.20, 0.70, 0.10], 0.2],
  ["modify_canon", 0.05, 5, 0.040, [0.05, 0.85, 0.10], 0.5],
  ["external_call", 0.05, 4, 0.060, [0.10, 0.60, 0.30], 0.6],
  ["spawn_subagent", 0.05, 3, 0.030, [0.40, 0.50, 0.10], 0.4],
  ["commit_receipt", 0.04, 2, 0.010, [0.92, 0.06, 0.02], 0.8],
  ["ambiguous", 0.03, 5, 0.150, [0.10, 0.30, 0.60], 0.2]
];

const SAT_CLASSES = [
  ["urp_query_read", 0.35, 1, 0.002, [0.98, 0.01, 0.01]],
  ["capability_advertise", 0.10, 1, 0.005, [0.92, 0.06, 0.02]],
  ["wisdom_retrieve", 0.20, 2, 0.005, [0.95, 0.03, 0.02]],
  ["resource_offer_serve", 0.10, 2, 0.010, [0.85, 0.10, 0.05]],
  ["log_bus_emit", 0.10, 1, 0.003, [0.99, 0.005, 0.005]],
  ["poi_record", 0.05, 2, 0.010, [0.90, 0.08, 0.02]],
  ["wisdom_promote", 0.04, 4, 0.020, [0.05, 0.90, 0.05]],
  ["cross_node_handshake", 0.04, 4, 0.030, [0.10, 0.85, 0.05]],
  ["cross_node_write", 0.02, 5, 0.050, [0.00, 0.90, 0.10]]
];
...
function runMicroLoop({ organ, complexity, gateDist, haltP }, rng) {
  ...
  vdpu.V += 3;
  if (rng() < 0.0001) return { vdpu, halts: ["mu1: canon hash mismatch"], gate_decision, dod_result };
  ...
  if (gateRoll < permitP) {
    ...
    return { vdpu, halts, gate_decision, dod_result: "FAIL", emulated_receipt_bytes: 1024 };
  }
...
function updateCounters(counter, outcome, rng) {
  ...
  if (outcome.gate_decision === "REJECT") counter.rejected += 1;
    ...
    counter.completed += 1;
    if (rng && rng() < 0.10) counter.wisdom_capsules += 1;
...
function simulateNode(nodeId, patMissionCount, rng) {
  ...
  for (let i = 0; i < patMissionCount; i += 1) {
    ...
    const pat = runMicroLoop({ organ: "PAT", complexity, gateDist, haltP }, rng);
...
function sumNodeCounters(nodes, organ) {
  ...
  for (const node of nodes) {
    ...
    for (const key of ["missions", "completed", "blocked", "failed", "rejected", "pending_go", "wisdom_capsules"]) {
...
function humanizeBytes(bytes) {
  for (const [unit, divisor] of [["PB", 1e15], ["TB", 1e12], ["GB", 1e9], ["MB", 1e6], ["KB", 1e3]]) {
...
function aggregateScale({ label, nodeCount, patPerNode, sampleNodes, seed }) {
  ...
  const nodes = Array.from({ length: sampleNodes }, (_, index) => simulateNode(index, patPerNode, rng));
  const scaleFactor = nodeCount / sampleNodes;
  const pat = sumNodeCounters(nodes, "pat");
  const sat = sumNodeCounters(nodes, "sat");
  const receiptBytes = nodes.reduce((sum, node) => sum + node.emulated_receipt_bytes, 0);
  ...
  return {
    ...
    performance: {
      ...
      bottleneck_estimate:
        pendingTotal * scaleFactor > 1_000_000 ? "human_attention_typed_go_queue" : "local_llm_inference",
      evidence_kind: "design_estimate"
    },
    data: {
      emulated_receipt_bytes: scaleValue(receiptBytes, scaleFactor),
      emulated_receipt_size_human: humanizeBytes(scaleValue(receiptBytes, scaleFactor)),
      emulated_receipt_records: scaleValue(pat.missions + sat.missions, scaleFactor),
    ...
    impact: {
      completed_missions_estimate: scaleValue(pat.completed + sat.completed, scaleFactor),
      ...
      human_attention_if_30_sec_each: humanizeSeconds(scaleValue(pendingTotal, scaleFactor) * 30),
      estimated_value_usd_if_005_per_completion: Math.round((pat.completed + sat.completed) * scaleFactor * 5) / 100,
...
export function emulateLoopDesign({ seed = DEFAULT_SEED } = {}) {
  ...
  const scales = {
    node0_solo: aggregateScale({ label: "node0_solo", nodeCount: 1, patPerNode: 1000, sampleNodes: 1, seed: normalizedSeed }),
    pilot_100: aggregateScale({ label: "pilot_100", nodeCount: 100, patPerNode: 200, sampleNodes: 100, seed: normalizedSeed }),
    global_1m: aggregateScale({ label: "global_1m", nodeCount: 1_000_000, patPerNode: 100, sampleNodes: 1000, seed: normalizedSeed })
  };
  ...
  return {
    ...
    proof_of_truth_convergence: {
      formal: "Taxonomies, gates, and counters are deterministic for a fixed seed.",
      cryptographic: "No cryptographic receipt is minted or claimed by this preview.",
      empirical: "Use tests and CLI smoke only to verify deterministic preview behavior.",
      economic: "Value fields are assumptions, not measured PoI or financial claims."
    },
    self_critique: [
