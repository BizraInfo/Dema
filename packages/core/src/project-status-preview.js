...
const PMBOK_PRINCIPLES = Object.freeze([
  {
    ...
    bizra_embodiment:
      "CLAUDE.md halt gates · refuse-as-product taxonomy on every surface",
    anchor: "CLAUDE.md",
  },
  {
    id: "team",
    ...
    bizra_embodiment:
      "node-level team · operator + invited humans via Node ordinal law · concentric rings refuse to skip cohorts",
  ...
  },
  {
    ...
    bizra_embodiment:
      "evidence-first GTM rings · Ring-1 N=1 closed (Samy) · refuse to claim rings not earned",
  ...
  },
  {
    ...
    bizra_embodiment:
      "unit of value = IRONCLAD Proof-Forge receipt · not features · not LOC · not commits",
  ...
  },
  {
    ...
    bizra_embodiment:
      "3 structural laws (Node ordinal · Seed-pattern · Skill Growth) jointly govern topology",
  ...
  },
  {
    ...
    bizra_embodiment:
      "operator leads by typing exact-string GO · refuses fuzzy consent · holds Daughter Test before every act",
  ...
  },
  {
    ...
    bizra_embodiment:
      "Master Craftsmanship 10-invariant binding · tailored to each slice (preview vs runtime · pure vs impure)",
  ...
  },
  {
    ...
    bizra_embodiment:
      "5 verification gates run on every commit · 16-key canonical boundary on every spine surface · ≥15 adversarial tests per component",
  ...
  },
  {
    ...
    bizra_embodiment:
      "preview-only spine · runtime lives upstream · scope discipline · same installer for every node",
  ...
  },
  {
    ...
    bizra_embodiment:
      "risk_register surface · refuse-as-product taxonomy · rollback paths documented · halt-gates explicit",
  ...
  },
  {
    ...
    bizra_embodiment:
      "every slice is reversible · install.sh --uninstall returns clean state · receipt chain immutable but configurable per branch",
  ...
  },
  {
    ...
    bizra_embodiment:
      "version-bumped per slice · canon amendments via ADR or explicit canon-edit commit · Skill Growth Law governs self-improvement",
...
function freezeStakeholder(s) {
  return Object.freeze({
    ...
    node_ordinal:
      typeof s?.node_ordinal === "number" && Number.isInteger(s.node_ordinal)
        ? s.node_ordinal
        : null,
...
function freezeRisk(r) {
  ...
  const valid_statuses = [
    ...
    "monitored",
    "mitigated",
    "closed_with_mitigation",
    "accepted",
  ];
  const noMitigation = !isNonEmptyString(r?.mitigation);
  const tryingToClose = ["closed_with_mitigation", "mitigated"].includes(
    r?.status,
  );
...
function freezeOpenGo(go) {
  return Object.freeze({
    ...
    halt_gate_class: isNonEmptyString(go?.halt_gate_class)
      ? go.halt_gate_class
      : null,
  });
}
...
export function buildProjectStatusPreview({
  ...
  const projectOut = Object.freeze({
    ...
    operator_node: isNonEmptyString(proj.operator_node)
      ? proj.operator_node
      : "Node0",
    ...
    start_date_iso: isNonEmptyString(proj.start_date_iso)
      ? proj.start_date_iso
      : null,
  });
  ...
  const vs =
    value_stream && typeof value_stream === "object" ? value_stream : {};
  const valueStreamOut = Object.freeze({
    unit_of_value: "ironclad_proof_forge_receipt",
    receipts_total:
      typeof vs.receipts_total === "number" ? vs.receipts_total : null,
    receipts_today:
      typeof vs.receipts_today === "number" ? vs.receipts_today : null,
    spine_surfaces:
      typeof vs.spine_surfaces === "number" ? vs.spine_surfaces : null,
    structural_laws_canonized:
      typeof vs.structural_laws_canonized === "number"
        ? vs.structural_laws_canonized
        : null,
    tests_total: typeof vs.tests_total === "number" ? vs.tests_total : null,
    tests_failing:
      typeof vs.tests_failing === "number" ? vs.tests_failing : null,
    external_humans_in_canon:
      typeof vs.external_humans_in_canon === "number"
        ? vs.external_humans_in_canon
        : null,
    refusal_explicit:
      "value is NOT counted in features, LOC, commits, stars, or downloads",
  });
  ...
  const qp =
    quality_posture && typeof quality_posture === "object"
      ? quality_posture
      : {};
  const qualityPostureOut = Object.freeze({
    master_craftsmanship_compliance:
      qp.master_craftsmanship_compliance === true,
    five_gate_state: isNonEmptyString(qp.five_gate_state)
      ? qp.five_gate_state
      : "unknown",
    adversarial_floor_per_component:
  ...
  const counters = Object.freeze({
    ...
    stakeholders_active: stakeholdersOut.filter(
      (s) =>
        s.status === "active" ||
        s.status === "ghost_accepted_pending_device_install",
    ).length,
    risks_total: risksOut.length,
    risks_open: risksOut.filter(
      (r) => r.status === "open" || r.status === "monitored",
    ).length,
    risks_refused_close_without_mitigation: risksOut.filter(
      (r) => r.refused_close_without_mitigation,
    ).length,
  ...
  return Object.freeze({
    ...
    project: projectOut,
    current_head_commit: isNonEmptyString(current_head_commit)
      ? current_head_commit
      : null,
