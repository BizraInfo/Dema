...
function deriveRecommendation({ malformed, missingEvidence, forbiddenClaims, snr, processRsi, proofHardeningScore }) {
  ...
  if (snr.verdict === "PREVIEW_REJECT" || processRsi.verdict === "PREVIEW_REJECT") return "HOLD";
  if (snr.score < 0.5 || processRsi.score < 45 || proofHardeningScore < 0.5) return "HOLD";
  return "PROPOSE";
}

function deriveReason({ malformedReason, missingEvidence, forbiddenClaims, recommendation }) {
  ...
  if (recommendation === "HOLD") return "proposal_needs_more_signal_or_proof";
  return "proposal_is_safe_to_review_not_execute";
}
...
export function buildRsiProposalPreview({ evidenceAnchors = [], currentScores = {}, candidate = {}, targetFrameworks = [], processEvents = [], signalEvents, noiseEvents = [] } = {}) {
  ...
  const snr = computeSNRValue({ signalEvents: signalEvents ?? evidence.values, noiseEvents });
  ...
  const reason = deriveReason({ malformedReason, missingEvidence: evidence.missing, forbiddenClaims, recommendation });
  const rsiReadinessScore = malformed || evidence.missing ? 0 : round(0.35 * (snr.score ?? 0) + 0.25 * (processRsi.normalized_score ?? 0) + 0.25 * proofHardeningScore + 0.15 * (forbiddenClaims.length === 0 ? 1 : 0));
  ...
  const body = {
    ...
    what_this_proves: Object.freeze(["A candidate improvement can be scored and proposed from supplied evidence.", "The preview separates proposed_action from executed_action.", "Forbidden live-runtime, network, mint, signing, and self-change claims are rejected."]),
    what_this_does_not_prove: Object.freeze(["RSI is not autonomous or live.", "No code is modified by this kernel.", "No model, network, signing, key generation, mint, reward, PoI, MCP, A2A, or federation runtime is activated.", "This preview does not certify production readiness or economic value."]),
    checks: Object.freeze([{ check: "evidence_present", pass: !evidence.missing }, { check: "candidate_structured", pass: !normalizedCandidate.malformed }, { check: "target_frameworks_structured", pass: !frameworks.malformed }, { check: "forbidden_claims_absent", pass: forbiddenClaims.length === 0 }, { check: "proposed_action_not_executed", pass: true }, { check: "recommendation_valid", pass: RECOMMENDATIONS.includes(recommendation) }]),
