...
const NOTE =
  "Offer declares the resource. Publication is out of scope. Write/execute/call denied. Private types refused by construction.";
...
function validateOwnerNode(owner_node) {
  ...
  if (owner_node.includes("@") || owner_node.includes(":")) {
    return {
      ...
      code: "invalid_owner_node",
      detail:
        "owner_node must not contain '@' or ':' (person-identifier heuristic)",
    };
  }
  return { ok: true };
}
...
function validateNoRawDataProof(no_raw_data_proof) {
  if (
    !isNonEmptyString(no_raw_data_proof) ||
    no_raw_data_proof.trim().length < 30
  ) {
    return {
      ok: false,
      code: "no_raw_data_proof_too_short",
      detail:
        "no_raw_data_proof must be a non-empty string of at least 30 characters",
    };
  }
  return { ok: true };
}
...
function validateCarryingCostReference(carrying_cost_reference) {
  if (
    ...
    carrying_cost_reference === undefined
  ) {
    return { ok: true, normalized: null };
  }
  if (
    typeof carrying_cost_reference !== "string" ||
    !CARRYING_COST_REF_RE.test(carrying_cost_reference)
  ) {
    return {
      ok: false,
      code: "invalid_carrying_cost_reference",
      detail:
        "carrying_cost_reference must be null or match /^chal-[0-9a-f]{32}$/",
    };
  }
...
export function buildUrpResourceOfferPreview({
  ...
  if (!resourceTypeCheck.ok)
    return buildFailureEnvelope(
      resourceTypeCheck.code,
      resourceTypeCheck.detail,
    );

  const ownerCheck = validateOwnerNode(owner_node);
  if (!ownerCheck.ok)
    return buildFailureEnvelope(ownerCheck.code, ownerCheck.detail);

  const effectsCheck = validateEffects(declared_effects, denied_effects);
  if (!effectsCheck.ok)
    return buildFailureEnvelope(effectsCheck.code, effectsCheck.detail);

  const consentCheck = validateConsentField(consent_field_required);
  if (!consentCheck.ok)
    return buildFailureEnvelope(consentCheck.code, consentCheck.detail);

  const verdictCheck = validateSatVerdict(sat_verdict_required);
  if (!verdictCheck.ok)
    return buildFailureEnvelope(verdictCheck.code, verdictCheck.detail);

  const proofCheck = validateNoRawDataProof(no_raw_data_proof);
  if (!proofCheck.ok)
    return buildFailureEnvelope(proofCheck.code, proofCheck.detail);

  const carryingCostCheck = validateCarryingCostReference(
    carrying_cost_reference,
  );
  if (!carryingCostCheck.ok)
    return buildFailureEnvelope(
      carryingCostCheck.code,
      carryingCostCheck.detail,
    );
