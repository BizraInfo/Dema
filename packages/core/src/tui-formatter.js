...
function bold(s, noColor) {
  return noColor ? s : ESC + "1m" + s + ESC + "22m";
}
function dim(s, noColor) {
  return noColor ? s : ESC + "2m" + s + ESC + "22m";
}
function cyan(s, noColor) {
  return noColor ? s : ESC + "36m" + s + ESC + "39m";
}
function green(s, noColor) {
  return noColor ? s : ESC + "32m" + s + ESC + "39m";
}
function yellow(s, noColor) {
  return noColor ? s : ESC + "33m" + s + ESC + "39m";
}
function red(s, noColor) {
  return noColor ? s : ESC + "31m" + s + ESC + "39m";
}
...
export function formatOnboardingLifecyclePreview(
  preview,
  { noColor = false, termDumb = false, width = 76 } = {},
) {
  if (!preview || preview.schema !== "bizra.dema.onboarding_lifecycle.v0.1") {
    return formatError("Expected bizra.dema.onboarding_lifecycle.v0.1 input", {
      noColor,
      termDumb,
      width,
    });
  ...
  lines.push(
    lineBox(`${bold("DEMA · ONBOARDING", noColor)} · ${candidateLabel}`, {
      width,
      c,
    }),
  );
  ...
  for (const stage of preview.stages || []) {
    ...
    const marker = isCompleted
      ? green(c.circle_on, noColor)
      : isCurrent
        ? yellow(c.circle_on, noColor)
        : dim(c.circle_off, noColor);
    const ordTag = `[${stage.order}]`;
    const idText = isCurrent
      ? bold(`${stage.id} · current`, noColor)
      : isCompleted
        ? dim(`${stage.id} · done`, noColor)
        : stage.id;
    lines.push(
      lineBox(`  ${marker} ${dim(ordTag, noColor)} ${idText}`, { width, c }),
    );
  }
  lines.push(lineBox("", { width, c }));
  ...
  if (preview.current_stage?.id) {
    ...
    if (cur) {
      for (const l of wrappedLineBoxes(
        `${bold("Now:", noColor)} ${cur.title}`,
        { width, c },
        "      ",
      )) {
        lines.push(l);
    ...
    lines.push(
      lineBox(
        `${bold("Lifecycle complete.", noColor)} ${green("All 7 stages done.", noColor)}`,
        { width, c },
      ),
    );
    lines.push(lineBox("", { width, c }));
  ...
  lines.push(
    lineBox(
      dim(
        "Boundary: no network · no federation · no runtime · no mint · preview only.",
        noColor,
      ),
      { width, c },
    ),
...
export function formatNodeRegistryPreview(
  preview,
  { noColor = false, termDumb = false, width = 76 } = {},
) {
  if (!preview || preview.schema !== "bizra.dema.node_registry_preview.v0.1") {
    return formatError("Expected bizra.dema.node_registry_preview.v0.1 input", {
      noColor,
      termDumb,
      width,
    });
  ...
  lines.push(
    lineBox(
      `${bold("DEMA · NODE REGISTRY", noColor)}  ${dim("preview · no federation", noColor)}`,
      { width, c },
    ),
  );
  lines.push(dividerBox(width, c));
  ...
  lines.push(
    lineBox(
      `${bold("Connected nodes:", noColor)}  ${green(bold(connectedStr, noColor), noColor)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `${dim("Ghost pending  :", noColor)}  ${(rs.ghost_pending_count ?? 0).toString()}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `${dim("Companion devs :", noColor)}  ${(rs.companion_device_count ?? 0).toString()}`,
      { width, c },
    ),
  );
  lines.push(lineBox("", { width, c }));
  ...
  lines.push(
    lineBox(
      bold("URP shared pool · planned totals per Scaling table:", noColor),
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  ${cyan("PAT agents (local on each node)", noColor)}  : ${(totals.pat_agents ?? 0).toString().padStart(5)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  ${cyan("SAT agents (into shared URP)   ", noColor)}  : ${(totals.sat_agents ?? 0).toString().padStart(5)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  ${dim("Total agents                   ", noColor)}  : ${(totals.total_agents ?? 0).toString().padStart(5)}`,
      { width, c },
    ),
  );
  lines.push(lineBox("", { width, c }));
  ...
  const fedIcon = fedActive
    ? green(c.circle_on, noColor)
    : dim(c.circle_off, noColor);
  const fedLabel = fedActive
    ? "active"
    : "unreachable (by design · runtime lives upstream)";
  lines.push(
    lineBox(`${dim("Federation     :", noColor)}  ${fedIcon} ${fedLabel}`, {
      width,
      c,
    }),
  );
  ...
  if (Array.isArray(rs.accepted) && rs.accepted.length > 0) {
    ...
    for (const node of rs.accepted) {
      ...
      const companion = node.companion_of
        ? ` ${dim("(companion)", noColor)}`
        : "";
      lines.push(
        lineBox(
          `  ${green(c.circle_on, noColor)} ${node.node_label}${candidate}${companion}`,
          { width, c },
        ),
      );
    }
  ...
  if (Array.isArray(rs.ghost) && rs.ghost.length > 0) {
    lines.push(
      lineBox(bold("Ghost (pending acceptance):", noColor), { width, c }),
    );
    for (const node of rs.ghost) {
      const candidate = node.candidate_name ? ` · ${node.candidate_name}` : "";
      lines.push(
        lineBox(
          `  ${yellow(c.circle_on, noColor)} ${node.node_label}${candidate}`,
          { width, c },
        ),
      );
      if (node.ordinal_claim_phrase) {
        lines.push(
          lineBox(
            `     ${dim("type to accept:", noColor)} ${cyan(`"${node.ordinal_claim_phrase}"`, noColor)}`,
            { width, c },
          ),
        );
      }
  ...
  // Forbidden ordinals
  if (
    Array.isArray(rs.forbidden_ordinals) &&
    rs.forbidden_ordinals.length > 0
  ) {
    lines.push(
      lineBox(
        `${dim("Forbidden ordinals:", noColor)} [${rs.forbidden_ordinals.join(", ")}] ${dim("(canon-registry · amend to lift)", noColor)}`,
        { width, c },
      ),
    );
    lines.push(lineBox("", { width, c }));
  ...
  lines.push(
    lineBox(
      dim(
        "Boundary: all 16 canonical keys pinned false. Preview only.",
        noColor,
      ),
      { width, c },
    ),
...
export function formatSkillGrowthGovernorPreview(
  preview,
  { noColor = false, termDumb = false, width = 76 } = {},
) {
  if (!preview || preview.schema !== "bizra.dema.skill_growth_governor.v0.1") {
    return formatError("Expected bizra.dema.skill_growth_governor.v0.1 input", {
      noColor,
      termDumb,
      width,
    });
  ...
  lines.push(
    lineBox(
      `${bold("DEMA · SKILL GROWTH GOVERNOR", noColor)}  ${dim("preview · proof-governed", noColor)}`,
      { width, c },
    ),
  );
  lines.push(dividerBox(width, c));
  ...
  lines.push(
    lineBox(
      `  Candidates total      : ${(ct.candidates_total ?? 0).toString().padStart(3)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  ${green("Promotable", noColor)}             : ${(ct.candidates_promotable ?? 0).toString().padStart(3)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  ${yellow("Proposed (gates pending)", noColor)} : ${(ct.candidates_proposed ?? 0).toString().padStart(3)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  ${red("Halted (refused)", noColor)}      : ${(ct.candidates_halted ?? 0).toString().padStart(3)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  Human-edited protected: ${(ct.human_edited_skills_protected ?? 0).toString().padStart(3)}  ${dim("(sacred)", noColor)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  Pinned skills         : ${(ct.pinned_skills ?? 0).toString().padStart(3)}`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  Total refusals        : ${(ct.refusals_total ?? 0).toString().padStart(3)}`,
      { width, c },
    ),
  );
  lines.push(lineBox("", { width, c }));
  ...
  if (
    ...
    for (const e of preview.candidate_evaluations) {
      ...
      lines.push(
        lineBox(
          `  ${actionColor(actionMark, noColor)} ${e.skill_id} v${e.candidate_version}  ${dim("→", noColor)} ${actionColor(bold(e.next_action, noColor), noColor)}`,
          { width, c },
        ),
      );
      // Show per-gate results
      for (const [gName, gResult] of Object.entries(e.gates ?? {})) {
        const passSym = gResult.passed
          ? green(c.circle_on, noColor)
          : red(c.circle_off, noColor);
        const reason = gResult.passed
          ? ""
          : `  ${dim("· " + (gResult.reason ?? ""), noColor)}`;
        for (const l of wrappedLineBoxes(
          `      ${passSym} ${gName}${reason}`,
          { width, c },
          "        ",
        )) {
          lines.push(l);
      ...
      if (!e.gates?.human_consent_received?.passed) {
        for (const l of wrappedLineBoxes(
          `      ${dim("type to promote:", noColor)} ${cyan(`"${e.promotion_phrase_required}"`, noColor)}`,
          { width, c },
          "        ",
        )) {
          lines.push(l);
  ...
  if (
    Array.isArray(preview.protected_namespaces) &&
    preview.protected_namespaces.length > 0
  ) {
    lines.push(
      lineBox(
        `${dim("Protected namespaces:", noColor)} ${preview.protected_namespaces.join(", ")} ${dim("(no skills here without override)", noColor)}`,
        { width, c },
      ),
    );
    lines.push(lineBox("", { width, c }));
  ...
  lines.push(
    lineBox(
      dim(
        "Boundary: preview-only · no overwrite · no auto-promotion · refusal-as-product.",
        noColor,
      ),
      { width, c },
    ),
...
export function formatProjectStatusPreview(
  preview,
  { noColor = false, termDumb = false, width = 76 } = {},
) {
  if (!preview || preview.schema !== "bizra.dema.project_status.v0.1") {
    return formatError("Expected bizra.dema.project_status.v0.1 input", {
      noColor,
      termDumb,
      width,
    });
  ...
  lines.push(
    lineBox(
      `${bold("BIZRA / DEMA · PROJECT STATUS", noColor)}  ${dim("PMBOK 7th-edition aligned", noColor)}`,
      { width, c },
    ),
  );
  lines.push(dividerBox(width, c));
  ...
  lines.push(
    lineBox(
      `  ${dim("operator:", noColor)} ${p.operator ?? "?"} · ${dim("phase:", noColor)} ${p.current_phase ?? "?"}`,
      { width, c },
    ),
  );
  lines.push(lineBox("", { width, c }));
  ...
  lines.push(
    lineBox(
      `  ${dim("Unit of value:", noColor)} ${cyan(vs.unit_of_value ?? "?", noColor)}`,
      { width, c },
    ),
  );
  if (vs.receipts_total !== null && vs.receipts_total !== undefined) {
    lines.push(
      lineBox(
        `  ${green("Receipts on chain", noColor)}    : ${vs.receipts_total.toString().padStart(5)}  ${dim("IRONCLAD-class at head", noColor)}`,
        { width, c },
      ),
    );
  }
  if (vs.spine_surfaces !== null && vs.spine_surfaces !== undefined) {
    lines.push(
      lineBox(
        `  ${green("Canonical spine surfaces", noColor)} : ${vs.spine_surfaces.toString().padStart(5)}`,
        { width, c },
      ),
    );
  }
  if (
    vs.structural_laws_canonized !== null &&
    vs.structural_laws_canonized !== undefined
  ) {
    lines.push(
      lineBox(
        `  ${green("Structural laws canonized", noColor)} : ${vs.structural_laws_canonized.toString().padStart(5)}  ${dim("of 3 known", noColor)}`,
        { width, c },
      ),
    );
  }
  if (vs.tests_total !== null && vs.tests_total !== undefined) {
    lines.push(
      lineBox(
        `  ${green("Tests passing", noColor)}            : ${vs.tests_total.toString().padStart(5)}  ${dim("/", noColor)} ${(vs.tests_failing ?? 0).toString().padStart(3)} ${dim("failing", noColor)}`,
        { width, c },
      ),
    );
  }
  if (
    vs.external_humans_in_canon !== null &&
    vs.external_humans_in_canon !== undefined
  ) {
    lines.push(
      lineBox(
        `  ${green("External humans in canon", noColor)} : ${vs.external_humans_in_canon.toString().padStart(5)}`,
        { width, c },
      ),
    );
  }
  for (const l of wrappedLineBoxes(
    `  ${dim("Refusal: " + (vs.refusal_explicit ?? ""), noColor)}`,
    { width, c },
    "  ",
  )) {
    lines.push(l);
  ...
  if (Array.isArray(preview.stakeholders) && preview.stakeholders.length > 0) {
    lines.push(
      lineBox(bold("Stakeholders (concentric rings):", noColor), { width, c }),
    );
    for (const s of preview.stakeholders) {
      const ring =
        s.role === "founder"
          ? "Ring 0"
          : s.role === "first_invited"
            ? "Ring 1"
            : s.role === "candidate"
              ? "Ring 1.5"
              : s.role.startsWith("future_ring_")
                ? `Ring ${s.role.match(/\d+/)?.[0] ?? "?"}`
                : "—";
      const id = s.name ? s.name : (s.node_label ?? "?");
      lines.push(
        lineBox(
          `  ${dim(ring, noColor)} · ${id}  ${dim("·", noColor)} ${s.status}`,
          { width, c },
        ),
      );
    }
  ...
  if (
    Array.isArray(preview.risk_register) &&
    preview.risk_register.length > 0
  ) {
    lines.push(
      lineBox(
        bold(`Risk register (${preview.risk_register.length}):`, noColor),
        { width, c },
      ),
    );
    for (const r of preview.risk_register.slice(0, 5)) {
      ...
      lines.push(
        lineBox(
          `  ${sevColor(c.bullet, noColor)} ${r.risk_id} ${dim("[" + r.severity + "]", noColor)} ${r.title}`,
          { width, c },
        ),
      );
    }
    if (preview.risk_register.length > 5) {
      lines.push(
        lineBox(
          `  ${dim("... " + (preview.risk_register.length - 5) + " more · see project-status --json", noColor)}`,
          { width, c },
        ),
      );
    }
    if (ct.risks_refused_close_without_mitigation > 0) {
      lines.push(
        lineBox(
          `  ${red("✗ refused-close-without-mitigation:", noColor)} ${ct.risks_refused_close_without_mitigation}`,
          { width, c },
        ),
      );
    }
  ...
  const mcc = qp.master_craftsmanship_compliance
    ? green("yes", noColor)
    : red("no", noColor);
  lines.push(lineBox(`  Master Craftsmanship  : ${mcc}`, { width, c }));
  lines.push(
    lineBox(`  5-gate state           : ${qp.five_gate_state ?? "?"}`, {
      width,
      c,
    }),
  );
  lines.push(
    lineBox(
      `  Adversarial test floor: ${qp.adversarial_floor_per_component ?? "?"} per component`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  Canonical boundary    : ${qp.canonical_boundary_keys ?? 16} keys all false`,
      { width, c },
    ),
  );
  lines.push(lineBox("", { width, c }));
  ...
  lines.push(
    lineBox(
      `${dim("PMBOK 7th-edition · 12 principles surfaced (run --json for full mapping):", noColor)}`,
      { width, c },
    ),
  );
  const principleIds = preview.pmbok_principles.map((p) => p.id).join(", ");
  for (const l of wrappedLineBoxes(
    `  ${dim(principleIds, noColor)}`,
    { width, c },
    "  ",
  )) {
    lines.push(l);
  ...
  lines.push(
    lineBox(
      `  Stakeholders : ${ct.stakeholders_total ?? 0} total · ${ct.stakeholders_active ?? 0} active`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(
      `  Risks         : ${ct.risks_total ?? 0} total · ${ct.risks_open ?? 0} open/monitored`,
      { width, c },
    ),
  );
  lines.push(
    lineBox(`  Open typed-GOs: ${ct.open_typed_gos ?? 0}`, { width, c }),
  );
  lines.push(
    lineBox(`  Deferred acts : ${ct.deferred_actions ?? 0}`, { width, c }),
  );
  lines.push(lineBox("", { width, c }));
  ...
  lines.push(
    lineBox(
      dim(
        "Human-readable canon: docs/pm/PROJECT_CHARTER_AND_STATUS.md",
        noColor,
      ),
      { width, c },
    ),
  ...
  lines.push(
    lineBox(
      dim(
        "Boundary: preview-only · receipt-bound · refuse-as-product · canonical 16-key all-false.",
        noColor,
      ),
      { width, c },
    ),
...
export function formatHomebasePreview(
  preview,
  { noColor = false, termDumb = false, palette = "24bit", width = 76 } = {},
) {
  if (!preview || preview.schema !== "bizra.dema.homebase_v0_1.v0.1") {
    return formatError("Expected bizra.dema.homebase_v0_1.v0.1 input", {
      noColor,
      termDumb,
      width,
    });
  ...
  lines.push(topBorder(width, c));
  const headerLeft = Theme.title(`DEMA · ${h.node_name ?? "Node0"}`, {
    noColor,
    palette,
  });
  ...
  lines.push(
    lineBox(`${headerLeft}  ${headerMid}  ${headerRight}`, { width, c }),
  );
  lines.push(dividerBox(width, c));
  lines.push(lineBox("", { width, c }));
  ...
  if (m3.fallback_text) {
    ...
    lines.push(
      lineBox(bold("Three things I remember:", noColor), { width, c }),
    );
      ...
      if (e) {
        const label =
          typeof e.summary === "string" && e.summary.length > 0
            ...
            : (e.name ?? "?");
        lines.push(
          lineBox(`  ${dim(i + 1 + ".", noColor)} ${label}`, { width, c }),
        );
      } else {
  ...
  if (st.ring) {
    ...
    lines.push(
      lineBox(`  ${dim("Node0  ", noColor)}${barStr}  ${st.ring.label ?? ""}`, {
        width,
        c,
      }),
    );
  ...
  if (st.mission) {
    const icon =
      st.mission.icon ??
      (st.mission.label === "active" ? c.circle_on : c.circle_off);
    const missionColor = st.mission.label === "active" ? green : dim;
    lines.push(
      lineBox(
        `  ${dim("Mission", noColor)} ${missionColor(icon, noColor)}  ${st.mission.label ?? "?"}`,
        { width, c },
      ),
    );
  }
  if (st.gateway) {
    ...
    lines.push(
      lineBox(
        `  ${dim("Gateway", noColor)} ${dim(icon, noColor)}  ${dim(st.gateway.label ?? "", noColor)}`,
        { width, c },
      ),
    );
  }
  if (st.memory_bar) {
    const barStr =
      typeof st.memory_bar.bar === "string" ? st.memory_bar.bar : "";
    lines.push(
      lineBox(
        `  ${dim("Memory ", noColor)}${barStr}  ${dim(st.memory_bar.label ?? "", noColor)}`,
        { width, c },
      ),
    );
  }
  ...
  const actionText =
    typeof na.text === "string" ? na.text : "press ? to see available actions";
  for (const l of wrappedLineBoxes(
    `  ${c.arrow} ${cyan(actionText, noColor)}`,
    { width, c },
    "    ",
  )) {
    lines.push(l);
  ...
  lines.push(dividerBox(width, c));
  if (affs.length > 0) {
    const hintParts = affs.map(
      (a) => `${bold(`[${a.key}]`, noColor)} ${a.label}`,
    );
  ...
  if (preview.partial) {
    lines.push(
      lineBox(
        `${yellow(c.bullet, noColor)} ${dim("partial state · " + (preview.warnings?.length ?? 0) + " warning(s) · run with --json for detail", noColor)}`,
        { width, c },
      ),
    );
  }
...
function formatError(msg, { noColor, termDumb, width }) {
  ...
  lines.push(topBorder(width, c));
  lines.push(
    lineBox(red(bold("formatter error:", noColor), noColor), { width, c }),
  );
...
export function resolvePaletteFromEnv(env = process.env) {
  ...
  if (
    env.DEMA_PALETTE === "24bit" ||
    env.DEMA_PALETTE === "256" ||
    env.DEMA_PALETTE === "none"
  ) {
...
export function resolveFormatterOptsFromEnv(env = process.env) {
  ...
  return {
    ...
    palette,
    width: Number.isFinite(Number(env.DEMA_TUI_WIDTH))
      ? Number(env.DEMA_TUI_WIDTH)
      : 76,
  };
}
