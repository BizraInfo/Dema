...
const INTEGRATION_POINTS = [
  {
    ...
    purpose:
      "inspect workflow metadata, runs, jobs, and logs for release triage",
    allowed_methods: [
  ...
  },
  {
    ...
    server: "github-mcp-server",
    purpose:
      "read operator-selected Copilot Space context when explicitly named",
    allowed_methods: ["list_copilot_spaces", "get_copilot_space"],
    forbidden_methods: [
      "copying secrets",
      "publishing space content",
      "implicit space scanning",
    ],
    credential_source: "host_mcp_configuration",
...
const DATA_TRANSFORMATIONS = [
  ...
  },
  {
    ...
    output:
      "error class, failing step, and suggested local reproduction command",
...
const PROOF_OF_TRUTH_CONVERGENCE = {
  formal: {
    ...
    certifies: false,
    claim:
      "MCP integration points, controls, and boundaries are schema-tagged for review.",
  },
  cryptographic: {
    label: "Cryptographic",
    ...
    certifies: false,
    claim:
      "Dema does not store credentials or mint signed MCP access artifacts.",
  },
  empirical: {
    label: "Empirical",
    ...
    certifies: false,
    claim:
      "A live MCP tool should be checked read-only before any operation plan depends on it.",
  },
  economic: {
    label: "Economic",
    ...
    certifies: false,
    claim:
      "MCP reads do not create token, reward, spend, or Proof-of-Impact claims.",
  },
};

// MCP-domain boundary vocabulary per per-module-domain-boundary-pattern.
// Coexists with the canonical 16-key shape (preview-boundary.js). These
// keys capture MCP-specific runtime risks: read-only preview posture,
// host auth deferral, credential scope, mutation explicitly disambiguated
// via mcp_ prefix, and no network connection attempted by command.
const BOUNDARY = {
...
export function formatMcpIntegrationBlueprint(blueprint) {
  ...
  appendRows(
    ...
    (point) =>
      `${point.id} via ${point.server} credentials=${point.credential_source}: ${point.purpose}`,
  );
  lines.push("");
  lines.push("Security controls:");
  appendRows(
    lines,
    blueprint.security_controls,
    (control) => `${control.id}: ${control.rule}`,
  );
  lines.push("");
  ...
  appendRows(
    lines,
    blueprint.data_transformations,
    (item) => `${item.id}: ${item.output}`,
  );
  lines.push("");
