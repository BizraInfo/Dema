...
export const URP_SHARED_RUNTIME_DISCOVERY_SCHEMA = "bizra.dema.urp_shared_runtime_discovery.v0.1";
export const URP_SHARED_STATE_MANIFEST_SCHEMA = "bizra.dema.urp_shared_state_manifest.v0.1";
export const URP_SHARED_WRITE_BOUNDARY_SCHEMA = "bizra.dema.urp_shared_write_boundary.v0.1";

export const URP_SHARED_MANIFEST_RELATIVE_PATH = "urp/shared-state-manifest.json";
...
export function buildUrpSharedStateManifest({
  ...
  return deepFreeze(clone({
    ...
    manifest_version: typeof manifest_version === "number" && manifest_version > 0 ? manifest_version : 1,
...
function satPipelineAllowsWrite(sat_pipeline) {
  ...
  if (pipeline.overall_verdict !== "pipeline_verified" || pipeline.passed !== true) {
...
export function evaluateUrpSharedWriteBoundary({
  ...
  if (!ALLOWED_WRITE_KINDS_DISCOVERY.includes(kind) && !FORBIDDEN_WRITE_KINDS.includes(kind)) {
  ...
  const candidateId = safeString(candidateObj?.candidate_id || candidateObj?.id);
  ...
  if (candidateObj?.tier !== "URP_SHAREABLE" && candidateObj?.tier != null) {
    violations.push({ code: "tier_not_urp_shareable", detail: safeString(candidateObj.tier) });
  }
...
export function buildUrpSharedRuntimeDiscovery({
  ...
  return deepFreeze(clone({
    ...
    boundary: DISCOVERY_BOUNDARY,
    urp_discovery_flags: URP_DISCOVERY_FLAGS,
    next_safe_action: "run SAT pipeline on a PAT-proposed artifact before any URP manifest persist slice"
  }));
}
