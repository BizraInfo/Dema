...
const CANONICAL_STAGES = Object.freeze([
  {
    ...
    refusal_paths: [
      "candidate_does_not_pick_a_language_block_all_downstream_stages",
    ],
    boundary_note:
      "no action may proceed until language is set · consent is invalid if not understood",
  },
  {
    id: "technical_level",
    order: 1,
    ...
    options_default: Object.freeze([
      ...
      { level: 2, label: "Balanced", note: "explain important terms" },
      {
        level: 3,
        label: "Technical",
        note: "show commands, schemas, and proof",
      },
      { level: 4, label: "Expert", note: "show full diagnostic surfaces" },
    ]),
    refusal_paths: [
      "candidate_picks_level_higher_than_their_actual_comfort_break_explanation_density",
    ],
  ...
  },
  {
    ...
    boundary_note:
      "Node1 does not mean control over Node0 · Node0 does not mean ownership of Node1's data",
  },
  {
    id: "purpose",
    order: 3,
    ...
    boundary_note:
      "purpose declared here shapes future mission proposals · purpose can be revised any time",
  },
  {
    id: "resources",
    order: 4,
    ...
    options_default: Object.freeze([
      {
        id: "local_models",
        label: "Local models (inventory only · no invocation)",
        default_selected: false,
      },
      {
        id: "selected_folders",
        label: "Selected folders (declared explicitly)",
        default_selected: false,
      },
      {
        id: "notes_documents",
        label: "Notes / documents (declared explicitly)",
        default_selected: false,
      },
      {
        id: "chat_exports",
        label: "Chat exports (declared explicitly)",
        default_selected: false,
      },
      {
        id: "calendar_tasks",
        label: "Calendar / tasks (declared explicitly)",
        default_selected: false,
      },
      {
        id: "nothing_yet",
        label: "Nothing yet · show me first",
        default_selected: true,
      },
    ]),
    safest_default: "nothing_yet",
    refusal_paths: [
      "candidate_picks_everything_without_understanding_each_class_halt_for_clarification",
    ],
    boundary_note:
      "default is nothing_yet · operator must explicitly opt-in each class",
  },
  {
    ...
    refusal_paths: [
      "candidate_does_not_acknowledge_constitution_halt_no_action_allowed",
    ],
    boundary_note:
      "ADR-005 binding · constitution holds for every future L1+ action",
  },
  {
    id: "first_mission",
    order: 6,
    ...
    options_default: Object.freeze([
      {
        id: "free_text",
        label: "Type your first intent in your preferred language",
      },
    ]),
    example_intents_by_purpose: Object.freeze({
      ...
      coding_and_building:
        "Scan my project folder and produce a safe next-step plan",
    ...
    refusal_paths: [
      "candidate_skips_first_mission_record_node_state_inactive_intent_pending",
    ],
    boundary_note:
      "first mission is preview-only · proposal not execution · standard consent gating applies",
  },
]);
  ...
  for (const candidate of CANONICAL_STAGES) {
    ...
    if ((candidate.required_before ?? []).includes(stage.id)) {
      if (!completed.has(candidate.id)) {
        return {
          allowed: false,
          reason: `requires_completion_of_${candidate.id}`,
        };
      }
    }
  }
...
function freezeStage(stage, candidateOrdinal) {
  ...
  const title = String(stage.title).replace(
    "{ordinal}",
    String(candidateOrdinal),
  );
...
export function buildOnboardingLifecyclePreview({
  ...
  const allowanceCheck = current
    ? shouldAllowStage(current, progress)
  ...
  const allStagesComplete = GENESIS_REQUIRED_STAGES.every((id) =>
    completedSet.has(id),
  );
  ...
  const genesisPreviewCard = allStagesComplete
    ? buildGenesisPreviewCard({
        candidate: {
          ...
          preferred_name:
            typeof candidate_name === "string" ? candidate_name : null,
...
export const ONBOARDING_LIFECYCLE_STAGE_IDS = Object.freeze(
  CANONICAL_STAGES.map((s) => s.id),
);
