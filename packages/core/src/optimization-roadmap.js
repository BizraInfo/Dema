...
const PMBOK_LENSES = [
  {
    ...
    focus: "keep roadmap slices coherent across product, proof, review gates, CI evidence, and operator UX",
  ...
  },
  {
    ...
    focus: "map performance, test, check, guidance, and diff hygiene evidence without enforcing gates here",
    canon_source: "docs/TESTING.md"
  },
  {
    id: "schedule_management",
    focus: "sequence roadmap dependencies so CI/CD, quality, and proof evidence mature before runtime handoff",
    canon_source: "docs/ENGINEERING_DISCIPLINE.md"
  },
  {
    id: "communications_management",
    focus: "keep roadmap status, blockers, evidence anchors, and non-goals visible to operators and reviewers",
    canon_source: "docs/LLM_SYSTEM_FLOW.md"
  },
  {
    id: "risk_management",
    focus: "surface launch blockers, cascading dependencies, and proof gaps before execution",
    canon_source: "docs/ENGINEERING_DISCIPLINE.md"
  },
  {
...
const MANAGEMENT_BODY_OF_KNOWLEDGE = [
  {
    id: "mbok.integration",
    practice: "single roadmap schema for workstreams, dependencies, gates, and boundaries",
    advisory_signal: "one preview can be reviewed without changing runtime state",
    canon_source: "docs/LLM_SYSTEM_FLOW.md"
  },
  {
    id: "mbok.quality",
    practice: "verification ladder and performance-quality assurance evidence",
    advisory_signal: "quality posture is visible before any proposed gate is enforced",
    canon_source: "docs/TESTING.md"
  },
  {
    id: "mbok.risk",
    practice: "dependency graph review, blocker labels, and proof-safe language",
  ...
  },
  {
    ...
    advisory_signal: "stakeholders can distinguish advisory roadmap text from runtime authority",
    canon_source: "README.md"
  }
];
...
const SNR_LENSES = [
  {
    ...
    signal: "roadmap outputs favor exact evidence anchors, deterministic statuses, and explicit non-effects",
    noise_guard: "avoid success claims, runtime implication, token language, or hidden automation language",
    canon_source: "docs/LLM_SYSTEM_FLOW.md"
  },
  {
    id: "snr.review_compression",
    signal: "each roadmap item carries one primary blueprint focus and one quality signal",
    noise_guard: "avoid sprawling implementation instructions or CI workflow edits in the preview",
    canon_source: "docs/TESTING.md"
  },
  {
    id: "snr.blocker_visibility",
    signal: "proposed gates carry non-enforcing status and ownership labels",
    noise_guard: "avoid implying a gate is active unless local checks already provide read-only evidence",
...
const SAPE_LENSES = [
  {
    id: "symbolic",
    signal: "schema-tagged previews, finite gates, explicit blocked effects, and deterministic CI/CD labels",
    next_probe: "Preview stronger types for consent-spine actuator classes.",
    canon_source: "docs/superpowers/specs/2026-05-14-actuator-boundary-spine/01_specification.md"
  },
  {
    id: "abstraction",
    signal: "Dema acts as product face while Node0 remains governed runtime boundary",
    next_probe: "Keep roadmap items mapped to architecture, security, performance-quality, CI/CD, docs, or ethics.",
    canon_source: "docs/ARCHITECTURE.md"
  },
  {
    id: "probe",
    signal: "read-only checks turn doctrine into repeatable local evidence",
    next_probe: "Keep actuator, canon, and roadmap checks as advisory release evidence.",
    canon_source: "scripts/check.mjs"
  },
  {
    id: "elevation",
    signal: "Ihsan, Adl, and Amanah become reviewable engineering constraints",
    next_probe: "Attach ethical acceptance criteria to each future runtime handoff contract.",
...
const BLUEPRINT_COVERAGE = [
  {
    id: "blueprint.devops",
    domain: "devops",
    advisory_scope: "release hygiene, ownership labels, dependency visibility, and local verification sequencing",
    non_goal: "no CI workflow modification and no deployment path"
  },
  {
    id: "blueprint.ci_cd",
    domain: "ci_cd",
    advisory_scope: "immutable action review, proposed gate status, artifact naming, and check summarization",
    non_goal: "no gate enforcement and no hosted pipeline mutation"
  },
  {
    id: "blueprint.pipeline_automation",
    domain: "pipeline_automation",
    advisory_scope: "manual-to-automatable step inventory with consent and boundary labels",
  ...
  },
  {
    ...
    advisory_scope: "local-loop measurement contract, regression budget labels, and repeatable evidence anchors",
  ...
  },
  {
    ...
    advisory_scope: "Ihsan, Adl, Amanah, consent clarity, and anti-overclaiming acceptance criteria",
...
const ROADMAP_ITEMS = [
  {
    ...
    outcome: "Operators can see whether an intent implies Bash, filesystem mutation, external call, GUI, mobile-agent, or spend risk.",
    ...
    integrity_constraint: "exact-string consent remains required for any future operator action"
  },
  {
    id: "R2",
    priority: 2,
    ...
    outcome: "Intent, policy decision, and EffectCap handoff status render as one schema-tagged preview.",
  ...
  },
  {
    ...
    outcome: "CI launch blocker becomes a reviewable evidence inventory without changing workflows in this slice.",
  ...
  },
  {
    ...
    outcome: "Manual verification, advisory gates, and future automation candidates are separated by consent and effect class.",
    ...
    integrity_constraint: "no runner, daemon, dispatch, or hidden scheduler is introduced"
  },
  {
    id: "R5",
    priority: 5,
    ...
    outcome: "Design emulation can later consume measured Node0 profile data without upgrading claims.",
    ...
    integrity_constraint: "no benchmark certification or runtime performance claim"
  },
  {
    id: "R6",
    priority: 6,
    ...
    outcome: "Docs distinguish preview, placeholder, partial, and certified states for receipt-facing users.",
  ...
  },
  {
    ...
    outcome: "Reviewers can compare scope, quality, risk, schedule, and stakeholder signals before any runtime handoff proposal.",
    ...
    integrity_constraint: "scorecard status is advisory and cannot certify readiness"
  },
  {
    id: "R8",
    priority: 8,
    ...
    outcome: "Future runtime work can show Ihsan, Adl, and Amanah criteria before any execution path exists.",
    evidence_anchor: "docs/LLM_SYSTEM_FLOW.md",
    depends_on: ["R1", "R6", "R7"],
    effect_class: "advisory_only",
    requires_upstream: true,
    ...
    integrity_constraint: "no token, Proof-of-Impact, identity-bound, or economic claim"
  }
];
...
const PROPOSED_GATES = [
  {
    ...
    command: "npm test && npm run check && npm run llm:guidance && git diff --check",
  ...
  },
  {
    ...
    command: "review Ihsan, Adl, Amanah, consent clarity, and anti-overclaiming criteria",
    status: "not_enforced",
...
const PROOF_OF_TRUTH_CONVERGENCE = {
  formal: {
    ...
    certifies: false,
    claim: "Roadmap items, dependencies, gates, and boundaries are explicit and schema-tagged."
  },
  cryptographic: {
    label: "Cryptographic",
    ...
    certifies: false,
    claim: "This preview does not hash-commit runtime outcomes or mint receipts."
  },
  empirical: {
    label: "Empirical",
    ...
    certifies: false,
    claim: "Local verification commands can check the current code surface before any roadmap item advances."
  },
  economic: {
    label: "Economic",
    ...
    certifies: false,
    claim: "No reward, token, passive income, or Proof-of-Impact value is claimed."
  }
};
...
function sortedItems() {
  return clone(ROADMAP_ITEMS).sort((a, b) => a.priority - b.priority || a.id.localeCompare(b.id));
}
...
export function formatOptimizationRoadmapPreview(report) {
  ...
  appendRows(lines, report.management_body_of_knowledge, (lens) => `${lens.id}: ${lens.practice}`);
  lines.push("");
  ...
  appendRows(lines, report.blueprint_coverage, (blueprint) => `${blueprint.domain}: ${blueprint.advisory_scope}`);
  lines.push("");
  lines.push("Prioritized roadmap:");
  appendRows(
    lines,
    report.roadmap_items,
    (item) => `${item.priority}. ${item.id} [${item.workstream}] ${item.title} (${item.effect_class})`
  );
  lines.push("");
  lines.push("Cascading risk graph:");
  appendRows(lines, report.risk_graph.edges, (edge) => `${edge.from} -> ${edge.to}: ${edge.risk}`);
  lines.push("");
  lines.push("Proposed gates:");
  appendRows(lines, report.proposed_gates, (gate) => `${gate.status}: ${gate.id} - ${gate.command}`);
  lines.push("");
