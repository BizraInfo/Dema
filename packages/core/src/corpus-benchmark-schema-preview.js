export const CORPUS_BENCHMARK_SCHEMA_PREVIEW_SCHEMA = "bizra.dema.corpus_benchmark_schema_preview.v0.1";
...
const DATA_TIERS = Object.freeze([
  Object.freeze({ tier: "D0", label: "public_or_non_sensitive", benchmark_schema_allowed: true }),
  Object.freeze({ tier: "D1", label: "preferences_and_style", benchmark_schema_allowed: true }),
  Object.freeze({ tier: "D2", label: "project_reasoning", benchmark_schema_allowed: true }),
  Object.freeze({ tier: "D3", label: "private_strategy", benchmark_schema_allowed: false }),
  Object.freeze({ tier: "D4", label: "secrets_credentials_identity_financial_health", benchmark_schema_allowed: false })
]);
...
const EXPECTED_DISPOSITIONS = Object.freeze(["score_eval_only", "candidate_skill_pattern_preview", "candidate_preference_preview", "quarantine_private_strategy", "reject_secret_identity_or_credential"]);

const PROOF_OF_TRUTH_AXES = Object.freeze(["formal", "cryptographic", "empirical", "economic"]);
const SCORING_AXES = Object.freeze(["answer_correctness", "truth_discipline", "safety_compliance", "evidence_alignment", "uncertainty_calibration", "operator_utility"]);
const SAPE_PROBES = Object.freeze(["rare_path_probe", "symbolic_neural_bridge", "abstraction_lift", "logic_creative_tension"]);
...
const ALLOWED_USES = Object.freeze([
  "benchmark_schema_design", "manual_review_queue_design", "skill_pattern_candidate_schema",
  "preference_dataset_candidate_schema", "truth_discipline_eval_design", "sape_probe_design", "snr_scoring_design"
]);
...
const BLOCKED_USES = Object.freeze([
  ...
  "skill_pattern_promotion", "preference_dataset_promotion", "sharing_with_node1_node4", "receipt_minting",
  "step7_minting"
]);
...
const DEFAULT_CASES = Object.freeze([
  Object.freeze({
    ...
    scoring_axes: Object.freeze(["answer_correctness", "truth_discipline", "evidence_alignment"]),
  ...
  Object.freeze({
    ...
    declared_tags: Object.freeze(["debugging", "tool_reasoning", "skill_pattern"]),
    scoring_axes: Object.freeze(["answer_correctness", "safety_compliance", "operator_utility"]),
  ...
  Object.freeze({
    ...
    declared_tags: Object.freeze(["preference_pair", "agentic_workflow", "uncertainty"]),
    scoring_axes: Object.freeze(["truth_discipline", "uncertainty_calibration", "operator_utility"]),
    proof_of_truth_axes: Object.freeze(["formal", "empirical"]),
  ...
  Object.freeze({
    ...
    scoring_axes: Object.freeze(["truth_discipline", "safety_compliance", "evidence_alignment"]),
  ...
  Object.freeze({
    ...
    declared_tags: Object.freeze(["private_strategy", "quarantine", "eval_only"]),
...
function validateD3D4Rule(benchmarkCase) {
  if (benchmarkCase.tier === "D3") {
    return benchmarkCase.sample_role === "evaluation_only"
      && benchmarkCase.expected_disposition === "quarantine_private_strategy";
  }
  if (benchmarkCase.tier === "D4") {
    return benchmarkCase.sample_role === "evaluation_only"
      && benchmarkCase.expected_disposition === "reject_secret_identity_or_credential";
  }
  return !["quarantine_private_strategy", "reject_secret_identity_or_credential"].includes(
...
function validateCases(cases) {
  ...
  for (const benchmarkCase of cases) {
    ...
    if (!isSafeIdentifier(benchmarkCase.case_id) || seen.has(benchmarkCase.case_id)) {
      return { ok: false, reason: "case_id_must_be_unique_safe_identifier" };
    }
    seen.add(benchmarkCase.case_id);
    if (!SOURCE_IDS.includes(benchmarkCase.source_id)) return { ok: false, reason: "source_id_not_allowlisted" };
    if (!DOMAINS.includes(benchmarkCase.domain)) return { ok: false, reason: "domain_not_allowlisted" };
    if (!DIFFICULTIES.includes(benchmarkCase.difficulty)) return { ok: false, reason: "difficulty_not_allowlisted" };
    if (!DATA_TIERS.some((definition) => definition.tier === benchmarkCase.tier)) {
    ...
    if (!SAMPLE_ROLES.includes(benchmarkCase.sample_role)) return { ok: false, reason: "sample_role_not_allowlisted" };
    ...
    if (!validateD3D4Rule(benchmarkCase)) {
      return { ok: false, reason: "d3_d4_must_be_evaluation_only_and_quarantined_or_rejected" };
    }
    if (!Array.isArray(benchmarkCase.declared_tags)
...
function rejectPreview(reason) {
  return deepFreeze({
    ...
    self_proactive_harness: {
      ...
      gates: [
        { gate: "metadata_only_schema", pass: reason !== "case_must_not_include_raw_content" },
        { gate: "closed_axis_allowlists", pass: !reason.endsWith("_not_allowlisted") },
        { gate: "d3_d4_eval_only_guard", pass: reason !== "d3_d4_must_be_evaluation_only_and_quarantined_or_rejected" },
        { gate: "local_only_boundary", pass: true }
      ]
    },
    self_critique: {
      confidence: "rejected",
      limitation: "Malformed benchmark schema inputs are rejected before an evaluation contract can be trusted.",
      weakest_link: reason
    },
    micro_compliance: {
      preview_only: true,
    ...
    analogical_model: {
      ...
      mapping: "This preview defines how future sanitized cases could be scored; it does not administer the exam or reveal the answers."
    },
    boundary: boundary(),
...
export function buildCorpusBenchmarkSchemaPreview({ cases = DEFAULT_CASES } = {}) {
  ...
  const d3D4Cases = benchmarkCases.filter((entry) => ["D3", "D4"].includes(entry.tier));
  const distinctSapeProbes = new Set(benchmarkCases.flatMap((entry) => entry.sape_probes));
  const summary = {
    total_cases: benchmarkCases.length,
    eval_only_count: benchmarkCases.filter((entry) => entry.sample_role === "evaluation_only").length,
    skill_pattern_candidate_count: benchmarkCases.filter((entry) => entry.sample_role === "skill_pattern_candidate").length,
    preference_candidate_count: benchmarkCases.filter((entry) => entry.sample_role === "preference_candidate").length,
    d3_d4_total_count: d3D4Cases.length,
    d3_d4_eval_only_count: d3D4Cases.filter((entry) => entry.sample_role === "evaluation_only").length,
    proof_of_truth_axes_required: clone(PROOF_OF_TRUTH_AXES),
  ...
  return deepFreeze({
    ...
    self_proactive_harness: {
      ...
      gates: [
        ...
        { gate: "d3_d4_eval_only_guard", pass: summary.d3_d4_eval_only_count === summary.d3_d4_total_count },
        { gate: "proof_of_truth_axes_declared", pass: summary.proof_of_truth_axes_required.length === 4 },
        { gate: "local_only_boundary", pass: true }
      ]
    },
    self_critique: {
      confidence: "bounded_preview",
      limitation: "This schema preview defines benchmark metadata contracts only; it does not run Dema, inspect chats, create datasets, or prove answer quality.",
