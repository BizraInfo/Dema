export const CORPUS_MANUAL_REVIEW_QUEUE_PREVIEW_SCHEMA =
  "bizra.dema.corpus_manual_review_queue_preview.v0.1";
...
const REDACTION_STATES = Object.freeze([
...
const ALLOWED_USES = Object.freeze([
  ...
  "sanitized_benchmark_candidate_prioritization",
  "local_asset_inventory_planning",
  "truth_label_review_design",
  "skill_pattern_review_design",
  "preference_pair_review_design",
]);
...
function laneFor(candidate) {
  ...
  if (candidate.sample_role === "skill_pattern_candidate")
    return "skill_pattern_review";
  if (candidate.sample_role === "preference_candidate")
    return "preference_pair_review";
...
function validateTierLane(candidate) {
  if (candidate.tier === "D3") {
    return (
      candidate.sample_role === "evaluation_only" &&
...
function validateCandidates(candidates) {
  if (!Array.isArray(candidates) || candidates.length === 0)
    return { ok: false, reason: "candidates_must_be_non_empty_array" };
  const seen = new Set();
  for (const candidate of candidates) {
    if (!candidate || typeof candidate !== "object" || Array.isArray(candidate))
      return { ok: false, reason: "candidate_must_be_object" };
    if (!isPlainJsonMetadata(candidate))
      return { ok: false, reason: "candidate_must_be_plain_json_metadata" };
    if (hasRawContentKey(candidate))
      return { ok: false, reason: "candidate_must_not_include_raw_content" };
    if (
      ...
      return {
        ok: false,
        reason: "candidate_id_must_be_unique_safe_identifier",
      };
    }
    seen.add(candidate.candidate_id);
    if (!SOURCE_IDS.includes(candidate.source_id))
    ...
    if (!DIFFICULTIES.includes(candidate.difficulty))
      return { ok: false, reason: "difficulty_not_allowlisted" };
    if (!TIERS.includes(candidate.tier))
      return { ok: false, reason: "tier_not_allowlisted" };
    if (!CASE_TRUTH_LABELS.includes(candidate.case_truth_label))
      return { ok: false, reason: "case_truth_label_not_allowlisted" };
    if (!SAMPLE_ROLES.includes(candidate.sample_role))
      return { ok: false, reason: "sample_role_not_allowlisted" };
    if (!REDACTION_STATES.includes(candidate.redaction_state))
      return { ok: false, reason: "redaction_state_not_allowlisted" };
    if (!allAllowed(candidate.proof_of_truth_axes, PROOF_OF_TRUTH_AXES))
      return { ok: false, reason: "proof_of_truth_axis_not_allowlisted" };
    if (!SNR_PROFILES.includes(candidate.snr_profile))
      return { ok: false, reason: "snr_profile_not_allowlisted" };
    if (!validateTierLane(candidate))
      return { ok: false, reason: "tier_lane_policy_mismatch" };
  }
  return { ok: true };
}
...
function priorityScore(candidate) {
  ...
  const truth =
    {
      verified_good: 30,
      negative_overclaim: 24,
      right_for_wrong_reasons: 20,
      use_for_eval_only: 16,
    }[candidate.case_truth_label] ?? 8;
  const difficulty = { rare_path: 18, hard: 14, medium: 8, easy: 4 }[
    candidate.difficulty
  ];
  const snr = {
    benchmark_gold_candidate: 24,
    high_signal_low_noise: 20,
    overclaim_probe: 18,
    hidden_pattern_probe: 16,
  }[candidate.snr_profile];
...
function rejectPreview(reason) {
  return deepFreeze({
    ...
    self_proactive_harness: {
      ...
      gates: [
        {
          gate: "local_asset_metadata_only",
          pass: reason !== "candidate_must_not_include_raw_content",
        },
        {
          gate: "closed_allowlists",
          pass: !reason.endsWith("_not_allowlisted"),
        },
        {
          gate: "d3_d4_non_actionable_lanes",
          pass: reason !== "tier_lane_policy_mismatch",
        },
    ...
    self_critique: {
      confidence: "rejected",
      limitation:
        "Malformed review queue inputs are rejected before prioritization can be trusted.",
      weakest_link: reason,
    },
    micro_compliance: {
      preview_only: true,
    ...
    analogical_model: {
      ...
      mapping:
        "This preview orders local card-catalog entries for human review; it does not open books, copy pages, or train on them.",
    },
    boundary: boundary(),
...
export function buildCorpusManualReviewQueuePreview({
  ...
  return deepFreeze({
    ...
    summary: {
      ...
      actionable_review_count: queueItems.filter(
        (item) =>
          !["quarantine_review", "reject_log"].includes(item.queue_lane),
      ).length,
    ...
    self_proactive_harness: {
      ...
      gates: [
        ...
        {
          gate: "d3_d4_non_actionable_lanes",
          pass: lanes.quarantine_review > 0 && lanes.reject_log > 0,
        },
        { gate: "local_only_boundary", pass: true },
      ],
    },
    self_critique: {
      ...
      limitation:
        "Priorities are deterministic metadata heuristics; human gold labels and future proof receipts are still required.",
