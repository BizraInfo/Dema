...
import { readdir, readFile, stat, lstat, realpath, readlink } from "node:fs/promises";
import { join, resolve, isAbsolute, basename, extname, relative, dirname, sep } from "node:path";
import { homedir } from "node:os";
import { performance } from "node:perf_hooks";

export const CODEBASE_ARCHITECTURE_MAP_SCHEMA = "bizra.dema.codebase_architecture_map.v0.1";
...
export const DEFAULT_EXCLUSIONS = Object.freeze([
  ...
  "target", ".gradle",
  "__pycache__", ".venv", "venv", "env", ".pytest_cache", ".mypy_cache", ".ruff_cache", ".tox",
  "vendor", "tmp", "logs", ".cache",
  ".artifacts", ".proof-forge", ".qodo", ".claude"
]);
...
const BINARY_EXTS = new Set([
  ".png", ".jpg", ".jpeg", ".gif", ".pdf", ".zip", ".tar", ".gz", ".bz2", ".xz", ".7z",
...
const MANIFEST_BASENAMES = new Set([
  ...
  "tsconfig.json", "go.mod", "go.sum", "Gemfile", "Gemfile.lock"
]);
...
function isForbiddenSystemPath(p) {
  return /^\/proc(\/|$)/.test(p) || /^\/sys(\/|$)/.test(p) || /^\/dev(\/|$)/.test(p);
}

function isForbiddenOperatorPath(p, home) {
  const safe = home || homedir();
  return p === join(safe, ".ssh") || p.startsWith(join(safe, ".ssh") + sep) ||
         p === join(safe, ".dema") || p.startsWith(join(safe, ".dema") + sep);
}
...
function classifyFileRole(relPath, name, ext, isSymlink) {
  ...
  if (/(^|\/)\.github\/workflows\//.test(relPath) && (ext === ".yml" || ext === ".yaml")) return "workflow";
  if (name === ".gitlab-ci.yml" || /(^|\/)\.circleci\/config\.yml$/.test(relPath)) return "workflow";
  // test
  if (/(^|\/)tests?\//.test(relPath) || /(^|\/)spec\//.test(relPath) ||
      /\.(test|spec)\.(js|mjs|cjs|ts|tsx|jsx|py|rs|go)$/.test(name)) return "test";
  // docs
  ...
  // script
  if (/(^|\/)scripts?\//.test(relPath) || /(^|\/)bin\//.test(relPath) || ext === ".sh") return "script";
  // binary
...
function isTextRole(role) {
  return role === "source" || role === "test" || role === "docs" ||
         role === "config" || role === "script" || role === "workflow";
}
...
const RE_JS_IMPORT_FROM = /^\s{0,8}import\s+[^;'"]{0,200}\s+from\s+['"]([^'"]{1,500})['"]/;
...
function extractJsEdges(content, fromPath, lineCap) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) { edges.push({ from: fromPath, to_raw: m[1], kind: "import" }); continue; }
...
function extractPyEdges(content, fromPath, lineCap) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) {
      const mods = m[1].split(",").map((s) => s.trim().split(/\s+/)[0]).filter(Boolean);
      for (const mod of mods) edges.push({ from: fromPath, to_raw: mod, kind: "import" });
    }
  }
  return edges;
}
...
function extractGoEdges(content, fromPath, lineCap) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (line.length > lineCap) continue;
    if (/^\s*import\s+\(/.test(line)) { inGroup = true; continue; }
    if (inGroup && /^\s*\)/.test(line)) { inGroup = false; continue; }
    if (inGroup) {
...
function extractWorkflowEdges(content, fromPath, lineCap) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m) edges.push({ from: fromPath, to_raw: m[1], kind: "workflow_action" });
  }
  return edges;
}
...
function extractManifestEdges(content, fromPath, name, lineCap) {
  ...
  if (name === "package.json") {
    let parsed;
    try { parsed = JSON.parse(content); } catch { return edges; }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^\[(?:dependencies|dev-dependencies|build-dependencies)\]$/.test(trimmed) ||
          /^\[tool\.poetry\.(?:dependencies|dev-dependencies|group\.[a-z0-9_-]{1,40}\.dependencies)\]$/.test(trimmed)) {
        inDeps = true;
        continue;
      }
      if (/^\[/.test(trimmed) && trimmed !== "") { inDeps = false; continue; }
      if (inDeps) {
        const m = trimmed.match(RE_TOML_DEP_LINE);
        if (m && m[1] !== "version") edges.push({ from: fromPath, to_raw: m[1], kind: "manifest" });
      }
    }
...
function extractEdgesForFile(content, relPath, name, ext, role, lineCap) {
  ...
  if (role === "workflow") return extractWorkflowEdges(content, relPath, lineCap);
...
function tryResolveLocalEdge(edge, fileSetByRelPath, repoRoot) {
  ...
  const candExts = ["", ".js", ".mjs", ".cjs", ".ts", ".tsx", ".jsx", ".py", ".rs", ".go",
                    "/index.js", "/index.mjs", "/index.ts", "/index.tsx", "/mod.rs"];
...
async function walkRepo(rootRealpath, options) {
  const {
    maxFiles, maxDepth, exclusionSet, includeTests,
    nowMs, walkerBudgetMs
  } = options;
  ...
  try {
    ...
    return { files, symlinks, warnings, partial: true, errorReason: "root_stat_failed", errorMessage: err?.message ?? String(err) };
  }
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      warnings.push(`readdir_failed:${relative(rootRealpath, absPath) || "."}:${err?.code ?? "UNKNOWN"}`);
      continue;
    }
    ...
    for (const ent of entries) {
      ...
      if (ent.isSymbolicLink()) {
        let target = null;
        try { target = await readlink(childAbs); } catch { /* swallow */ }
      ...
      if (ent.isDirectory()) {
        ...
        let dstat;
        try { dstat = await stat(childAbs); } catch { continue; }
      ...
      let sstat;
      try { sstat = await stat(childAbs); } catch { continue; }
...
function analyzeHotspots(fileRecords, edgesOut, edgesIn, contentLineCounts, dirFileCounts, fileContents) {
  const hotspotsByPath = new Map();
  function add(path, reason, evidence) {
    if (!hotspotsByPath.has(path)) hotspotsByPath.set(path, { path, reasons: [], evidence: {} });
  ...
  for (const f of fileRecords) {
    ...
    if (f.size_bytes > 100_000) add(f.path, "file_exceeds_100KB", { size_bytes: f.size_bytes });
    const fanIn = edgesIn.get(f.path) ?? 0;
    if (fanIn > 20) add(f.path, "high_fan_in_central_dependency", { fan_in: fanIn });
    const fanOut = edgesOut.get(f.path) ?? 0;
    if (fanOut > 30) add(f.path, "high_fan_out_god_object_risk", { fan_out: fanOut });

    const content = fileContents.get(f.path);
    if (typeof content === "string" && content.length > 0) {
      const markers = (content.match(/\b(?:TODO|FIXME|XXX|HACK)\b/g) ?? []).length;
      if (markers > 5) add(f.path, "marker_concentration", { markers });
      if (loc > 0) {
        const noise = (content.match(/console\.log\(|\bprint\(|eprintln!\(/g) ?? []).length;
        if (loc >= 100 && noise / loc > 0.1) add(f.path, "noisy_logging_density", { logs: noise, LOC: loc });
      }
    }
    if (f.role === "workflow" && typeof content === "string") {
      ...
      if (unpinned > 0) add(f.path, "unpinned_action_supply_chain_risk", { unpinned_action_count: unpinned });
    }
  }
  ...
  for (const [dir, count] of dirFileCounts) {
    if (count > 50) {
      const key = `dir:${dir}`;
      if (!hotspotsByPath.has(key)) hotspotsByPath.set(key, { path: dir, reasons: [], evidence: {} });
...
export async function buildCodebaseArchitectureMap(repoPath, options = {}) {
  ...
  if (!isAbsolute(repoPath)) {
    return shapedFailure({ reason: "path_must_be_absolute", scannedAt, repo_path: repoPath });
  }
  if (isForbiddenSystemPath(repoPath)) {
    return shapedFailure({ reason: "system_path_forbidden", scannedAt, repo_path: repoPath });
  }
  if (isForbiddenOperatorPath(repoPath, options.home)) {
    return shapedFailure({ reason: "operator_protected_path_forbidden", scannedAt, repo_path: repoPath });
  }
  ...
  try {
    ...
    return shapedFailure({
      ...
      scannedAt, repo_path: repoPath, error_message: err?.message ?? String(err)
    });
  }
  ...
  const maxTotalBytes = positiveInt(options.maxTotalBytesRead, DEFAULT_MAX_TOTAL_BYTES_READ);
  const walkerBudgetMs = positiveInt(options.walkerBudgetMs, DEFAULT_WALKER_BUDGET_MS);
  ...
  const walkResult = await walkRepo(realRepoPath, {
    maxFiles, maxDepth, exclusionSet, includeTests,
    nowMs: startedNowMs, walkerBudgetMs
  });

  if (walkResult.errorReason === "root_stat_failed") {
    return shapedFailure({
      reason: "root_stat_failed", scannedAt, repo_path: realRepoPath,
  ...
  walkResult.symlinks.sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0));
  ...
  for (const f of walkResult.files) {
    if (f.role === "secret_metadata_only" || f.role === "binary" ||
        f.role === "symlink") {
      // Never read these
      continue;
    }
    ...
    try {
      ...
      walkResult.warnings.push(`readfile_failed:${f.path}:${err?.code ?? "UNKNOWN"}`);
      continue;
    }
    ...
    const edges = extractEdgesForFile(content, f.path, f.name, f.ext, f.role, REGEX_LINE_MAX_CHARS);
  ...
  const packages = [...packageDirs.entries()].map(([dir, manifests]) => ({
    path: dir || ".",
  ...
  for (const pkg of packages) {
    ...
    for (const f of walkResult.files) {
      const inside = pkg.path === "." ? true : (f.path === pkg.path || f.path.startsWith(pkg.path + "/"));
      if (!inside) continue;
  ...
  if (hotspotsEnabled) {
    hotspots = analyzeHotspots(
      walkResult.files, fanOutByPath, fanInByPath, lineCountByPath, dirFileCounts, contentByPath
    );
  }
...
function shapedFailure({ reason, scannedAt, repo_path, error_message }) {
  return Object.freeze({
    ...
    scan_config: null,
    totals: Object.freeze({
      file_count: 0, symlink_count: 0, total_bytes: 0, total_bytes_read: 0,
      by_extension: Object.freeze({}), by_role: Object.freeze({})
    }),
...
export function formatCodebaseMapSummary(envelope) {
  ...
  const lines = [
    ...
    `By role: ${Object.entries(t.by_role).map(([k, v]) => `${k}=${v}`).join(" · ") || "(none)"}`,
