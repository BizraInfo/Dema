...
const FORBIDDEN_TECHNIQUES = [
  {
    ...
    pattern: /\b(covert|subliminal|without (?:them|user) knowing)\b/i,
  },
  { code: "manipulation", pattern: /\bmanipulat(?:e|ion|ive)\b/i },
  { code: "dark_pattern", pattern: /\bdark pattern\b/i },
  {
    code: "addiction_loop",
    pattern: /\b(addict(?:ion|ive)?|hook(?:ed)?|compulsion)\b/i,
  },
  {
    code: "emotional_exploitation",
    pattern: /\bexploit (?:fear|guilt|shame|loneliness)\b/i,
  },
  {
    code: "financial_pressure",
    pattern: /\bpressure .* (?:buy|pay|invest|token|reward)\b/i,
  },
];
...
function classifySurface(intent) {
  ...
  if (/\bsafety|boundary|block|gate|halt\b/.test(text))
    return "safety_boundary";
  if (/\binterface|ui|display|show|guide\b/.test(text))
    return "interface_guidance";
  return "recommendation_style";
}

function forbiddenFindings(intent) {
  return FORBIDDEN_TECHNIQUES.filter(({ pattern }) => pattern.test(intent)).map(
    ({ code }) => code,
  );
}
...
export function buildBehavioralModulationPreview({
  ...
  const checks = [
    ...
    check(
      ...
      forbidden.length === 0,
      forbidden.length === 0
        ? "no forbidden behavior-shaping pattern detected"
        : forbidden.join(","),
    ),
    check(
  ...
  const verdict = checks.every((item) => item.pass)
    ? "PARTIAL_PLACEHOLDER"
    : "PREVIEW_REJECT";
...
export function formatBehavioralModulationPreview(preview) {
  ...
  for (const item of preview.constitutional_gate.checks) {
    lines.push(
      `  - ${item.pass ? "PASS" : "REJECT"} ${item.check}: ${item.detail}`,
    );
  }
