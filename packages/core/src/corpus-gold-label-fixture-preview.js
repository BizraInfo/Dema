export const CORPUS_GOLD_LABEL_FIXTURE_PREVIEW_SCHEMA = "bizra.dema.corpus_gold_label_fixture_preview.v0.1";
...
const LABEL_KINDS = Object.freeze(["eval_reference_fixture", "skill_pattern_fixture", "preference_pair_fixture", "negative_eval_fixture", "quarantine_no_label", "reject_no_label"]);
const REVIEW_OUTCOMES = Object.freeze(["candidate_ready_for_human_scoring", "candidate_needs_human_adjudication", "quarantine_without_label", "reject_without_label"]);
const RUBRIC_AXES = Object.freeze(["correctness", "truth_discipline", "safety", "evidence_alignment", "operator_utility"]);
...
const ALLOWED_USES = Object.freeze([
  "gold_label_fixture_design", "human_scoring_rubric_design", "truth_label_review_design",
  "sanitized_eval_reference_planning", "skill_pattern_label_planning", "preference_pair_label_planning"
]);
...
const BLOCKED_USES = Object.freeze([
  ...
  "embedding_creation", "runtime_memory_mutation", "label_persistence", "dataset_write",
  "skill_pattern_promotion", "preference_dataset_promotion", "sharing_with_node1_node4",
...
const DEFAULT_FIXTURES = Object.freeze([
  Object.freeze({
    ...
    rubric_axes: Object.freeze(["correctness", "truth_discipline", "evidence_alignment", "operator_utility"]),
  ...
  Object.freeze({
    ...
    rubric_axes: Object.freeze(["truth_discipline", "safety", "operator_utility"]),
    proof_of_truth_axes: Object.freeze(["formal", "empirical"]),
  ...
  Object.freeze({
    ...
    review_outcome: "candidate_ready_for_human_scoring",
    rubric_axes: Object.freeze(["truth_discipline", "safety", "evidence_alignment"]),
...
function validateFixtures(fixtures) {
  ...
  for (const fixture of fixtures) {
    ...
    if (!isPlainJsonMetadata(fixture)) return { ok: false, reason: "fixture_must_be_plain_json_metadata" };
    if (hasRawContentKey(fixture)) return { ok: false, reason: "fixture_must_not_include_raw_content" };
    ...
    if (!isSafeIdentifier(fixture.candidate_id)) return { ok: false, reason: "candidate_id_must_be_safe_identifier" };
    if (!SOURCE_IDS.includes(fixture.source_id)) return { ok: false, reason: "source_id_not_allowlisted" };
    if (!DOMAINS.includes(fixture.domain)) return { ok: false, reason: "domain_not_allowlisted" };
    if (!TIERS.includes(fixture.tier)) return { ok: false, reason: "tier_not_allowlisted" };
    if (!QUEUE_LANES.includes(fixture.queue_lane)) return { ok: false, reason: "queue_lane_not_allowlisted" };
    if (!CASE_TRUTH_LABELS.includes(fixture.case_truth_label)) return { ok: false, reason: "case_truth_label_not_allowlisted" };
    if (!LABEL_KINDS.includes(fixture.label_kind)) return { ok: false, reason: "label_kind_not_allowlisted" };
    if (!REVIEW_OUTCOMES.includes(fixture.review_outcome)) return { ok: false, reason: "review_outcome_not_allowlisted" };
    if (!allAllowed(fixture.rubric_axes, RUBRIC_AXES)) return { ok: false, reason: "rubric_axis_not_allowlisted" };
    if (!allAllowed(fixture.proof_of_truth_axes, PROOF_OF_TRUTH_AXES)) return { ok: false, reason: "proof_of_truth_axis_not_allowlisted" };
    if (!validateTierLabelPolicy(fixture)) return { ok: false, reason: "tier_label_policy_mismatch" };
  }
  return { ok: true };
}
...
function rejectPreview(reason) {
  return deepFreeze({
    ...
    self_proactive_harness: {
      ...
      gates: [
        { gate: "metadata_only_label_slots", pass: reason !== "fixture_must_not_include_raw_content" },
        { gate: "closed_rubric_allowlists", pass: !reason.endsWith("_not_allowlisted") },
        { gate: "d3_d4_no_label_guard", pass: reason !== "tier_label_policy_mismatch" },
        { gate: "local_only_boundary", pass: true }
      ]
    },
    self_critique: {
      confidence: "rejected",
      limitation: "Malformed label fixtures are rejected before gold-label slots can be trusted.",
      weakest_link: reason
    },
    micro_compliance: {
      preview_only: true,
    ...
    analogical_model: {
      ...
      mapping: "This preview defines where future human gold labels could go; it does not reveal answers or write the key."
    },
    boundary: boundary(),
...
export function buildCorpusGoldLabelFixturePreview({ fixtures = DEFAULT_FIXTURES } = {}) {
  ...
  const noLabelCount = labelSlots.filter((slot) => ["quarantine_no_label", "reject_no_label"].includes(slot.label_kind)).length;
  ...
  return deepFreeze({
    ...
    summary: {
      ...
      ready_for_human_scoring_count: labelSlots.filter((slot) => slot.review_outcome === "candidate_ready_for_human_scoring").length,
      adjudication_required_count: labelSlots.filter((slot) => slot.review_outcome === "candidate_needs_human_adjudication").length
    },
    self_proactive_harness: {
      mode: "DETERMINISTIC_GOLD_LABEL_FIXTURE_PREVIEW",
    ...
    self_critique: {
      ...
      limitation: "This preview defines label slots and rubrics only; no human judgment, answer key, or dataset artifact is produced.",
      weakest_link: "future_gold_labels_require_explicit_human_review_and_consent"
    },
    micro_compliance: {
      preview_only: true,
      deterministic: true,
