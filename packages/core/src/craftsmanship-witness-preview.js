...
export const MASTER_CRAFTSMANSHIP_INVARIANTS = Object.freeze([
  Object.freeze({
    ...
    evidence_anchor: "this file · schema/truth_label/MODE constants + buildPreviewBoundary()",
  }),
  Object.freeze({
    id: "test_backed",
    text: "≥80% coverage · ≥15 adversarial scenarios per component",
    evidence_anchor: "tests/craftsmanship-witness-preview.test.js · 16 base + 16 ADV + 3 exports",
  }),
  Object.freeze({
    id: "consent_gated",
    text: "exact-string consent per ADR-005 · no fuzzy · no case-insensitive · no paste",
    evidence_anchor: "every next_slice_observable carries consent_phrase_required_to_act",
  }),
  Object.freeze({
    id: "receipt_emitting",
    text: "receipt_shape_ready flag on every valid emission",
    evidence_anchor: "receipt_shape_ready: true on every build (structurally pinned)",
  }),
  Object.freeze({
    id: "doctrine_coherent",
    text: "Key Maker §3 V/D/A/U claim labeling on every claim",
    evidence_anchor: "rsi_signals[*].claim_state ∈ {V,D,A,U} · enforced by builder",
  }),
  Object.freeze({
    id: "boundary_disciplined",
    text: "declared blocked_effects (no implicit denials)",
    evidence_anchor: "CRAFTSMANSHIP_WITNESS_REQUIRED_BLOCKED_EFFECTS exported · enforced in output",
  }),
  Object.freeze({
    id: "adversarial_tested",
    text: "red-team probes (prototype pollution · fuzzy match · forgery · forbidden injection)",
    evidence_anchor: "tests/craftsmanship-witness-preview.test.js ADV-01..ADV-16",
  }),
  Object.freeze({
    id: "verify_before_asserting",
    text: "refusal verdicts explicitly named with reasons",
    evidence_anchor: "CRAFTSMANSHIP_WITNESS_PRIMARY_REFUSALS · 8 named with refuse_to_<action> pattern",
  }),
  Object.freeze({
    id: "reversible",
    text: "pure function · preview-only · no I/O in builder",
    evidence_anchor: "this builder · zero fs/network/clock/env reads at call-time",
  }),
  Object.freeze({
    id: "cross_referenced",
    text: "links to ADR + Key Maker canon + relevant memory anchors",
    evidence_anchor: "canon_anchors block · cites LoA + ADR-005 + ADR-008 + MC source + 4 structural laws",
  }),
]);
...
function freezeRsiSignal(s) {
  return Object.freeze({
    ...
    claim_state: VALID_CLAIM_STATES.includes(s?.claim_state) ? s.claim_state : "U",
...
function freezeDriftMarker(m) {
  return Object.freeze({
    ...
    severity: ["info", "low", "medium", "high"].includes(m?.severity) ? m.severity : "info",
...
export function buildCraftsmanshipWitnessPreview({
  ...
  const nextSignals = Array.isArray(next_slice_signals) ? next_slice_signals : [];

  const rsiSignalsOut = Object.freeze(rsiInputs.map(freezeRsiSignal));

  const dh = doctrine_health_inputs && typeof doctrine_health_inputs === "object"
    ? doctrine_health_inputs
    : {};
  const refusalEvents = Array.isArray(dh.refusal_events) ? dh.refusal_events : [];
  const doctrineCatches = Array.isArray(dh.doctrine_catches) ? dh.doctrine_catches : [];
  ...
  const sh = slice_history && typeof slice_history === "object" ? slice_history : {};
  const processMiningOfSelfOut = Object.freeze({
    commits_in_session: typeof sh.commits_in_session === "number" ? sh.commits_in_session : null,
    tests_total: typeof sh.tests_total === "number" ? sh.tests_total : null,
    tests_delta: typeof sh.tests_delta === "number" ? sh.tests_delta : null,
    first_run_green_streak: typeof sh.first_run_green_streak === "number" ? sh.first_run_green_streak : null,
    refusal_as_product_N: typeof sh.refusal_as_product_N === "number" ? sh.refusal_as_product_N : null,
    receipt_chain_length: typeof sh.receipt_chain_length === "number" ? sh.receipt_chain_length : null,
    spine_surfaces: typeof sh.spine_surfaces === "number" ? sh.spine_surfaces : null,
    mining_mode: "self_observing_self_via_declared_inputs_only · no_filesystem_walk",
  });
  ...
  const allCompliant = invariantsOut.every((inv) => inv.self_assertion === true);
  ...
  const countersOut = Object.freeze({
    ...
    blocked_effects_total: CRAFTSMANSHIP_WITNESS_REQUIRED_BLOCKED_EFFECTS.length,
  ...
  const canonAnchorsOut = Object.freeze({
    ...
    master_craftsmanship_source: "HANDOVER.md §4 · ADR-008 · feedback_preflight_adversarial_slice_pattern (operator memory)",
