...
export const CORPUS_PREVIEW_INDEX_SCHEMA = "bizra.dema.corpus_preview_index.v0.1";
...
const BLOCKED_USES = Object.freeze([
  ...
  "score_computation", "hash_computation", "receipt_minting", "receipt_persistence", "local_model_invocation",
  "external_upload", "supervised_fine_tuning", "dpo_or_rlhf_training", "embedding_creation",
  "runtime_memory_mutation", "dataset_write", "skill_pattern_promotion", "preference_dataset_promotion",
  "sharing_with_node1_node4", "runtime_start", "federation_start", "step7_minting"
]);
...
function validateSurfaceIds(surfaceIds) {
  ...
  for (const surfaceId of surfaceIds) {
    if (!surfaceById(surfaceId)) return { ok: false, reason: "surface_id_not_allowlisted" };
    if (seen.has(surfaceId)) return { ok: false, reason: "surface_id_must_be_unique" };
    seen.add(surfaceId);
  }
  return { ok: true };
}
...
function rejectPreview(reason) {
  return deepFreeze({
    ...
    summary: { total_surfaces: 0, ready_surface_count: 0, rejected_surface_count: 0 },
    self_proactive_harness: {
      mode: "DETERMINISTIC_CORPUS_INDEX_PREVIEW",
      recommended_micro_action: "fix_malformed_corpus_index_inputs",
      gates: [
        { gate: "metadata_only_index", pass: reason !== "index_must_not_include_raw_content_or_scores" },
        { gate: "surface_ids_allowlisted", pass: reason !== "surface_id_not_allowlisted" },
    ...
    self_critique: {
      confidence: "rejected",
      limitation: "Malformed index inputs are rejected before corpus preview integration can be trusted.",
      weakest_link: reason
    },
    micro_compliance: {
      preview_only: true,
    ...
    analogical_model: {
      ...
      mapping: "This preview lists safe corpus preview shelves; it does not open books, run evaluations, or issue receipts."
    },
    boundary: boundary(),
...
function buildSurfaceEntry(surface) {
  ...
  return {
    ...
    boundary_ok: Object.values(preview.boundary ?? {}).every((value) => value === false),
    preview_only: preview.mode === "PREVIEW_ONLY" && preview.micro_compliance?.preview_only === true,
    authority_state: "no_authority_expansion"
  };
}

export function buildCorpusPreviewIndex({ surfaceIds = SURFACES.map((surface) => surface.surface_id), metadata = {} } = {}) {
  if (hasRawContentKey(metadata)) return rejectPreview("index_must_not_include_raw_content_or_scores");
  const validation = validateSurfaceIds(surfaceIds);
  if (!validation.ok) return rejectPreview(validation.reason);

  const surfaces = surfaceIds.map((surfaceId) => buildSurfaceEntry(surfaceById(surfaceId)));
  const rejectedSurfaceCount = surfaces.filter((surface) => surface.verdict === "PREVIEW_REJECT").length;
  ...
  return deepFreeze({
    ...
    self_proactive_harness: {
      ...
      gates: [
        ...
        { gate: "all_children_preview_only", pass: surfaces.every((surface) => surface.preview_only) },
        { gate: "all_child_boundaries_closed", pass: surfaces.every((surface) => surface.boundary_ok) },
        { gate: "local_only_boundary", pass: true }
      ]
    },
    self_critique: {
      confidence: "bounded_preview",
      limitation: "This index integrates preview metadata only; it does not expose a CLI command or authorize corpus processing.",
