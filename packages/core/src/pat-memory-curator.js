...
const PAT4_CONSENT_PHRASE_TEMPLATE = "GO: invoke PAT-4 memory_curator to classify";

const CANONICAL_MEMORY_CATEGORIES = Object.freeze([
  "user", "feedback", "project", "reference", "uncategorized"
]);
...
export function buildPATMemoryCuratorKernel({ mission_intent = "", max_iterations = AGENT_KERNEL_MAX_ITERATIONS } = {}) {
...
export function classifyMemoryEntry({
  ...
  const fromFrontmatter = CANONICAL_MEMORY_CATEGORIES.includes(typeHint) ? typeHint : null;
  ...
  let confidence;
  if (fromFrontmatter && fromFrontmatter !== "uncategorized") confidence = "high";
...
export const PAT_MEMORY_CURATOR_CLASSIFICATION_SCHEMA_NAME = CLASSIFICATION_SCHEMA;
export const PAT_MEMORY_CURATOR_PERSONA = PAT4_PERSONA;
export const PAT_MEMORY_CURATOR_CANONICAL_CATEGORIES = CANONICAL_MEMORY_CATEGORIES;
