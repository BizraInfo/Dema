...
const AUTHORIZATION_INTENT_PATTERN =
  /\b(authori[sz]e|authori[sz]ation|permission|permit|approve|approval|consent|proceed)\b/i;
...
export function buildSelfCritique({ malformed }) {
  return deepFreeze({
    ...
    limitation:
      "This preview refuses or classifies supplied consent language only; it is not the governed Step 7 ceremony gate.",
    weakest_link: malformed
      ? "input_shape"
...
export function buildAnalogicalModel() {
  return deepFreeze({
    ...
    mapping:
      "The preview can label a door as locked and explain why broad permission is not the key; it cannot unlock the door.",
  });
}
...
export function buildStep7ConsentRefusalPreview({
  observedText,
  now = new Date(),
} = {}) {
  ...
  const refusalReason = checkedAt.malformed
    ? "malformed_now"
    : observed.refusalReason;
  const nextSafeAction = malformed
    ? "fix_malformed_process_inputs"
    : "hold_step7_ceremony";
  ...
  return deepFreeze({
    ...
    observed_text_class: checkedAt.malformed
      ? "malformed"
      : observed.observedTextClass,
    ...
    checks: [
      ...
      {
        check: "authorization_phrase_not_emitted",
        pass: boundary.authorization_phrase_emitted === false,
      },
