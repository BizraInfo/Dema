...
const RESOURCE_CATEGORIES = Object.freeze([
  "hardware", "data_corpus", "knowledge_base", "experience_history", "skill_library"
]);
...
export function buildURPLocalPreview({
  ...
  const hw = hardware_summary && typeof hardware_summary === "object" ? hardware_summary : null;
  const dc = data_corpus_summary && typeof data_corpus_summary === "object" ? data_corpus_summary : null;
  const kb = knowledge_base_summary && typeof knowledge_base_summary === "object" ? knowledge_base_summary : null;
  const eh = experience_history_summary && typeof experience_history_summary === "object" ? experience_history_summary : null;
  const sl = skill_library_summary && typeof skill_library_summary === "object" ? skill_library_summary : null;
  ...
  return Object.freeze({
    ...
    knowledge_base: kb ? Object.freeze({
    ...
    experience_history: eh ? Object.freeze({
      receipts_count: safeNumber(eh.receipts_count, 0),
      sessions_count: safeNumber(eh.sessions_count, 0),
      cryptographic_anchors_count: safeNumber(eh.cryptographic_anchors_count, 0),
      data_present: true
    }) : Object.freeze({ data_present: false }),
    skill_library: sl ? Object.freeze({
      skills_count: safeNumber(sl.skills_count, 0),
      categories: Object.freeze(safeArray(sl.categories).filter((c) => typeof c === "string")),
      data_present: true
    }) : Object.freeze({ data_present: false }),
    blocked_effects: REQUIRED_BLOCKED_EFFECTS,
    sharing_consent_phrase_template: "GO: share <resource_id> from Node0 to <target_node>",
...
export function buildResourceAllocationCandidate({
  ...
  if (category === "unknown") violations.push(`unknown_resource_category · expected one of ${RESOURCE_CATEGORIES.join(",")}`);
...
export function buildURPLocalSummary(inputs = {}) {
  ...
  const presentCategories = RESOURCE_CATEGORIES.filter((c) => preview[c]?.data_present === true);
  return Object.freeze({
    ...
    categories_without_data_count: RESOURCE_CATEGORIES.length - presentCategories.length,
