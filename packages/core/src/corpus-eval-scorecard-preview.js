export const CORPUS_EVAL_SCORECARD_PREVIEW_SCHEMA =
  "bizra.dema.corpus_eval_scorecard_preview.v0.1";
...
const READINESS_STATES = Object.freeze([
  "schema_only",
  "awaiting_human_gold_labels",
  "blocked_until_authorized_eval",
]);
const INPUT_STATES = Object.freeze([
  "metadata_only_fixture",
  "sanitized_label_slots",
  "quarantine_marker",
  "reject_marker",
]);
...
function validateMetric(metric) {
  if (!metric || typeof metric !== "object" || Array.isArray(metric))
    return "metric_must_be_object";
  if (!isPlainJsonMetadata(metric)) return "metric_must_be_plain_json_metadata";
  if (hasRawContentKey(metric))
    return "metric_must_not_include_raw_content_or_scores";
  if (!METRIC_IDS.includes(metric.metric_id))
    return "metric_id_not_allowlisted";
  if (!DOMAINS.includes(metric.domain)) return "domain_not_allowlisted";
  if (!INPUT_STATES.includes(metric.input_state))
    return "input_state_not_allowlisted";
  if (!READINESS_STATES.includes(metric.readiness_state))
    return "readiness_state_not_allowlisted";
  if (!allAllowed(metric.proof_of_truth_axes, PROOF_OF_TRUTH_AXES))
    return "proof_of_truth_axis_not_allowlisted";
  if (!["gte", "lte", "eq"].includes(metric.direction))
    return "direction_not_allowlisted";
  if (
    typeof metric.target !== "number" ||
    metric.target < 0 ||
    metric.target > 5
  )
    return "target_must_be_number_between_zero_and_five";
  if (typeof metric.d3_d4_included !== "boolean")
    return "d3_d4_included_must_be_boolean";
  if (metric.d3_d4_included && metric.metric_id !== "quarantine_integrity_rate")
    return "d3_d4_only_allowed_for_quarantine_integrity";
  if (
    metric.input_state === "reject_marker" &&
    metric.metric_id !== "quarantine_integrity_rate"
  )
    return "reject_marker_not_scoreable";
  return null;
}

function validateMetrics(metrics) {
  if (!Array.isArray(metrics) || metrics.length === 0)
    return { ok: false, reason: "metrics_must_be_non_empty_array" };
  const seen = new Set();
  for (const metric of metrics) {
    const reason = validateMetric(metric);
    if (reason) return { ok: false, reason };
    if (seen.has(metric.metric_id))
      return { ok: false, reason: "metric_id_must_be_unique" };
...
function rejectPreview(reason) {
  return deepFreeze({
    ...
    self_proactive_harness: {
      ...
      gates: [
        {
          gate: "metadata_only_metrics",
          pass: reason !== "metric_must_not_include_raw_content_or_scores",
        },
        {
          gate: "closed_metric_allowlists",
    ...
    self_critique: {
      confidence: "rejected",
      limitation:
        "Malformed scorecard metrics are rejected before aggregate evaluation posture can be trusted.",
      weakest_link: reason,
    },
    micro_compliance: {
      preview_only: true,
    ...
    analogical_model: {
      ...
      mapping:
        "This preview draws the scoreboard and threshold labels; it does not play games, record scores, or award points.",
    },
    boundary: boundary(),
...
export function buildCorpusEvalScorecardPreview({
  ...
  return deepFreeze({
    ...
    summary: {
      ...
      computed_metric_count: 0,
      awaiting_gold_label_count: metricSlots.filter(
        (slot) => slot.readiness_state === "awaiting_human_gold_labels",
      ).length,
      blocked_until_eval_count: metricSlots.filter(
        (slot) => slot.readiness_state === "blocked_until_authorized_eval",
      ).length,
      d3_d4_metric_count: metricSlots.filter((slot) => slot.d3_d4_included)
        .length,
    },
    self_proactive_harness: {
      mode: "DETERMINISTIC_EVAL_SCORECARD_PREVIEW",
      ...
      gates: [
        ...
        { gate: "no_score_computation", pass: true },
        {
          gate: "quarantine_integrity_is_only_d3_d4_metric",
          pass: metricSlots
            .filter((slot) => slot.d3_d4_included)
            .every((slot) => slot.metric_id === "quarantine_integrity_rate"),
        },
        { gate: "local_only_boundary", pass: true },
      ],
    },
    self_critique: {
      ...
      limitation:
        "This preview defines aggregate metric slots only; no benchmark run, human scoring, or empirical quality claim exists yet.",
