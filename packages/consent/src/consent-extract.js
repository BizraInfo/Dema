const FILE_PATTERN =
  /(?:^|[\s"'`])(~?[A-Za-z0-9_./-]+\.(?:py|js|ts|tsx|jsx|md|json|ya?ml|toml|rs|go|sh))/gi;
...
const SPEND_VERBS =
  /\b(spend|pay|buy|purchase|subscribe|charge|credits?|budget|tokens?)\b/i;
const GUI_HINTS =
  /\b(gui|desktop|screen|click|press|button|browser|mouse|keyboard|browser automation)\b/i;
const MOBILE_AGENT_HINTS =
  /\b(mobile agent|across hosts?|remote hosts?|move across|copy across|node1|node2|federat(?:e|ion))\b/i;
const COMMAND_HINTS =
  /\b(run|execute|launch)\b.*\b(script|command|verification|verify|test|checks?)\b/i;
...
const SERVICE_RULES = [
  ...
  {
    pattern:
      /\b(external brief|share|publish|upload|post|webhook|api access)\b/i,
...
function permission(resourceId, action, purpose, reason, confidence = 0.78) {
  return {
    ...
    reason,
    confidence,
    requires_human_consent: ["write", "execute", "call", "spend"].includes(
      action,
    ),
  };
}
...
function addPermission(permissions, next) {
  ...
  if (
    !permissions.some(
      (existing) => `${existing.resource_id}:${existing.action}` === key,
    )
  ) {
...
export function buildPermissions(
  intent,
  files = extractFileReferences(intent).safe,
) {
  const permissions = [];
...
export function buildPolicyPreview({
  ...
  if (unsafe_file_references.length > 0) {
    decisions.push({
      ...
      reason:
        "absolute, home-relative, or parent traversal paths are excluded from consent permissions",
    });
  }
  ...
  if (actuator_classes.includes("filesystem_mutation")) {
    decisions.push({
      ...
      reason:
        "file mutation needs an exact consent scope before any governed handoff",
    });
  }
  ...
  if (actuator_classes.includes("gui")) {
    decisions.push({
      ...
      reason:
        "GUI or input automation is an actuator and remains outside Dema preview execution",
    });
  }
  if (actuator_classes.includes("mobile_agent")) {
    decisions.push({
      ...
      reason:
        "mobile-agent movement stays blocked until Node handoff and proof gates exist",
    });
  }
  ...
  if (decisions.length === 0 && actuator_classes.length === 0) {
    decisions.push({
      ...
      reason:
        "no effecting actuator class was detected; narrow intent before any future approval",
    });
  }
  return {
    mode: "PREVIEW_ONLY",
    approval_recorded: false,
    runtime_handoff_required: decisions.some(
      (decision) => decision.verdict === "requires_governed_runtime_handoff",
    ),
    decisions,
  };
}
...
function detectRiskLevel(permissions, actuator_classes) {
  if (
    permissions.some((p) =>
      ["write", "execute", "call", "spend"].includes(p.action),
    )
  ) {
    return "high";
  }
...
export function buildAnalogicalNotes(
  intent,
  permissions,
  unsafeFileReferences = [],
) {
  return [
    {
    ...
    {
      when:
        isAuditIntent(intent) && permissions.some((p) => p.action === "call"),
