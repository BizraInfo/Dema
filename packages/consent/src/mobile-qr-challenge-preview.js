...
export function buildMobileQrChallengePreview({
  ...
  if (!nonEmptyString(mission_id)) {
    return failChallenge("invalid_mission_id", "mission_id must be a non-empty string");
  }
  if (!nonEmptyString(action)) {
    return failChallenge("invalid_action", "action must be a non-empty string");
  }
  if (!nonEmptyString(purpose)) {
    return failChallenge("invalid_purpose", "purpose must be a non-empty string");
  }
  if (!isValidDate(now)) {
    return failChallenge("invalid_now", "now must be a valid Date");
  }
  if (!Number.isFinite(expires_in_seconds) || expires_in_seconds <= 0) {
    return failChallenge("invalid_expires_in_seconds", "expires_in_seconds must be a positive finite number");
  }

  const expires_at = new Date(now.getTime() + expires_in_seconds * 1000).toISOString();
  const canonicalPayload = stableStringify({ mission_id, action, purpose, expires_at });
...
function verificationResult({ ok, reason, detail = "", challenge_id = null, recorded_at = null }) {
  return deepFreeze(clone({
    schema: MOBILE_QR_CHALLENGE_PREVIEW_SCHEMA,
    mode: "PREVIEW_ONLY",
    truth_label: "DECLARED",
...
export function verifyMobileQrChallengePreview(challenge, typed_phrase, { now = new Date() } = {}) {
  if (!challenge || typeof challenge !== "object" || challenge.valid !== true) {
    return verificationResult({ ok: false, reason: "invalid_challenge", detail: "challenge must be a valid build result" });
  }
  if (!isValidDate(now)) {
    return verificationResult({ ok: false, reason: "invalid_now", detail: "now must be a valid Date", challenge_id: challenge.challenge_id });
  }
  if (!nonEmptyString(typed_phrase)) {
    return verificationResult({ ok: false, reason: "missing_phrase", detail: "typed_phrase must be a non-empty string", challenge_id: challenge.challenge_id });
  }

  const expiresMs = Date.parse(challenge.expires_at);
  if (!Number.isFinite(expiresMs) || now.getTime() >= expiresMs) {
    return verificationResult({ ok: false, reason: "expired", detail: "challenge has expired", challenge_id: challenge.challenge_id });
  }

  if (consumedChallengeIds.has(challenge.challenge_id)) {
    return verificationResult({ ok: false, reason: "replay", detail: "challenge_id was already consumed", challenge_id: challenge.challenge_id });
  }

  if (typed_phrase !== challenge.phrase) {
    return verificationResult({ ok: false, reason: "phrase_mismatch", detail: "typed phrase does not match challenge phrase", challenge_id: challenge.challenge_id });
  }
