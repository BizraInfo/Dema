// Verification Result Save — preview-grade local persistence for the v0.1
// routed invocation verification envelope emitted by
// `dema model-broker verify-invocation`.
//
// IMPORTANT NAMING (mirrors PR #83 route-receipt-save + PR #85
// invocation-result-save):
...
//   verification envelope JSON to a content-addressed file under
...
//
// SAVE-vs-CHAIN-BOUND: this save persists the deterministic verification
// judgment (verdict + 17 invariant probes) for operator audit. A future
// C12 path may consume these envelopes and graduate them into chain-bound
// mints; that path is out of v0.1 scope.
//
// Boundary:
//   - read-only over the verification object (never mutates the envelope body)
//   - write-only to $DEMA_HOME/receipts/verification-<sha256>.json (no other path)
//   - no network call
//   - no model invocation
...
export function buildVerificationResultSavePath(
  ...
  const filename = `verification-${sha}.json`;
...
// the route-receipt-save / invocation-result-save mirrors (PR #83 + #85).
...
// saveVerificationResult: persist a routed invocation verification envelope to
