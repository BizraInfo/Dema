 ...
 */
export function createThoughtPacket(
  type,
  claim,
  confidence,
  requiresHumanReview,
...
export function screenProposal(proposal) {
  ...
  if (businessActivity === "fail") {
    packets.push(
      createThoughtPacket(
        "objection",
        `Sector is prohibited by screening rule: ${sector}.`,
        0.95,
        true,
      ),
    );
  ...
  if (financialScreen !== "pass") {
    packets.push(
      createThoughtPacket(
        "risk",
        "Debt ratio exceeds configured screening threshold.",
        0.75,
        true,
      ),
    );
  ...
  if (tokenMechanics === "fail") {
    packets.push(
      createThoughtPacket("objection", "Guaranteed APR detected.", 0.95, true),
    );
  } else {
    packets.push(
      createThoughtPacket(
        "verification",
        "No guaranteed APR detected.",
        0.9,
        false,
      ),
    );
  ...
  // Impact evidence
  const evidence = Array.isArray(proposal?.impact_evidence)
  ...
  if (evidence.length === 0) {
    ...
    packets.push(
      createThoughtPacket(
        "risk",
        "No Proof-of-Impact evidence submitted.",
        0.9,
        true,
      ),
    );
...
function createDecisionHash(
  ...
  return (
    "sha256:" +
    createHmac("sha256", "local-demo-only").update(canonical).digest("hex")
  );
}
...
export function signReceipt(
  decision,
  typedGo,
  secretKey = Buffer.from("local-demo-key-replace-me"),
) {
  if (typedGo !== "GO") {
  ...
  return Object.freeze({
    receipt_id:
      "sha256:" +
      createHmac("sha256", secretKey).update(canonical).digest("hex"),
    payload,
    signature_scheme: "hmac-sha256-demo",
    signature,
    warning:
      "DEMO signature only. Replace with Ed25519 or wallet signature before any production use. [PROTOTYPE]",
  });
}
...
export function loadExampleProposal() {
  const proposalPath = path.resolve(
    __dirname,
    "../../../fixtures/covenant/example-impact-proposal.json",
  );
