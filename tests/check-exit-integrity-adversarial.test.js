...
test("A7 direct TAP is isolated and a later authoritative gate executes", () => {
  ...
  // 2026-08-20: NODE0-CONSTELLATION-REGISTRY-1A appends
  // node0-base-constellation-check directly after the capability-truth-registry
  // gate, ahead of the isolated TAP command: 208 -> 209, isolated 132 -> 133,
  // coverage 133 -> 134. Positions MEASURED by importing `commands`.
  assert.equal(commands.length, 209);
  assert.equal(commands.indexOf(isolated), 133);
  assert.deepEqual(commands[134].slice(0, 2), ["npm", ["run", "coverage"]]);
