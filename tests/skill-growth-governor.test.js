...
function makeValidCandidate(overrides = {}) {
  ...
  return {
    ...
    evidence_receipt_ids: overrides.evidence_receipt_ids ?? [
      "2026-05-18_082658",
    ],
    ...
    no_boundary_violation:
      overrides.no_boundary_violation !== undefined
        ? overrides.no_boundary_violation
        : true,
    ...
    protected_namespace_override:
      overrides.protected_namespace_override ?? false,
  };
}
...
test("consent block locks ADR-005 exact-string discipline", () => {
  ...
  assert.equal(
    r.consent.promotion_phrase_template,
    "GO promote skill <id> v<version>",
  );
  assert.equal(
    r.consent.promotion_phrase_template,
    SKILL_GROWTH_GOVERNOR_PROMOTION_PHRASE_TEMPLATE,
...
test("ADVERSARIAL: self-reflection-only candidate is REFUSED", () => {
  ...
  const e = r.candidate_evaluations[0];
  assert.ok(
    e.refusals.includes("refuse_to_score_skill_by_self_reflection_alone"),
  );
});
...
test("ADVERSARIAL: protected namespace 'boundary' without override is REFUSED", () => {
  ...
  assert.ok(
    r.candidate_evaluations[0].refusals.includes(
      "refuse_to_create_skill_overlapping_protected_namespace",
    ),
  );
});
...
test("ADVERSARIAL: success_metric.passed=false → treated as no usable metric", () => {
  ...
  assert.equal(e.gates.success_metric_present.passed, false);
  assert.equal(
    e.gates.success_metric_present.reason,
...
test("ADVERSARIAL: no_boundary_violation defaults to false (must be explicitly true)", () => {
  ...
  assert.equal(
    r2.candidate_evaluations[0].gates.no_boundary_violation.passed,
    false,
  );
});
...
test("ADVERSARIAL: sat_review_status not 'passed' → gate refuses", () => {
  ...
  assert.equal(
    r.candidate_evaluations[0].gates.sat_review_passed.passed,
    false,
  );
});
...
test("ADVERSARIAL: wrong consent phrase typed → human_consent_received fails", () => {
  ...
  assert.ok(
    e.refusals.includes("refuse_to_emit_skill_change_without_typed_consent"),
  );
});
...
test("ADVERSARIAL: prototype-pollution attempt via candidate input does NOT leak", () => {
  ...
  Object.setPrototypeOf(polluted, {
    secret_skill_overwrite_token: "SHOULD_NOT_LEAK",
  });
...
test("ADVERSARIAL: mutation attempt on returned candidate_evaluations is rejected (frozen)", () => {
  ...
  try {
    r.candidate_evaluations.push({ skill_id: "evil" });
  } catch (e) {
    /* expected */
  }
  assert.equal(r.candidate_evaluations.length, 1);
  try {
    r.candidate_evaluations[0].refusals.push("fake_refusal");
  } catch (e) {
    /* expected */
  }
...
test("HERMES REGRESSION · CustomMerkins · agent self-overwrite of human-edited skill must be REFUSED", () => {
  ...
  assert.equal(
    e.next_action,
    "halt",
    "Hermes-mode promotion MUST yield next_action: halt",
  );
  // All 4 refusal paths from the doctrine must fire on this scenario
  assert.ok(
    e.refusals.includes("refuse_to_overwrite_human_edited_skill"),
  ...
  assert.ok(
    e.refusals.includes("refuse_to_promote_without_evidence"),
  ...
  assert.equal(
    e.promotion_phrase_required,
    "GO promote skill fetch_water_test_results v3",
  );
});
