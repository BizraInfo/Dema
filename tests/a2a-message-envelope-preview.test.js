...
test("T-04 A2A_MESSAGE_TYPES is the frozen canonical enum", () => {
  ...
  for (const mt of A2A_MESSAGE_TYPES) {
    const env = buildA2aMessageEnvelopePreview({
      ...VALID_ARGS,
      message_type: mt,
    });
    assert.equal(env.valid, true);
...
test("T-08 effect_level outside read-only subset rejected", () => {
  for (const bad of [
    ...
    assert.equal(
      env.valid,
      false,
      `effect_level ${JSON.stringify(bad)} must reject`,
    );
...
test('T-09 effect_level accepts [] and ["read"] only', () => {
  const empty = buildA2aMessageEnvelopePreview({
    ...VALID_ARGS,
    effect_level: [],
  });
  assert.equal(empty.valid, true);
  assert.deepEqual(empty.effect_level, []);
  const read = buildA2aMessageEnvelopePreview({
    ...VALID_ARGS,
    effect_level: ["read"],
  });
...
test("T-10 claims must be an array of strings; non-array or non-string entries reject", () => {
  for (const bad of [
    ...
    assert.equal(
      env.valid,
      false,
      `claims=${JSON.stringify(bad.claims)} must reject`,
    );
...
test("T-15 deterministic: same args produce deeply-equal envelope with fresh references", () => {
  ...
  assert.notEqual(
    a.boundary,
    b.boundary,
    "boundary must be a fresh frozen reference per call",
  );
...
test("T-16 module is pure (no fs/http/net/child_process imports)", async () => {
  ...
  assert.ok(
    ...
    "must not call spawn/exec/fetch",
  );
});
