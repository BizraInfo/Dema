...
const specDir = fileURLToPath(new URL("../docs/superpowers/specs/2026-05-14-effectcap-invariant/", import.meta.url));
const specFiles = [
...
const registry = new Map([
  ["file:notes:read", {
    risk: "file",
    action: "read",
    validate: (params) => typeof params?.path === "string" && !("payload" in params),
    run: () => ({ ok: true, effect: "read" })
  }],
  ["file:notes:write", {
    risk: "file",
    action: "write",
    validate: (params) => typeof params?.path === "string" && typeof params?.payload === "string",
...
function performPreview(intent, consent, now = new Date("2026-05-14T00:00:00.000Z")) {
  if (typeof intent?.exec === "function" || typeof intent?.params?.exec === "function") {
  ...
  if (consent.resource_id !== intent.resourceId) return deny("resource_mismatch");
...
test("EffectCap invariant spec files exist and stay under 500 lines", async () => {
  for (const file of specFiles) {
    ...
    assert.ok(body.split("\n").length < 500, `${file} should stay under 500 lines`);
  }
});

test("EffectCap spec states the object-capability law", async () => {
  const body = await readFile(new URL("01_specification.md", `file://${specDir}/`), "utf8");
...
test("declared read intent cannot trigger write or execute effects", () => {
  assert.deepEqual(
    performPreview(
      { resourceId: "file:notes", action: "read", params: { path: "notes.md", payload: "write attempt" } },
      { status: "active", resource_id: "file:notes", action: "read", expires_at: "2026-05-15T00:00:00.000Z" }
    ),
    deny("invalid_params")
  );
});
...
test("revoked consent blocks a previously valid mission", () => {
  const intent = { resourceId: "file:notes", action: "read", params: { path: "notes.md" } };
  const active = { status: "active", resource_id: "file:notes", action: "read", expires_at: "2026-05-15T00:00:00.000Z" };
...
test("unknown operations fail closed", () => {
  assert.deepEqual(
    performPreview(
      { resourceId: "service:unknown", action: "call", params: { url: "https://example.invalid" } },
      { status: "active", resource_id: "service:unknown", action: "call", expires_at: "2026-05-15T00:00:00.000Z" }
    ),
...
test("invalid params deny before sealed registry dispatch", () => {
  assert.deepEqual(
    performPreview(
      { resourceId: "file:notes", action: "write", params: { path: "notes.md" } },
      { status: "active", resource_id: "file:notes", action: "write", expires_at: "2026-05-15T00:00:00.000Z" }
    ),
    deny("invalid_params")
  );
});
...
test("high-risk Bash requires explicit human approval", () => {
  const intent = { resourceId: "command:pytest", action: "execute", params: { args: ["tests/status.test.js"] } };
  const consent = {
    status: "active",
  ...
  assert.deepEqual(performPreview(intent, consent), deny("explicit_human_approval_required"));
  assert.equal(performPreview(intent, { ...consent, explicit_human_approval: true }).allowed, true);
});
