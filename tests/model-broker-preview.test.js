...
test("local_only=true rejects remote models with explicit reason", () => {
  ...
  const receipt = routeForTask(broker, { task_kind: "planning", local_only: true });
  ...
  assert.match(receipt.rejected_candidates[0].reason, /locality_remote_under_local_only/);
});
...
test("unknown-locality model requires explicit allow_unknown=true to be selected", () => {
  ...
  const rejectedReceipt = routeForTask(broker, { task_kind: "planning", local_only: true });
  assert.equal(rejectedReceipt.selected_model_id, null);
  assert.match(rejectedReceipt.rejected_candidates[0].reason, /locality_unknown_under_local_only/);
...
test("rejected candidates each include a reason string", () => {
  ...
  const receipt = routeForTask(broker, { task_kind: "claim_review", local_only: true });
  ...
  for (const r of receipt.rejected_candidates) {
    ...
    assert.ok(r.reason.length > 0, `rejection for ${r.model_id} has empty reason`);
  }
});
...
test("max_size_class constraint rejects oversized candidates", () => {
  ...
  assert.ok(demaFaceRejection, "dema-face candidate should appear in rejected list");
