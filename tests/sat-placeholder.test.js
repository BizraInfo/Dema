...
function makeGatewayHandoffReceipt(overrides = {}) {
  return {
    ...
    receipt_id:
      "1ae13ab609c3b88eebaeb177abac386e893ecc978ef39599ec5f537a6a1e964b",
    artifact_id: "ARTIFACT-011",
    action: "bounded_diagnostic_activation",
    truth_label: "GATEWAY_ISSUED_HANDOFF",
    created_at: "2026-05-06T12:00:00.000Z",
    handoff_note:
      "Gateway sealed first mission; Dema-local mirror for listReceipts/readReceipt.",
    gateway: {
      base_url: "http://127.0.0.1:7421",
      mission_id:
        "04273dc2427284446a5aa7ec6727d33c085bbe6396659602dad9ba05ffb9fe86",
      receipt_id:
        "1ae13ab609c3b88eebaeb177abac386e893ecc978ef39599ec5f537a6a1e964b",
      chain_head:
        "9391e6fe08cb1671daa99eb28f3d574b06ea6c9c88736111436ccec89ad78483",
      chain_length: 8,
      admissibility_verdict: "Permit",
      final_stage: "Replayability",
    },
    proof_anchors: {
      evidence_hash_niyyah_sha256:
        "659d822ba4cbaa61dac2d61008da0ed06f5c824a6208dc02f9b2b7fb2d5f8b27",
      preview_json_sha256:
        "d7cc50207ea88004eafbc54e01225de46fcf8d4701114b78b3c36bbcfaaf9f0d",
      ideal_state_hash_sha256:
        "24938181d3b2d4e85ca9abb924557857552e6493a6343b87a1e989abc10a6efe",
    },
...
test("verifyReceipt routes task-receipt schema to verifyReceiptPlaceholder", async () => {
  ...
  const taskReceipt = await runDownloadsAuditPreview({
    downloadsRoot,
    demaRoot,
  });
...
test("verifyGatewayHandoffReceipt PARTIAL_PLACEHOLDER on a valid handoff receipt", () => {
  ...
  assert.ok(
    ...
    `all checks should pass; got ${JSON.stringify(verdict.checks)}`,
  );
});
...
test("verifyGatewayHandoffReceipt REJECTs when gateway.admissibility_verdict is not Permit", () => {
  ...
  assert.ok(
    verdict.checks.find(
      (c) => c.check === "gateway_admissibility_permit" && !c.pass,
    ),
  );
});
...
test("verifyGatewayHandoffReceipt REJECTs when chain_head is missing or malformed", () => {
  ...
  assert.ok(
    verdict.checks.find(
      (c) => c.check === "chain_head_present_64hex" && !c.pass,
    ),
  );
});
...
test("verifyGatewayHandoffReceipt REJECTs when consent_phrase_record is missing", () => {
  ...
  assert.ok(
    verdict.checks.find(
      (c) => c.check === "consent_phrase_recorded_and_canonical" && !c.pass,
    ),
  );
});
...
test("verifyGatewayHandoffReceipt REJECTs wrong consent phrase for a known action", () => {
  ...
  assert.equal(verdict.verdict, "REJECT");
  const consentCheck = verdict.checks.find(
    (c) => c.check === "consent_phrase_recorded_and_canonical",
  );
...
test("verifyGatewayHandoffReceipt PASSes when gateVerdicts is entirely missing", () => {
  ...
  const verdict = verifyGatewayHandoffReceipt(r);
  assert.equal(verdict.verdict, "PARTIAL_PLACEHOLDER");
  const gatesCheck = verdict.checks.find(
    (c) => c.check === "gate_verdicts_exposed",
  );
...
test("verifyGatewayHandoffReceipt REJECTs when IHSAN_FLOOR score is below 0.95", () => {
  ...
  r.preserved_post_response_body.admissibility.gateVerdicts =
    r.preserved_post_response_body.admissibility.gateVerdicts.map((v) =>
      v.scorerId === "IHSAN_FLOOR" ? { ...v, score: 0.94 } : v,
    );
  const verdict = verifyGatewayHandoffReceipt(r);
  assert.equal(verdict.verdict, "REJECT");
  assert.ok(
    verdict.checks.find(
      (c) =>
        c.check === "gate_verdicts_all_permit_with_required_scorers" && !c.pass,
    ),
  );
});
