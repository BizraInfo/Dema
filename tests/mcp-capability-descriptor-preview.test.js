...
const modulePath = fileURLToPath(new URL("../packages/consent/src/mcp-capability-descriptor-preview.js", import.meta.url));
...
test("T-04 every entry of declared_effects must be in OPERATIONS", () => {
  for (const bad of [["bogus"], ["read", "nope"], [""]]) {
    const d = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, declared_effects: bad });
    assert.equal(d.valid, false, `declared_effects ${JSON.stringify(bad)} should be rejected`);
    assert.ok(d.denial);
  }
  for (const op of OPERATIONS_LIST) {
    const d = buildMcpCapabilityDescriptorPreview({
      ...
      sat_verdict_required: (op === "execute" || op === "call") ? "REVIEW" : "PERMIT"
    });
...
test("T-06 resource_type must be in RESOURCE_TYPES", () => {
  for (const rt of RESOURCE_TYPES_LIST) {
    const d = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, resource_type: rt });
    assert.equal(d.valid, true, `RESOURCE_TYPES member ${rt} should be accepted`);
  }
  for (const bad of ["socket", "", "FILE", null]) {
    const d = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, resource_type: bad });
    assert.equal(d.valid, false, `resource_type ${JSON.stringify(bad)} should be rejected`);
    assert.ok(d.denial);
  }
});
...
test("T-07 consent_field_required must be in MICRO_CONSENT_SHAPE or null", () => {
  for (const field of MICRO_CONSENT_SHAPE_LIST) {
    const d = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, consent_field_required: field });
    assert.equal(d.valid, true, `MICRO_CONSENT_SHAPE member ${field} should be accepted`);
  }
  const dNull = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, consent_field_required: null });
  assert.equal(dNull.valid, true);
  assert.equal(dNull.consent_field_required, null);

  for (const bad of ["bogus_field", "", "MISSION_ID"]) {
    const d = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, consent_field_required: bad });
    assert.equal(d.valid, false, `consent_field_required ${JSON.stringify(bad)} should be rejected`);
    assert.ok(d.denial);
  }
});
...
test("T-08 sat_verdict_required must be in GateVerdict", () => {
  for (const verdict of GATE_VERDICT_LIST) {
    ...
    assert.equal(d.valid, true, `GateVerdict member ${verdict} should be accepted`);
  }
  for (const bad of ["ALLOW", "permit", "", null]) {
    const d = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, sat_verdict_required: bad });
    assert.equal(d.valid, false, `sat_verdict_required ${JSON.stringify(bad)} should be rejected`);
    assert.ok(d.denial);
  }
});
...
test("T-14 fail-closed on invalid tool_id (empty, non-string, missing)", () => {
  for (const bad of [{ tool_id: "" }, { tool_id: "   " }, { tool_id: null }, { tool_id: 42 }, { tool_id: undefined }]) {
...
test("T-16 fail-closed envelope keeps invocable_now=false and boundary all false", () => {
  const d = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, resource_type: "bogus" });
  assert.equal(d.valid, false);
  assert.equal(d.invocable_now, false);
  for (const key of [
    "runtime",
    ...
    assert.equal(d.boundary[key], false, `fail-closed boundary.${key} must be false`);
  }
});
...
test("T-17 module is pure (no fs/http/net/child_process imports, no spawn/exec)", async () => {
  ...
  assert.ok(!/spawn\(|execSync\(|execFile\(|spawnSync\(|exec\(/.test(body), "must not call spawn/exec*");
});

test("T-18 invalid now (not a Date) produces fail-closed envelope", () => {
  for (const bad of [{ now: "2026-05-16" }, { now: 1715865600000 }, { now: new Date("invalid") }, { now: null }]) {
    const d = buildMcpCapabilityDescriptorPreview({ ...VALID_ARGS, ...bad });
    assert.equal(d.valid, false, `now ${JSON.stringify(bad.now)} should be rejected`);
    assert.ok(d.denial);
  }
});
