...
async function readRegistry(home) {
  const path = join(home, "consent", "used-nonces.json");
...
describe("KEYCONSENT-2B · verdict-attest nonce-registry integration", () => {
  it("DOD-10.1: first attest with a given consent proof → attested:true, registry has nonce", async () => {
    ...
    try {
      ...
      assert.ok(
        Object.prototype.hasOwnProperty.call(registry, FIXED_NONCE_A),
        "registry must contain the consumed nonce",
      );
      const entry = registry[FIXED_NONCE_A];
      assert.equal(entry.action_type, ATTEST_ACTION_TYPE);
      assert.equal(entry.target_hash, inputHashOf(VALID_INPUT));
      assert.equal(
        entry.consent_proof_hash,
        cp.consent_proof.consent_proof_hash,
      );
      assert.ok(
        typeof entry.consumed_at_iso === "string" &&
          entry.consumed_at_iso.length > 0,
        "consumed_at_iso must be captured",
      );
    } finally {
  ...
  it("DOD-10.2: second attest with SAME consent proof (replay) → attested:false, error consent_consent_nonce_already_used", async () => {
    const home = await freshHomeWithKey();
    try {
      const cp = await buildAttestConsent({ home, input: VALID_INPUT });
      ...
      assert.equal(second.error, "consent_consent_nonce_already_used");
    } finally {
      await rm(home, { recursive: true, force: true });
    }
  });
  ...
  it("DOD-10.4: pre-existing nonce in registry (out-of-band) → attest with that nonce → attested:false", async () => {
    ...
    try {
      ...
      assert.equal(r.attested, false);
      assert.equal(r.error, "consent_consent_nonce_already_used");

      // Pre-existing entry must NOT have been overwritten by the failed
      // call (kernel guarantees this; gate must not bypass).
      const registry = await readRegistry(home);
      assert.equal(
        registry[FIXED_NONCE_A].consent_proof_hash,
        "deadbeef".repeat(8),
      );
    } finally {
      await rm(home, { recursive: true, force: true });
