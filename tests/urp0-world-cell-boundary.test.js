...
import { admissionCard, admitHuman0, bindWorldCellSystemPlane, worldState } from '../scripts/genesis/urp0-runtime.mjs';
...
test('DEMA bridge: exposes only the governed mission handoff and preserves proposal binding', async () => {
  const root = mkdtempSync(join(tmpdir(), 'urp-dema-bridge-'));
  const source = mkdtempSync(join(tmpdir(), 'urp-dema-source-'));
  ...
  const { server, url } = await startUrp0Server({ stateRootDir: root, port: 0, demaBridge: true });
  try {
    const admitted = admitHuman0(root, { phrase: admissionCard().required_phrase, now_iso: '2026-09-10T00:00:00.000Z' });
    assert.equal(admitted.ok, true, JSON.stringify(admitted.blocked_by));

    const cardResponse = await fetch(`${url}/api/consent-card`, {
      method: 'POST',
      headers: {'content-type': 'application/json'},
      body: JSON.stringify({ root: source, mission_id: 'MISSION-0123456789abcdef', proposal_binding: binding }),
    });
    assert.equal(cardResponse.status, 200);
    const card = await cardResponse.json();
    assert.equal(card.contract.proposal_binding.bridge_hash, binding.bridge_hash);

    const runResponse = await fetch(`${url}/api/authorize`, {
      method: 'POST',
      headers: {'content-type': 'application/json'},
      body: JSON.stringify({ consent_context: card.consent_context, phrase: card.card.required_phrase }),
    });
    assert.equal(runResponse.status, 200);
    const run = await runResponse.json();
    assert.equal(run.ok, true, JSON.stringify(run.blocked_by));
    assert.equal(run.receipt.contract_hash, card.contract_hash);
    const missionKey = Object.keys(worldState(root).missions).find((key) => key.startsWith('MISSION-0123456789abcdef#'));
    assert.equal(worldState(root).missions[missionKey].status, 'RECEIPTED');

    for (const [method, path] of [['GET', '/api/admission-card'], ['POST', '/api/admit'], ['POST', '/api/block0/seal']]) {
      const response = await fetch(`${url}${path}`, { method, headers: {'content-type': 'application/json'}, body: method === 'POST' ? '{}' : undefined });
      assert.equal(response.status, 403, `${method} ${path}`);
      assert.equal((await response.json()).blocked_by[0], 'dema_bridge_route_not_exposed');
    }
  } finally {
    await new Promise(resolve => server.close(resolve));
    rmSync(root,{recursive:true,force:true});
