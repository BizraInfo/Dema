...
const pythonStatus = spawnSync("python3", [
  "-c",
  "import sys; sys.exit(0 if sys.version_info >= (3, 10) else 1)"
], { encoding: "utf8" });
const pythonSkip = pythonStatus.status === 0 ? false : "python3 >= 3.10 is required";
...
test("verify_artifacts records hashes without shell injection or absolute path leakage", { skip: pythonSkip }, async () => {
  ...
  try {
    const { stdout } = await execFileAsync("python3", [
      verifyScript,
      "--project-dir", project,
      "--description", "Verify local proof script safety",
      "--artifact", join(project, "src", "artifact.txt"),
      "--command", "echo safe; touch pwned.txt",
      "--output", reportPath
    ]);
...
test("forge_evidence appends and verifies a local proof-forge chain", { skip: pythonSkip }, async () => {
  ...
  try {
    await writeFile(reportPath, JSON.stringify({
      ...
      commands: [{ command: "true", cwd: ".", exit_code: 0, duration_seconds: 0, stdout: "", stderr: "", passed: true }],
      verification_type: "automated",
      all_passed: true
    }, null, 2));

    const first = JSON.parse((await execFileAsync("python3", [
      forgeScript,
      "--project-dir", project,
      "--description", "First local proof anchor",
      "--verification-report", reportPath
    ])).stdout);
    const second = JSON.parse((await execFileAsync("python3", [
      forgeScript,
      "--project-dir", project,
      "--description", "Second local proof anchor",
      "--verification-report", reportPath
    ])).stdout);

    const firstReceipt = await readJson(first.receipt);
    const secondReceipt = await readJson(second.receipt);
    const index = await readJson(join(project, ".proof-forge", "EVIDENCE_INDEX.json"));

    assert.equal(index.chain_length, 2);
    assert.equal(firstReceipt.chain.previous_hash, null);
    assert.equal(secondReceipt.chain.previous_hash, firstReceipt.chain.evidence_hash);

    const verified = JSON.parse((await execFileAsync("python3", [
      forgeScript,
      "--verify",
      "--project-dir", project
    ])).stdout);
    ...
    await assert.rejects(
      execFileAsync("python3", [forgeScript, "--verify", "--project-dir", project]),
      (error) => {
...
test("proof_summary writes only inside the requested project", { skip: pythonSkip }, async () => {
  ...
  try {
    ...
    await writeFile(receiptPath, JSON.stringify({
      ...
      artifacts: [{ path: "src/artifact.txt", type: "doc", sha256: "1".repeat(64) }],
      verification_report: { commands: [{ command: "true", passed: true, duration_seconds: 0 }] },
      chain: { position: 1, previous_hash: null, evidence_hash: "2".repeat(64) },
      confidence: { label: "Strong" }
    }, null, 2));

    const result = JSON.parse((await execFileAsync("python3", [
      summaryScript,
      "--receipt", receiptPath,
      "--project-dir", project
    ])).stdout);
    ...
    assert.equal(result.summary.startsWith(join(project, ".proof-forge", "summaries")), true);
    assert.equal(result.latest, join(project, "PROOF_SUMMARY.md"));
    assert.equal(await exists(join(repoRoot, ".proof-forge")), repoProofDirExisted);
    assert.equal(await exists(join(repoRoot, "PROOF_SUMMARY.md")), repoSummaryExisted);
  } finally {
    await rm(project, { recursive: true, force: true });
  }
});
