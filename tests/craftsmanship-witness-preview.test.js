...
test("CW-08: every MC invariant declares self_assertion + evidence_anchor", () => {
  ...
  for (const inv of out.master_craftsmanship_compliance.invariants) {
    assert.equal(
      typeof inv.self_assertion,
      "boolean",
      `${inv.id} missing self_assertion`,
    );
    assert.equal(
      typeof inv.evidence_anchor,
      "string",
      `${inv.id} missing evidence_anchor`,
    );
    assert.ok(
      inv.evidence_anchor.length > 0,
      `${inv.id} empty evidence_anchor`,
    );
  }
});
...
test("CW-09: every emitted next_slice_observable carries its own consent_phrase_required_to_act", () => {
  const out = buildCraftsmanshipWitnessPreview({
    next_slice_signals: [
      {
        id: "test-signal",
        text: "consider adding T-N",
        evidence: "N=8 preflight pattern",
      },
    ],
  });
  ...
  for (const s of out.next_slice_observables) {
    ...
    assert.ok(
      s.consent_phrase_required_to_act.startsWith(
        "GO: act on craftsmanship-witness suggestion ",
      ),
    ...
    assert.equal(
      s.auto_applied,
      false,
      "auto_applied must be structurally false",
    );
  }
});
...
test("CW-10: rsi_signals declare V/D/A/U claim_state on every entry", () => {
  const out = buildCraftsmanshipWitnessPreview({
    rsi_signal_inputs: [
      {
        kind: "first_run_green_streak",
        value: 8,
        claim_state: "V",
        evidence: "tests 1420/1420 first-run",
      },
      {
        kind: "refusal_density",
        value: 5,
        claim_state: "V",
        evidence: "refusal-as-product N=5",
      },
    ],
  });
  for (const s of out.rsi_signals) {
    assert.ok(
      ["V", "D", "A", "U"].includes(s.claim_state),
      `bad claim_state: ${s.claim_state}`,
    );
  }
});
...
test("CW-13: doctrine_health surfaces refusal_count + doctrine_catch_count + drift_markers", () => {
  const out = buildCraftsmanshipWitnessPreview({
    doctrine_health_inputs: {
      ...
      doctrine_catches: [
        { name: "preferred_name vs name", evidence: "commit 5b2e89e" },
      ],
    },
  });
...
test("CW-15: canon_anchors block cites foundational canon", () => {
  ...
  assert.ok(
    out.canon_anchors.law_of_assumption.includes("LAW_OF_ASSUMPTION.md"),
  );
...
test("CW-16: counters aggregate correctly across input arrays", () => {
  const out = buildCraftsmanshipWitnessPreview({
    ...
    doctrine_health_inputs: {
      refusal_events: [
        { phrase_refused: "x", reason: "y" },
        { phrase_refused: "z", reason: "w" },
      ],
...
test("ADV-05: caller cannot inject truth_label=PRODUCTION", () => {
  const out = buildCraftsmanshipWitnessPreview({
    truth_label: "PRODUCTION_VERIFIED",
  });
  assert.equal(out.truth_label, "NODE0_LOCAL_SEED");
});
...
test("ADV-06: non-array rsi_signal_inputs handled gracefully", () => {
  const out = buildCraftsmanshipWitnessPreview({
    rsi_signal_inputs: "not-an-array",
  });
...
test("ADV-08: function-valued evidence is silently dropped to empty string", () => {
  const out = buildCraftsmanshipWitnessPreview({
    rsi_signal_inputs: [
      { kind: "x", value: 1, claim_state: "V", evidence: () => "lol" },
    ],
  });
...
test("ADV-09: symbol-valued kind silently coerced", () => {
  const out = buildCraftsmanshipWitnessPreview({
    rsi_signal_inputs: [
      { kind: Symbol("not-string"), value: 1, claim_state: "V", evidence: "e" },
    ],
  });
...
test("ADV-12: deterministic: same input → byte-equal JSON", () => {
  const input = {
    ...
    slice_history: {
      commits_in_session: 5,
      tests_total: 100,
      tests_delta: 10,
      first_run_green_streak: 3,
    },
  };
...
test("ADV-15: empty input still produces valid Master Craftsmanship 10-invariant compliance", () => {
  ...
  assert.equal(
    typeof out.master_craftsmanship_compliance.overall_compliant,
    "boolean",
  );
});
...
test("EXPORTS-02: CRAFTSMANSHIP_WITNESS_REQUIRED_BLOCKED_EFFECTS frozen array of strings", () => {
  ...
  assert.equal(
    Object.isFrozen(CRAFTSMANSHIP_WITNESS_REQUIRED_BLOCKED_EFFECTS),
    true,
  );
