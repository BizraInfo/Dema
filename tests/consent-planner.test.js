...
test("unsafe file references produce a high-severity consent note", () => {
  ...
  assert.ok(
    notes.some(
      (note) =>
        note.code === "unsafe_file_reference" && note.severity === "high",
    ),
  );
});
...
test("buildConsentPlanPreview maps intent to a deterministic preview boundary", () => {
  ...
  const first = buildConsentPlanPreview({
    intent: "Fix auth.py and run pytest",
    now,
  });
  const second = buildConsentPlanPreview({
    intent: "Fix auth.py and run pytest",
    now,
  });
...
test("buildConsentPlanPreview exposes actuator classes and policy decisions", () => {
  ...
  assert.deepEqual(
    plan.policy_preview.decisions.map(
      (decision) => `${decision.verdict}:${decision.code}`,
    ),
    [
...
test("policy preview covers every detected actuator class without false no-effect decisions", () => {
  ...
  for (const item of cases) {
    ...
    const decision = plan.policy_preview.decisions.find(
...
test("actuator classification covers unsafe paths, spend, GUI, and mobile-agent hints", () => {
  const plan = buildConsentPlanPreview({
    intent:
      "Use the desktop GUI to move a mobile agent across hosts, spend 5 credits, and edit ~/private/key.py",
    now: new Date("2026-05-15T00:00:00.000Z"),
  });

  assert.deepEqual(plan.actuator_classes, ["gui", "mobile_agent", "spend"]);
  assert.equal(plan.mission_draft.risk_level, "high");
  assert.ok(plan.unsafe_file_references.includes("~/private/key.py"));
  assert.ok(
    plan.policy_preview.decisions.some(
      (decision) =>
...
test("consent plan exposes self-proactive, critique, micro-compliance, micro-consent, and analogical harnesses", () => {
  ...
  assert.equal(
    plan.self_proactive_harness.mode,
    "DETERMINISTIC_CONSENT_POLICY_PREVIEW",
  );
  assert.match(plan.self_proactive_harness.mode, /^DETERMINISTIC_/);
  assert.equal(
    plan.self_proactive_harness.recommended_micro_action,
    "draft_exact_micro_consent_scope",
  );
  assert.equal(
    plan.self_proactive_harness.gates.find(
      (gate) => gate.gate === "policy_covers_detected_actuators",
    ).pass,
    true,
  );
  ...
  assert.equal(
    plan.micro_consent.exact_string_required_for_gated_actions,
    true,
  );
...
test("formatConsentPlanPreview renders permissions and preview boundary", () => {
  const output = formatConsentPlanPreview(
    buildConsentPlanPreview({
      intent: "Fix auth.py and run pytest",
      now: new Date("2026-05-15T00:00:00.000Z"),
    }),
