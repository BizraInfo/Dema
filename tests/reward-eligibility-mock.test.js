...
test("G20 accepts ADR-024 allowed input fields and emits local mock review shape", () => {
  ...
  assert.equal(
    result.review.schema,
    "bizra.impact.reward-eligibility.mock-review.v0.1",
  );
  ...
  assert.equal(
    result.review.receipt_expectation.schema,
    "bizra.impact.reward.eligibility.v0.1.local",
  );
  assert.equal(result.review.receipt_expectation.placeholder, true);
  assert.equal(
    result.review.prototype_posture,
...
test("G20 output excludes forbidden economic, public, bridge, and certification fields", () => {
  ...
  for (const key of forbiddenOutputKeys) {
    assert.equal(
      Object.hasOwn(result.review, key),
      false,
