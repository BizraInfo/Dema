...
test("accepts one valid local write-plan input", () => {
  ...
  const plan = createMockRewardReceiptLocalWritePlan(
    { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
    input,
  );
  assert.ok(
    plan.local_write_plan_id && plan.local_write_plan_id.startsWith("sha256:"),
    "accepts valid input and returns id [DECLARED]",
  );
  assert.ok(
    plan.proposed_path && !plan.proposed_path.includes(".."),
    "proposed_path is safe [DECLARED]",
  );
});
...
test("rejects missing exact consent", () => {
  ...
  assert.throws(
    () =>
      createMockRewardReceiptLocalWritePlan({ requireConsent: "WRONG" }, input),
    /CONSENT_REQUIRED/,
...
test("rejects forbidden mint/write/publish/bridge/economic language", () => {
  const input = {
    ...loadExampleRewardReceiptLocalWriteInput(),
    receipt_context: { note: "expected mint and reward" },
  };
  assert.throws(
    () =>
      createMockRewardReceiptLocalWritePlan(
    ...
    /FORBIDDEN_PROMOTION/,
    "rejects forbidden language [DECLARED]",
  );
});
...
test("rejects unsafe path traversal", () => {
  ...
  assert.throws(
    ...
    /UNSAFE_PATH/,
    "rejects unsafe path traversal [DECLARED]",
  );
});
...
test("never writes to filesystem", () => {
  ...
  const hasWriteSideEffect =
    "file_written" in plan ||
    "receipt_minted" in plan ||
    "reward_authorized" in plan ||
    "token_amount" in plan ||
    "contract_call" in plan;
  assert.ok(
    !hasWriteSideEffect,
    "never returns filesystem or mint side-effect fields [DECLARED]",
  );
});
...
test("deterministic local_write_plan_id for same semantic input", () => {
  ...
  const p2 = createMockRewardReceiptLocalWritePlan(
    { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
    input,
  );
  assert.strictEqual(
    p1.local_write_plan_id,
    p2.local_write_plan_id,
    "deterministic id [DECLARED]",
  );
});
...
test("exercises all four write_status values", () => {
  ...
  assert.strictEqual(
    statuses.size,
    4,
    "exercises all four write_status [DECLARED]",
  );
});
