...
} from "../scripts/reward-receipt-mock.mjs";
...
test("accepts one valid local mock receipt review input", () => {
  ...
  const review = createMockRewardReceiptReview(
    { requireConsent: REWARD_RECEIPT_MOCK_CONSENT },
    input,
  );
  assert.ok(
    review.receipt_review_id && review.receipt_review_id.startsWith("sha256:"),
...
test("rejects missing exact consent", () => {
  const input = loadExampleRewardReceiptInput();
  assert.throws(
    () => createMockRewardReceiptReview({ requireConsent: "WRONG" }, input),
    /CONSENT_REQUIRED/,
...
test("rejects forbidden mint/write/publish/bridge language", () => {
  const input = {
    ...loadExampleRewardReceiptInput(),
    receipt_context: { note: "expected mint and publish" },
  };
  assert.throws(
    () =>
      createMockRewardReceiptReview(
...
test("never returns mint/write/reward/token/contract fields", () => {
  ...
  const forbiddenPresent =
    "receipt_written" in review ||
    "receipt_minted" in review ||
    "reward_authorized" in review ||
    "token_amount" in review ||
    "contract_call" in review;
  assert.ok(
    !forbiddenPresent,
    "never returns forbidden economic/mint fields [DECLARED]",
  );
});
...
test("deterministic receipt_review_id for same input", () => {
  ...
  const r2 = createMockRewardReceiptReview(
    { requireConsent: REWARD_RECEIPT_MOCK_CONSENT },
    input,
  );
  assert.strictEqual(
    r1.receipt_review_id,
    r2.receipt_review_id,
...
test("exercises all four receipt_status values", () => {
  ...
  assert.strictEqual(
    statuses.size,
    4,
    "exercises all four receipt_status [DECLARED]",
  );
});
