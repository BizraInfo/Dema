...
const modulePath = new URL("../packages/core/src/corpus-manual-review-queue-preview.js", import.meta.url);
...
test("buildCorpusManualReviewQueuePreview emits a prioritized metadata-only queue", () => {
  ...
  assert.equal(preview.summary.highest_priority_candidate, "gold_architecture_eval");
  assert.deepEqual(preview.queue_items.map((item) => item.candidate_id).slice(0, 4), [
...
test("queue lanes keep D3 and D4 non-actionable", () => {
  const preview = buildCorpusManualReviewQueuePreview();
  const byId = new Map(preview.queue_items.map((item) => [item.candidate_id, item]));

  assert.equal(byId.get("private_strategy_quarantine").queue_lane, "quarantine_review");
  ...
  assert.equal(byId.get("credential_marker_reject").review_state, "log_rejection_without_opening_content");
  assert.equal(preview.self_proactive_harness.gates.find((gate) => gate.gate === "d3_d4_non_actionable_lanes").pass, true);
...
test("ownership is provenance and never processing authorization", () => {
  ...
  assert.equal(preview.ownership_scope.interpretation, "local_ownership_and_provenance_not_processing_authorization");
...
test("preview emits self-proactive harness, self-critique, micro-compliance, and analogical model", () => {
  ...
  assert.equal(preview.self_proactive_harness.mode, "DETERMINISTIC_MANUAL_REVIEW_QUEUE_PREVIEW");
  assert.equal(preview.self_proactive_harness.recommended_micro_action, "build_corpus_gold_label_fixture_preview");
  assert.equal(preview.self_proactive_harness.gates.find((gate) => gate.gate === "local_asset_metadata_only").pass, true);
  ...
  assert.equal(preview.analogical_model.model, "library_card_catalog_not_book_opening");
});
