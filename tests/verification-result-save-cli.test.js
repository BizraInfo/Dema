...
const SAVE_VERIFICATION_CONSENT = "GO: save local model invocation verification";
...
async function makeDemaHomeWithEnvelope(envelope, { filename = "invocation-deadbeefcafebabe.json" } = {}) {
...
test("'--save-verification-result' with valid consent writes file under $DEMA_HOME/receipts/verification-<hash>.json", async () => {
  ...
  const { exitCode } = await runCli(
    [
      "model-broker", "verify-invocation",
      "--invocation-result-file", path,
      "--save-verification-result",
      "--save-verification-consent", SAVE_VERIFICATION_CONSENT
    ],
    { env: { DEMA_HOME: home } }
  );
  assert.equal(exitCode, 0);
  const files = await readdir(join(home, "receipts"));
  const verificationFiles = files.filter((f) => f.startsWith("verification-") && f.endsWith(".json"));
  assert.equal(verificationFiles.length, 1, `expected 1 verification file; got ${verificationFiles.length}: ${files.join(",")}`);
...
test("'--save-verification-result' with wrong consent exits non-zero with consent_mismatch", async () => {
  ...
  const { stderr, exitCode } = await runCli(
    [
      ...
      "--invocation-result-file", path,
      "--save-verification-result",
      "--save-verification-consent", "wrong phrase"
    ],
    { env: { DEMA_HOME: home } }
  );
  assert.notEqual(exitCode, 0);
...
test("with valid consent, saved verification file matches stdout byte-for-byte", async () => {
  ...
  const files = await readdir(join(home, "receipts"));
  const verificationFile = files.find((f) => f.startsWith("verification-") && f.endsWith(".json"));
  assert.ok(verificationFile, "expected verification file");
  const onDisk = await readFile(join(home, "receipts", verificationFile), "utf8");
...
test("non_compliant verification envelope is STILL saved (auditability for both verdicts)", async () => {
  ...
  const files = await readdir(join(home, "receipts"));
  const verificationFile = files.find((f) => f.startsWith("verification-") && f.endsWith(".json"));
  assert.ok(verificationFile, "non_compliant verification envelope must still be saved");
  const onDisk = JSON.parse(await readFile(join(home, "receipts", verificationFile), "utf8"));
...
test("no --save-verification-result flag means no verification-*.json file is written", async () => {
  ...
  if (existsSync(join(home, "receipts"))) {
    const files = await readdir(join(home, "receipts"));
    const verificationFiles = files.filter((f) => f.startsWith("verification-"));
    assert.equal(verificationFiles.length, 0, "no verification-* file should exist without --save-verification-result");
  }
});
...
test("'--save-verification-result' on 'route' action exits non-zero (only valid for verify-invocation)", async () => {
  ...
  assert.notEqual(exitCode, 0);
  assert.match(stderr, /--save-verification-result is only valid for the 'verify-invocation' action/);
});
...
test("re-running verify-invocation creates content-addressed files; timestamps make each verification unique", async () => {
  ...
  const files = await readdir(join(home, "receipts"));
  const verificationFiles = files.filter((f) => f.startsWith("verification-") && f.endsWith(".json")).sort();
  assert.equal(verificationFiles.length, 2, `expected 2 distinct verification files (verified_at timestamps differ); got ${verificationFiles.length}`);
