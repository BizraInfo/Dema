...
test("dema memory query → Dema envelope wraps wrapper output", { skip: pythonSkip }, async () => {
...
test("dema memory query envelope passes through wrapper hits", { skip: pythonSkip }, async () => {
  const wrapper = await makeFakeWrapper();
  const { stdout } = await execFileAsync("node", [cliPath, "memory", "query", "TESTQ", "--json"], {
    env: { ...process.env, DEMA_AGENT_DB_QUERY_PATH: wrapper }
  });
...
test("dema memory query envelope has honest Dema-side boundary", { skip: pythonSkip }, async () => {
  const wrapper = await makeFakeWrapper();
  const { stdout } = await execFileAsync("node", [cliPath, "memory", "query", "TESTQ", "--json"], {
    env: { ...process.env, DEMA_AGENT_DB_QUERY_PATH: wrapper }
  });
...
test("dema memory query --top N passed to wrapper", { skip: pythonSkip }, async () => {
  const wrapper = await makeFakeWrapper();
...
test("dema memory query → wrapper missing → exit 1 with clear error envelope", { skip: pythonSkip }, async () => {
...
test("dema memory query → wrapper non-zero exit captured in envelope AND propagated to Dema exit code", { skip: pythonSkip }, async () => {
...
test("dema memory query → snippets truncated to 200 chars even if wrapper returns longer", { skip: pythonSkip }, async () => {
...
`);
  ...
  const env = JSON.parse(stdout);
  assert.equal(env.hits.length, 1);
  assert.equal(env.hits[0].snippet.length, 200, "Dema must defensively truncate snippets to 200 chars");
});

test("dema memory query → --top non-integer rejected", async () => {
  const res = await execFileAsync("node",
    [cliPath, "memory", "query", "Q", "--top", "abc"]
...
test("dema memory query → --top out of range [1, 20] rejected", async () => {
  const res = await execFileAsync("node",
    [cliPath, "memory", "query", "Q", "--top", "50"]
  ).catch((e) => ({ stdout: e.stdout, stderr: e.stderr, code: e.code }));
  assert.notEqual(res.code, 0, "Dema must reject --top > 20");
  assert.match(res.stderr ?? "", /top out of range/i);
});

test("dema memory query → boundary.tool_executed is false (canon: RUNTIME_EMISSION_STRICTLY_FALSE_KEYS)", { skip: pythonSkip }, async () => {
  const wrapper = await makeFakeWrapper();
  const { stdout } = await execFileAsync("node",
    [cliPath, "memory", "query", "Q", "--json"],
    { env: { ...process.env, DEMA_AGENT_DB_QUERY_PATH: wrapper }}
  ...
  const env = JSON.parse(stdout);
  assert.equal(env.boundary.tool_executed, false,
