...
test("dema models scan --json stdout contains no braille spinner chars", async () => {
  ...
  assert.doesNotMatch(stdout, BRAILLE_PATTERN, "--json output must not contain spinner frames");
  // Also verify it is valid JSON (regression guard).
  assert.doesNotThrow(() => JSON.parse(stdout), "output must still be valid JSON");
});
...
test("dema models scan (non-TTY subprocess) stdout contains no braille spinner chars", async () => {
  ...
  assert.doesNotMatch(stdout, BRAILLE_PATTERN, "non-TTY human output must not contain spinner frames");
