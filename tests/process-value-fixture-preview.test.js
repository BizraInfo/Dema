...
const modulePath = new URL(
  "../packages/core/src/process-value-fixture-preview.js",
  import.meta.url,
);
...
const expectedStates = new Map([
  [
    "clean_progress",
    ["proof_process_preview", "continue_verified_micro_slice"],
  ],
  ["dirty_step7_gated", ["process_dirty", "restore_clean_baseline"]],
  [
    "clean_step7_hold",
    ["node0_proof_ready_step7_gated", "hold_step7_ceremony"],
  ],
  [
    "noisy_failure",
    ["proof_process_preview", "reduce_noise_before_next_slice"],
  ],
  [
    "node_connection_blocked",
    ["node_connection_gated", "continue_preview_only_readiness"],
  ],
...
test("default fixture pack matches locked golden states", () => {
  ...
  for (const entry of pack.entries) {
    ...
    assert.deepEqual(
      [
        entry.preview_summary.process_state,
        entry.preview_summary.next_safe_action,
      ],
      expected,
    );
...
test("dirty Step 7 fixture restores clean baseline before ceremony work", () => {
  ...
  const entry = pack.entries.find(
    (candidate) => candidate.id === "dirty_step7_gated",
  );
...
test("clean Step 7 fixture locks ceremony into hold posture", () => {
  const pack = buildProcessValueFixturePackPreview();
  const entry = pack.entries.find(
    (candidate) => candidate.id === "clean_step7_hold",
  );

  assert.equal(
    entry.preview_summary.process_state,
    "node0_proof_ready_step7_gated",
  );
...
test("malformed inner fixture is separate from malformed pack rejection", () => {
  const pack = buildProcessValueFixturePackPreview();
  const entry = pack.entries.find(
    (candidate) => candidate.id === "malformed_rejected",
  );
...
test("fixture pack keeps every authority boundary false", () => {
  ...
  for (const key of expectedFalseBoundaries) {
    ...
    for (const entry of pack.entries)
      assert.equal(entry.boundary[key], false, `${entry.id}.${key}`);
  }
});
...
test("fixture pack emits micro-compliance, micro-consent, and analogy without authority", () => {
  ...
  assert.equal(
    pack.micro_consent.current_preview_requires_operator_authorization,
    false,
  );
  ...
  assert.equal(
    pack.analogical_model.boundary,
    "fixture_pack_not_process_engine",
  );
});
...
test("fixture pack is deterministic, deeply frozen, and returns fresh objects", () => {
  ...
  assert.equal(
    Object.isFrozen(first.entries[0].fixture.processEvents[0]),
    true,
  );
