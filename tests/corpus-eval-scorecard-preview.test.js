...
const modulePath = new URL("../packages/core/src/corpus-eval-scorecard-preview.js", import.meta.url);
...
test("scorecard declares proof axes and threshold directions without execution", () => {
  const preview = buildCorpusEvalScorecardPreview();
  const byId = new Map(preview.metric_slots.map((slot) => [slot.metric_id, slot]));

  assert.deepEqual(byId.get("correctness_floor").proof_of_truth_axes, ["empirical", "formal"]);
...
test("D3/D4 participation is limited to quarantine integrity", () => {
  ...
  assert.equal(preview.self_proactive_harness.gates.find((gate) => gate.gate === "quarantine_integrity_is_only_d3_d4_metric").pass, true);
...
test("malformed scorecards fail closed without echoing raw content or scores", () => {
  ...
  assert.doesNotMatch(JSON.stringify(raw), /raw-score-answer-should-not-appear/);
  assert.equal(badMetric.reason, "metric_id_not_allowlisted");
  assert.equal(badScore.reason, "metric_must_not_include_raw_content_or_scores");
});

test("preview emits self-proactive harness, self-critique, micro-compliance, and analogical model", () => {
  const preview = buildCorpusEvalScorecardPreview();

  assert.equal(preview.self_proactive_harness.mode, "DETERMINISTIC_EVAL_SCORECARD_PREVIEW");
  assert.equal(preview.self_proactive_harness.recommended_micro_action, "build_corpus_scorecard_receipt_schema_preview");
  assert.equal(preview.self_proactive_harness.gates.find((gate) => gate.gate === "no_score_computation").pass, true);
  ...
  assert.equal(preview.analogical_model.model, "scoreboard_layout_not_game_played");
});
