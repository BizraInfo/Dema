...
test("B2 source control: exact consent approves its sole named act", () => {
  const { b2 } = observeVulnerableKernel();
  assert.deepEqual(b2.single_exact, { "ACT-A": true });
});

test("B2 source: unrelated or differently-scoped GO cannot approve another pending act", () => {
  const { b2 } = observeVulnerableKernel();
  assert.deepEqual({
  ...
  }, "B2 source: generic GO consumed authority for acts it did not name");
});

test("B3 source control: an observed true predicate completes", () => {
  const { b3 } = observeVulnerableKernel();
  assert.deepEqual(b3.observed_true, { state: "archived", passed: ["predicate"] });
});

test("B3 source: an unobserved or false required predicate cannot complete", () => {
  const { b3 } = observeVulnerableKernel();
  assert.deepEqual({
  ...
  }, "B3 source: cmd_run promoted unobserved or false predicates to passed completion");
});
