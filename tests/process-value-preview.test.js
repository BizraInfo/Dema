...
test("computeProcessRsi handles neutral and malformed inputs explicitly", () => {
  ...
  assert.equal(computeProcessRsi({ events: [{ type: "gate_passed" }], window: 0 }).verdict, "PREVIEW_REJECT");
  assert.equal(computeProcessRsi({ events: [{ type: "gate_passed", weight: Infinity }] }).verdict, "PREVIEW_REJECT");
  assert.equal(computeProcessRsi({ events: [{ type: "gate_failed" }], window: 20 }).events_considered, 1);
});
...
test("computeSNRValue separates signal from noise", () => {
  ...
  assert.equal(computeSNRValue({ signalEvents: -1, noiseEvents: 0 }).verdict, "PREVIEW_REJECT");
});
...
test("buildTrueValuePreview fails closed across each malformed input family", () => {
  ...
  for (const preview of [malformedProcessEvents, malformedProofSignals, malformedBlockers, malformedNow]) {
    assert.equal(preview.process_state, "preview_reject");
    assert.equal(preview.true_value_score, null);
    assert.equal(preview.ihsan_safety, null);
  }
  assert.equal(malformedNow.checked_at, null);
  assert.equal(malformedNow.checks.find((check) => check.check === "checked_at_valid").pass, false);
});
...
test("buildTrueValuePreview uses structured blockers rather than Step 7 text matching", () => {
  const textOnly = buildTrueValuePreview({
    processEvents: [{ type: "gate_passed" }],
    proofSignals,
    blockers: [{ kind: "resolved_note", severity: "review", note: "resolved step7 last week" }],
    now: fixedNow
  });
...
test("buildTrueValuePreview always uses allowlisted safe actions and invariant blocks", () => {
  const previews = [
    buildTrueValuePreview({ processEvents: [{ type: "dirty_tree" }], proofSignals, blockers: [], now: fixedNow }),
    buildTrueValuePreview({ processEvents: [{ type: "gate_passed" }], proofSignals, blockers: [], now: fixedNow }),
    buildTrueValuePreview({ processEvents: [{ type: "gate_passed" }], proofSignals, blockers: [{ kind: "step7_ready_unminted", severity: "halt_gate" }], now: fixedNow })
  ];
...
test("process value preview source has no runtime, network, or filesystem side effects", async () => {
  ...
  assert.doesNotMatch(source, /from\s+["']node:(net|dgram|http|https|tls|dns|child_process|fs)["']/);
});
