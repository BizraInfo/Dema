...
test("writes one local receipt artifact under temporary DEMA_HOME only", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    const result = await writeLocalRewardReceipt(
      { requireConsent: REWARD_RECEIPT_LOCAL_WRITER_CONSENT, demaHome },
      input,
    );
    assert.strictEqual(
      result.write_result_status,
      "local_write_performed_local_only",
      "performs local write [DECLARED]",
    );
    const rel = relative(demaHome, result.final_local_path);
    assert.ok(
      rel && !rel.startsWith("..") && !rel.startsWith("/"),
      "path is contained under DEMA_HOME [DECLARED]",
    );
...
test("rejects unsafe path traversal", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    assert.strictEqual(
      result.write_result_status,
      "local_write_refused_unsafe_path",
      "rejects unsafe traversal [DECLARED]",
    );
  });
});
...
test("rejects absolute arbitrary path", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    assert.strictEqual(
      result.write_result_status,
      "local_write_refused_unsafe_path",
      "rejects absolute arbitrary path [DECLARED]",
    );
  });
});
...
test("writes canonical JSON and verifies content hash", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    assert.ok(
      result.content_hash && result.content_hash.startsWith("sha256:"),
      "content_hash present [DECLARED]",
    );
    assert.ok(
      result.read_back_verified,
      "integrity verified after write [DECLARED]",
    );
  });
});
...
test("read-back verification succeeds", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    assert.strictEqual(
      result.read_back_verified,
      true,
      "read-back verification succeeds [DECLARED]",
    );
  });
});
...
test("never returns forbidden economic/public fields", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    for (const field of forbiddenFields) {
      assert.ok(
        !(field in result),
        `forbidden field absent: ${field} [DECLARED]`,
      );
    }
  });
});
...
test("deterministic local_writer_result_id for same semantic write result, excluding created_at", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    const r2 = await writeLocalRewardReceipt(
      { requireConsent: REWARD_RECEIPT_LOCAL_WRITER_CONSENT, demaHome },
      input,
    );
    assert.strictEqual(
      r1.local_writer_result_id,
      r2.local_writer_result_id,
      "deterministic id (excl. created_at) [DECLARED]",
    );
  });
});
...
test("file mode expectation is 0o600", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    assert.strictEqual(
      result.file_mode_expected,
      "0o600",
      "file mode expectation 0o600 [DECLARED]",
    );
  });
});
...
test("refuses proof_gaps empty", async () => {
  await withTempDemaHome(async (demaHome) => {
    ...
    assert.strictEqual(
      result.write_result_status,
      "local_write_refused_proof_gaps",
      "refuses empty proof_gaps [DECLARED]",
    );
  });
});
