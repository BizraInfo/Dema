...
test("constitution file exists and is non-empty", () => {
  assert.ok(
    constitution && constitution.length > 2000,
    "constitution document must be substantial",
  );
});
...
test("core DNA pillars and structures present", () => {
  assert.ok(
    constitution.includes("Human Mission Center"),
    "Human Mission Center required",
  );
  ...
  assert.ok(
    constitution.includes("Third Fact Proof Chain"),
    "Third Fact Proof Chain required",
  );
  assert.ok(constitution.includes("Seven Pillars"), "Seven Pillars required");
  assert.ok(constitution.includes("Ihsān"), "Ihsān required");
  assert.ok(
    constitution.includes("Agent Prohibitions"),
    "Agent Prohibitions required",
  );
  assert.ok(
    constitution.includes("Local Node0 Posture"),
    "Local Node0 Posture required",
  );
...
test("references core canon and proof ladder (ADRs, LCC-6, G-Ladder, Third Fact, DNA siblings)", () => {
  assert.ok(
    constitution.includes("ADR-033") || constitution.includes("LCC-6"),
    "LCC-6 / ADR-033 reference required",
  );
  assert.ok(
    constitution.includes("ADR-034") || constitution.includes("G-Ladder"),
    "G-Ladder / ADR-034 reference required",
  );
  assert.ok(
    constitution.includes("Third Fact") ||
      constitution.includes("BIZRA — The Third Fact"),
    "Third Fact reference required",
  );
  assert.ok(
    constitution.includes("BIZRA_AGENT_DNA_LAW_OF_ASSUMPTION") ||
      constitution.includes("DEMA_AGENT_HARNESS"),
    "sibling DNA docs reference required",
  );
  // delivery-check / spine is present in related canon but not mandatory in every sentence of this constitution; soft check
  const hasSpineRef =
    constitution.includes("delivery-check") ||
    constitution.includes("Delivery Spine") ||
    constitution.includes("LCC-6") ||
    constitution.includes("G-Ladder");
  assert.ok(
    hasSpineRef,
    "proof ladder / spine references expected via LCC-6 / G-Ladder / ADRs",
  );
});

test("declares doctrine / boundary / no-implementation posture", () => {
  assert.ok(
    constitution.includes("doctrine") ||
      constitution.includes("boundary") ||
      constitution.includes("No Implementation") ||
      constitution.includes("future slices"),
    "doctrine/boundary/no-implementation posture declaration required",
  );
});
