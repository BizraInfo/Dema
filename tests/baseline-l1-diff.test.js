...
function makeBaseline({
  ...
  return {
    ...
    source_state: {
      ...
      apps_loc: 500,
      apps_files: 1,
      schemas_declared_unique: schemas,
      cli_commands_in_help: cli_in_help,
    },
    test_state: {
      pass: tests_pass,
      fail: 0,
...
function runDiff(beforePath, afterPath) {
  const stdout = execFileSync(
    "node",
    [SCRIPT, "--files", beforePath, afterPath],
    { encoding: "utf8" },
  );
...
test("verify_before_assert_trend names tests_keep_up_or_outpace_packages when tests grow faster", () => {
  withFixtures((dir) => {
    ...
    const diff = runDiff(a, b);
    assert.equal(
      diff.verify_before_assert_trend,
...
test("verify_before_assert_trend names tests_lag_packages_significantly when packages grow faster", () => {
  withFixtures((dir) => {
    ...
    assert.equal(
      diff.verify_before_assert_trend,
      "tests_lag_packages_significantly",
    );
  });
});
