...
test("builds deterministic frozen 32-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 32);
  assert.equal(first.measured_repo_count, 32);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 32);
  assert.equal(gate.measured_repo_count, 32);
