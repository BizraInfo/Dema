...
test("builds deterministic frozen eight-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 8);
  assert.equal(first.measured_repo_count, 8);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 8);
  assert.equal(gate.measured_repo_count, 8);
