...
test("builds deterministic frozen twenty-two-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 22);
  assert.equal(first.measured_repo_count, 22);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 22);
  assert.equal(gate.measured_repo_count, 22);
