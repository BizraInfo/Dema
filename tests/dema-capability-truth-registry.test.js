...
test("builds deterministic frozen twenty-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 20);
  assert.equal(first.measured_repo_count, 20);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 20);
  assert.equal(gate.measured_repo_count, 20);
