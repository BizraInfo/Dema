...
test("builds deterministic frozen twenty-nine-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 29);
  assert.equal(first.measured_repo_count, 29);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 29);
  assert.equal(gate.measured_repo_count, 29);
