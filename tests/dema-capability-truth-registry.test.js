...
test("builds deterministic frozen twenty-seven-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 27);
  assert.equal(first.measured_repo_count, 27);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 27);
  assert.equal(gate.measured_repo_count, 27);
