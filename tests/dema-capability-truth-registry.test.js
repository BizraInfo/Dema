...
test("builds deterministic frozen thirty-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 30);
  assert.equal(first.measured_repo_count, 30);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 30);
  assert.equal(gate.measured_repo_count, 30);
