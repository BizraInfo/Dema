...
test("builds deterministic frozen seventeen-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 17);
  assert.equal(first.measured_repo_count, 17);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 17);
  assert.equal(gate.measured_repo_count, 17);
