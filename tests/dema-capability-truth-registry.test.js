...
test("builds deterministic frozen twenty-six-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 26);
  assert.equal(first.measured_repo_count, 26);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 26);
  assert.equal(gate.measured_repo_count, 26);
