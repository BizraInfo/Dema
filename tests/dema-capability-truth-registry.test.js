...
test("builds deterministic frozen 33-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 41);
  assert.equal(first.measured_repo_count, 41);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 41);
  assert.equal(gate.measured_repo_count, 41);
