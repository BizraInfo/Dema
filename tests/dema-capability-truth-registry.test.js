...
test("builds deterministic frozen twenty-five-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 25);
  assert.equal(first.measured_repo_count, 25);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 25);
  assert.equal(gate.measured_repo_count, 25);
