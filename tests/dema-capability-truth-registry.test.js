...
test("builds deterministic frozen fourteen-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 14);
  assert.equal(first.measured_repo_count, 14);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 14);
  assert.equal(gate.measured_repo_count, 14);
