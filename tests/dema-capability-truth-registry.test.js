...
test("builds deterministic frozen sixteen-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 16);
  assert.equal(first.measured_repo_count, 16);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 16);
  assert.equal(gate.measured_repo_count, 16);
