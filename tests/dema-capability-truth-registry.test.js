...
test("builds deterministic frozen six-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 6);
  assert.equal(first.measured_repo_count, 6);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 6);
  assert.equal(gate.measured_repo_count, 6);
