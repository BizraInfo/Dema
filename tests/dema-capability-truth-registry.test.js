...
test("builds deterministic frozen twenty-one-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 21);
  assert.equal(first.measured_repo_count, 21);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 21);
  assert.equal(gate.measured_repo_count, 21);
