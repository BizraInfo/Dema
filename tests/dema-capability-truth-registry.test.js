...
test("builds deterministic frozen eighteen-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 18);
  assert.equal(first.measured_repo_count, 18);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 18);
  assert.equal(gate.measured_repo_count, 18);
