...
test("builds deterministic frozen capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 71);
  assert.equal(first.measured_repo_count, 71);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 71);
  assert.equal(gate.measured_repo_count, 71);
