...
test("builds deterministic frozen twenty-eight-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 28);
  assert.equal(first.measured_repo_count, 28);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 28);
  assert.equal(gate.measured_repo_count, 28);
