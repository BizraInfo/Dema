...
test("builds deterministic frozen twenty-three-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 23);
  assert.equal(first.measured_repo_count, 23);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 23);
  assert.equal(gate.measured_repo_count, 23);
