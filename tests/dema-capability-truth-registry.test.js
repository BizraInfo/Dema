...
test("builds deterministic frozen ten-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 10);
  assert.equal(first.measured_repo_count, 10);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 10);
  assert.equal(gate.measured_repo_count, 10);
