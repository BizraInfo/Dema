...
test("builds deterministic frozen nineteen-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 19);
  assert.equal(first.measured_repo_count, 19);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 19);
  assert.equal(gate.measured_repo_count, 19);
