...
test("builds deterministic frozen 33-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 60);
  assert.equal(first.measured_repo_count, 60);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 60);
  assert.equal(gate.measured_repo_count, 60);
