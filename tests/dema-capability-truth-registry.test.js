...
test("builds deterministic frozen capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 82);
  assert.equal(first.measured_repo_count, 82);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 82);
  assert.equal(gate.measured_repo_count, 82);
