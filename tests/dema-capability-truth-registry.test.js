...
test("builds deterministic frozen capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 75);
  assert.equal(first.measured_repo_count, 75);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 75);
  assert.equal(gate.measured_repo_count, 75);
