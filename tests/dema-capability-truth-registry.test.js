...
test("builds deterministic frozen 31-capability truth registry", () => {
  ...
  assert.equal(first.capability_count, 31);
  assert.equal(first.measured_repo_count, 31);
...
test("gate passes against the real merged repository files", () => {
  ...
  assert.equal(gate.capability_count, 31);
  assert.equal(gate.measured_repo_count, 31);
