...
test("devops/release-readiness proof scope allows only release-readiness files", () => {
  ...
  assert.throws(
    () =>
      validateProofScope({
        reviewClass: "devops/release-readiness",
        files: devopsFiles.filter(
          (file) => file !== "scripts/release-readiness.mjs",
        ),
      }),
...
test("tooling/claim-ledger-checker proof scope allows only claim checker files", () => {
  ...
  assert.throws(
    () =>
      validateProofScope({
        reviewClass: "tooling/claim-ledger-checker",
        files: claimLedgerCheckerFiles.filter(
          (file) => file !== "scripts/claim-ledger-check.mjs",
        ),
      }),
    /missing required file/,
  );
...
test("u2.1/amana-kernel-contracts proof scope allows only Amana contract files", () => {
  ...
  assert.throws(
    () =>
      validateProofScope({
        reviewClass: "u2.1/amana-kernel-contracts",
        files: amanaKernelContractFiles.filter(
          (file) => file !== "packages/evidence/src/evidence-chain.js",
        ),
      }),
    /missing required file/,
  );
...
test("proof/u1 remains strict and does not accept proof-pin docs", () => {
  assert.equal(
    validateProofScope({ reviewClass: "proof/u1", files: u1Files }).ok,
    true,
  );
  assert.throws(
    () =>
...
test("MAIN-04: ci/* branches map to policy/broad-scope (parity with adr/* and fix/*)", () => {
  const out = validatePrClass({
    ...
    branch: "ci/push-trigger-and-walkdir-tests",
  });
  assert.equal(out.ok, true);
});
...
test("MAIN-05: docs/* branches map to policy/broad-scope (parity with adr/* fix/* ci/*)", () => {
  const out = validatePrClass({
    reviewClass: "policy/broad-scope",
    branch: "docs/canonize-step7-second-boundary-vocab",
  });
  assert.equal(out.ok, true);
});
...
test("MAIN-06: feat/* branches map to policy/broad-scope (parity with adr/* fix/* ci/* docs/*)", () => {
  const out = validatePrClass({
    reviewClass: "policy/broad-scope",
    branch: "feat/palette-env-resolver",
  });
  assert.equal(out.ok, true);
});
