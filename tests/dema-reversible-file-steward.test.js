...
test("T12d verify rejects a forged extra boundary key even when rehashed", async () => {
  const { sha256CanonicalJsonV1 } = await import(
    "../packages/canon/src/sha256-canonical-json-v1.js"
  );
  const payload = buildDemaReversibleFileStewardPayload(VALID);
  // Forge an extra capability flag into the all-false boundary, then recompute
  // the whole-body hash so hash_ok would pass. Only the deep-equal key-set check
  // catches it — a subset "all reference keys false" check is vacuous here.
  const { content_hash: _drop, ...body } = payload;
  const tampered = {
    ...body,
    boundary: { ...payload.boundary, live_capability_smuggled: true },
  };
  const rehashed = Object.freeze({
    ...tampered,
    content_hash: sha256CanonicalJsonV1(tampered),
  });
  const v = verifyDemaReversibleFileSteward(rehashed);
  assert.equal(v.hash_ok, true, "rehash makes the body self-consistent");
  assert.equal(v.boundary_ok, false, "extra boundary key must be rejected");
