...
test("dema explain bizra (no flag) → shows simple perspective output", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "explain", "bizra"], { env });
...
test("dema explain --simple bizra → identical to no-flag output", async () => {
  const { stdout: noFlag } = await execFileAsync("node", [cliPath, "explain", "bizra"], { env });
  const { stdout: withFlag } = await execFileAsync("node", [cliPath, "explain", "--simple", "bizra"], { env });
  assert.equal(noFlag, withFlag);
});

test("dema explain --technical bizra → shows technical perspective with canonical anchors", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "explain", "--technical", "bizra"], { env });
...
test("dema explain --arabic bizra → output contains Arabic Unicode characters", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "explain", "--arabic", "bizra"], { env });
  // Arabic Unicode block: U+0600–U+06FF
  assert.match(stdout, /[؀-ۿ]/);
});

test("dema explain --game bizra → output contains game/world/quest analogy language", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "explain", "--game", "bizra"], { env });
...
test("dema explain --all bizra → output contains all 4 perspective section markers", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "explain", "--all", "bizra"], { env });
...
test("dema explain --technical ihsan (non-seed concept) → shows 'not yet authored' hint", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "explain", "--technical", "ihsan"], { env });
...
test("dema explain --json --all bizra → JSON contains perspectives map with all 4 keys", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "explain", "--json", "--all", "bizra"], { env });
  const parsed = JSON.parse(stdout);
  assert.ok(parsed.perspectives && typeof parsed.perspectives === "object", "no perspectives field");
  assert.ok(typeof parsed.perspectives.simple === "string", "missing simple");
  assert.ok(typeof parsed.perspectives.technical === "string", "missing technical");
