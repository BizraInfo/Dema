...
test("dema onboard --preview-card --json contains would_mint_if_consented.receipt_id_preview", async () => {
  ...
  try {
    ...
    const card = JSON.parse(stdout);
    assert.ok(
      typeof card.would_mint_if_consented.receipt_id_preview === "string",
    );
    assert.match(
...
test("After running dema onboard --preview-card, preview-card show lists the written card", async () => {
  const home = await makeTmpHome();
  try {
    const { stdout: cardJson } = await runCli(
      ["onboard", "--preview-card", "--json"],
      home,
    );
    ...
    assert.ok(
      listOut.includes(hash),
      "list output must include the written card's hash",
    );
  } finally {
    await rm(home, { recursive: true, force: true });
  }
