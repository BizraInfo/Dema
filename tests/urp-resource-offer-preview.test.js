...
const modulePath = fileURLToPath(new URL("../packages/core/src/urp-resource-offer-preview.js", import.meta.url));
...
const VALID_ARGS = Object.freeze({
  ...
  no_raw_data_proof: "manifest declares only public templates and rubric metadata; no raw corpus bytes",
...
test("T-04 forbidden resource type fails closed with forbidden_resource_type", () => {
  for (const type of FORBIDDEN_RESOURCE_TYPES) {
    const env = buildUrpResourceOfferPreview({ ...VALID_ARGS, resource_type: type });
    assert.equal(env.valid, false, `forbidden type ${type} must be fail-closed`);
    assert.equal(env.denial.code, "forbidden_resource_type", `forbidden type ${type} denial.code must match`);
  }
});

test("T-05 unknown resource type fails with unknown_resource_type", () => {
  const env = buildUrpResourceOfferPreview({ ...VALID_ARGS, resource_type: "not_a_real_type" });
...
test("T-08 denied_effects MUST include write, execute, call", () => {
  for (const missing of ["write", "execute", "call"]) {
    ...
    const env = buildUrpResourceOfferPreview({ ...VALID_ARGS, denied_effects: denied });
    assert.equal(env.valid, false, `missing ${missing} from denied_effects must reject`);
    assert.equal(env.denial.code, "denied_effects_incomplete", `missing ${missing} must produce denied_effects_incomplete`);
  }
});

test("T-09 consent_field_required must be in MICRO_CONSENT_SHAPE", () => {
  const env = buildUrpResourceOfferPreview({ ...VALID_ARGS, consent_field_required: "nope" });
  assert.equal(env.valid, false);
  assert.equal(env.denial.code, "invalid_consent_field");
  // sanity: known fields all accepted
  for (const field of ["mission_id", "agent_id", "resource_id", "action", "purpose", "expires_at", "commitment_hash"]) {
    const ok = buildUrpResourceOfferPreview({ ...VALID_ARGS, consent_field_required: field });
    assert.equal(ok.valid, true, `consent field ${field} must be accepted`);
  }
});

test("T-10 sat_verdict_required must be in GateVerdict", () => {
  const env = buildUrpResourceOfferPreview({ ...VALID_ARGS, sat_verdict_required: "MAYBE" });
  assert.equal(env.valid, false);
  assert.equal(env.denial.code, "invalid_sat_verdict");
  for (const v of ["PERMIT", "REJECT", "REVIEW", "SCORE_ONLY"]) {
    const ok = buildUrpResourceOfferPreview({ ...VALID_ARGS, sat_verdict_required: v });
    assert.equal(ok.valid, true, `verdict ${v} must be accepted`);
  }
});

test("T-11 no_raw_data_proof must be ≥ 30 chars", () => {
  const env = buildUrpResourceOfferPreview({ ...VALID_ARGS, no_raw_data_proof: "too short" });
  assert.equal(env.valid, false);
...
test("T-12 carrying_cost_reference must be null or match /^chal-[0-9a-f]{32}$/", () => {
  const okNull = buildUrpResourceOfferPreview({ ...VALID_ARGS, carrying_cost_reference: null });
  ...
  assert.equal(okMatch.carrying_cost_reference, "chal-0123456789abcdef0123456789abcdef");

  const bad = buildUrpResourceOfferPreview({ ...VALID_ARGS, carrying_cost_reference: "chal-not-hex" });
...
test("T-13 owner_node rejects person-identifier heuristics (@ or :)", () => {
  for (const candidate of ["mumu@example.com", "user:123", "alice@node0", "x:y", "@handle"]) {
    const env = buildUrpResourceOfferPreview({ ...VALID_ARGS, owner_node: candidate });
    assert.equal(env.valid, false, `person-identifier ${candidate} must be rejected`);
    assert.equal(env.denial.code, "invalid_owner_node", `person-identifier ${candidate} must produce invalid_owner_node`);
  }
...
test("T-14 boundary keeps all 8 authority flags false on valid + failure envelopes", () => {
  ...
  const failureEnv = buildUrpResourceOfferPreview({ ...VALID_ARGS, resource_type: "secrets" });
...
test("T-18 pure-module imports (no fs/net/http/child_process)", async () => {
  ...
  assert.ok(!/spawn\(|execSync\(|execFile\(|spawnSync\(|fetch\(/.test(body), "module must not invoke processes or network");
});
...
test("T-20 invalid now Date rejected", () => {
  const env = buildUrpResourceOfferPreview({ ...VALID_ARGS, now: new Date("not-a-date") });
  assert.equal(env.valid, false);
