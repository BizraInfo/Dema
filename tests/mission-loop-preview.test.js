...
test("MissionLoop transitions to 'narrowing_scope' on narrow_scope", () => {
  ...
  assert.equal(
    ...
    "redraft_pat_proposal_with_narrower_scope",
  );
});
...
test("MissionLoop transitions to 'complete_preview_approved' on approve_c2_draft_only — but HOLD verdict pinned", () => {
  ...
  assert.equal(
    loop.next_safe_action,
    "preview_complete_typed_go_required_for_any_execution",
  );
  assert.equal(
    loop.preview_lifecycle_status,
    "HOLD",
...
test("MissionLoop composes the 3 primitives: state_load + profile_foundation + consent_card", () => {
  ...
  assert.equal(
    loop.profile_foundation.schema,
    "bizra.dema.profile_foundation.v0.1",
  );
  assert.equal(
    loop.consent_card.schema,
...
test("MissionLoop INVARIANT: local_model_invocation always has routing_allowed=false", () => {
  for (const decision of [
    ...
    assert.equal(
      loop.local_model_invocation.invocation_status,
...
test("MissionLoop INVARIANT: evidence_chain_event.chain_advance=false in all phases", () => {
  for (const decision of [
    ...
    assert.equal(
      loop.evidence_chain_event.chain_advance,
      false,
...
test("MissionLoop evidence_chain_event prepared only on approved completion", () => {
  const incomplete = buildMissionLoopPreview({
  ...
  assert.equal(
    approved.evidence_chain_event.schema,
    "bizra.dema.evidence_chain_event.preview.v0.1",
  );
...
test("MissionLoop receipt_preview prepared only on approved completion", () => {
  ...
  assert.equal(
    approved.receipt_preview.schema,
    "bizra.dema.mission_loop_receipt.preview.v0.1",
  );
...
test("MissionLoop ADVERSARIAL: evidence_refs raw payload is stripped via consent-card view", () => {
  ...
  assert.equal(
    loop.evidence_chain_event.referenced_evidence[0].schema,
    "bizra.dema.evidence.v0.1",
  );
  assert.equal(
    "content" in loop.evidence_chain_event.referenced_evidence[0],
    false,
  );
  assert.equal(
    "secret" in loop.evidence_chain_event.referenced_evidence[0],
    false,
  );
});
...
test("MissionLoop ADVERSARIAL: mission intent body is stripped from consent_card.mission view", () => {
  ...
  assert.equal(
    "intent" in loop.consent_card.mission,
    false,
...
test("MissionLoop is deeply frozen across all sub-views", () => {
  ...
  assert.equal(
    Object.isFrozen(loop.evidence_chain_event.referenced_evidence),
    true,
  );
...
test("MissionLoop lifecycle phase always one of the canonical 6", () => {
  ...
  phasesObserved.add(
    buildMissionLoopPreview({ mission: { missionId: "m" } }).lifecycle_phase,
  );
  phasesObserved.add(
    buildMissionLoopPreview({
      mission: { missionId: "m" },
  ...
  assert.equal(
    phasesObserved.size,
    6,
    "all 6 canonical phases must be reachable",
  );
});
...
test("MissionLoop composition preserves all embedded boundary booleans (none flipped to true)", () => {
  ...
  for (const k of Object.keys(loop.state_load.boundary)) {
    assert.equal(
      loop.state_load.boundary[k],
      false,
      `state_load.boundary.${k} flipped`,
    );
  }
  // profile_foundation boundary
  for (const k of Object.keys(loop.profile_foundation.boundary)) {
    assert.equal(
      loop.profile_foundation.boundary[k],
      false,
      `profile_foundation.boundary.${k} flipped`,
    );
  }
  // consent_card boundary
  for (const k of Object.keys(loop.consent_card.boundary)) {
    assert.equal(
      loop.consent_card.boundary[k],
      false,
      `consent_card.boundary.${k} flipped`,
    );
  }
});
