...
test("'--use-local-registry' with valid $DEMA_HOME/models/registry.json routes synthesis to operator entry", async () => {
  const home = await makeDemaHome({ withRegistry: { entries: [OPERATOR_TEST_DEMA_FACE_ENTRY] } });
...
test("'--registry-file /abs/path' with valid operator entry routes synthesis correctly", async () => {
  ...
  await writeFile(filePath, JSON.stringify({ entries: [OPERATOR_TEST_DEMA_FACE_ENTRY] }));
  const { stdout, exitCode } = await runCli(
    ["model-broker", "route", "--task", "synthesis", "--registry-file", filePath]
  );
...
test("'--registry-file relative/path' exits non-zero with 'must be absolute' error", async () => {
  const { stdout, stderr, exitCode } = await runCli(
    ["model-broker", "route", "--task", "synthesis", "--registry-file", "relative/path.json"]
  );
  assert.notEqual(exitCode, 0);
  assert.equal(stdout, "");
...
test("'--registry-file /nonexistent.json' exits non-zero with file-not-found error", async () => {
  const { stdout, stderr, exitCode } = await runCli(
    ["model-broker", "route", "--task", "synthesis", "--registry-file", "/tmp/this-file-does-not-exist-12345.json"]
  );
  assert.notEqual(exitCode, 0);
  assert.equal(stdout, "");
  assert.match(stderr, /registry file not found/);
...
test("'--registry-file <malformed.json>' exits non-zero gracefully", async () => {
  ...
  const { stdout, stderr, exitCode } = await runCli(
    ["model-broker", "route", "--task", "synthesis", "--registry-file", malformedPath]
  );
  assert.notEqual(exitCode, 0);
  assert.equal(stdout, "");
...
test("'--registry-file <oversized.json>' over 1 MB exits non-zero with size-limit error", async () => {
  ...
  const { stdout, stderr, exitCode } = await runCli(
    ["model-broker", "route", "--task", "synthesis", "--registry-file", oversizedPath]
  );
  assert.notEqual(exitCode, 0);
  assert.equal(stdout, "");
...
test("multiple registry input flags exit non-zero (3 pair combinations)", async () => {
  ...
  const r1 = await runCli(
    ["model-broker", "route", "--task", "synthesis", "--registry-file", "/tmp/x.json", "--registry-stdin"],
    { stdin: "{}" }
  );
  ...
  const r2 = await runCli(
    ["model-broker", "route", "--task", "synthesis", "--use-local-registry", "--registry-stdin"],
    { stdin: "{}" }
  );
  ...
  const r3 = await runCli(
    ["model-broker", "route", "--task", "synthesis", "--registry-file", "/tmp/x.json", "--use-local-registry"]
  );
