...
test("8 artifact saying URP is DESIGNED_NOT_LIVE passes", () => {
  const result = evaluateArtifactSafety("URP shared runtime is DESIGNED_NOT_LIVE per canon.");
  assert.equal(result.verdict, "PUBLIC_SAFE");
});
...
test("11 CLI returns non-zero on unsafe artifact", async () => {
  ...
  await writeFile(path, JSON.stringify({ leak: "/home/operator/secret" }), "utf8");
...
test("12 CLI returns zero on safe artifact", async () => {
  ...
  const { stdout } = await execFileAsync("node", [scriptPath, "--artifact", path, "--json"]);
