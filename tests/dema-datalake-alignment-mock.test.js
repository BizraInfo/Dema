...
test('creates a local alignment envelope with sha256 alignment_boundary_id', () => {
  ...
  const env = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  assert.ok(env.alignment_boundary_id && env.alignment_boundary_id.startsWith('sha256:'), 'creates envelope with sha256 alignment_boundary_id [DECLARED]');
  assert.strictEqual(env.face_body_alignment_status, 'REFERENCE_EXPECTATION_ONLY', 'face/body status REFERENCE_EXPECTATION_ONLY [DECLARED]');
});

// 2. requires exact consent
test('requires exact consent', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  assert.throws(() => createMockDemaDataLakeAlignment({ requireConsent: 'WRONG' }, input), /CONSENT_REQUIRED/, 'rejects missing exact consent [DECLARED]');
});
...
test('requires dema and datalake artifact refs', () => {
  ...
  assert.throws(() => createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, bad), /dema_artifact_ref/, 'requires dema_artifact_ref [DECLARED]');
});
...
test('declares Dema face artifact reference boundary', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  const env = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  assert.ok(env.dema_ref && env.dema_ref.startsWith('sha256:'), 'includes dema_ref [DECLARED]');
});
...
test('declares Data Lake body artifact reference boundary', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  const env = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  assert.ok(env.datalake_ref && env.datalake_ref.includes('datalake'), 'includes datalake_ref [DECLARED]');
});
...
test('declares face body alignment status boundary as REFERENCE_EXPECTATION_ONLY', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  const env = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  assert.strictEqual(env.face_body_alignment_status, 'REFERENCE_EXPECTATION_ONLY', 'face/body alignment status boundary [DECLARED]');
});
...
test('declares PAT-7 expectation boundary without runtime', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  const env = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  const hasPat = env.pat7_expectation && env.pat7_expectation.placeholder === true && env.pat7_expectation.runtime_implemented === false;
  assert.ok(hasPat, 'PAT-7 expectation boundary scaffold without runtime [DECLARED]');
});
...
test('declares SAT-5 expectation boundary without runtime', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  const env = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  const hasSat = env.sat5_expectation && env.sat5_expectation.placeholder === true && env.sat5_expectation.runtime_implemented === false;
  assert.ok(hasSat, 'SAT-5 expectation boundary scaffold without runtime [DECLARED]');
});
...
test('declares FATE expectation boundary without runtime', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  const env = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  const hasFate = env.fate_expectation && env.fate_expectation.placeholder === true && env.fate_expectation.runtime_implemented === false;
  assert.ok(hasFate, 'FATE expectation boundary scaffold without runtime [DECLARED]');
});
...
test('declares URP expectation non-claim boundary', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  const env = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  const hasUrp = env.urp_expectation && env.urp_expectation.placeholder === true && env.urp_expectation.urp_sync_implemented === false && env.urp_expectation.public_publication === false;
...
test('includes allowed alignment input output envelope and proof gaps', () => {
  ...
  const hasPosture = env.prototype_posture && env.prototype_posture.includes('PROTOTYPE');
  assert.ok(hasGaps && hasPosture, 'allowed alignment input output envelope boundary + proof gaps [DECLARED]');
});
...
test('rejects forbidden sync mutation bridge runtime rejection boundary', () => {
  ...
  assert.throws(() => createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, bad), /FORBIDDEN/, 'forbidden sync/mutation/bridge/runtime rejection [DECLARED]');
});
...
test('deterministic alignment_boundary_id and proof-gap invariant boundary', () => {
  const input = loadExampleDemaDataLakeAlignmentInput();
  const r1 = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  const r2 = createMockDemaDataLakeAlignment({ requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT }, input);
  assert.strictEqual(r1.alignment_boundary_id, r2.alignment_boundary_id, 'deterministic (excl created_at) [DECLARED]');
  const hasNoForbidden = !Object.keys(r1).some(k => k.includes('sync') || k.includes('bridge') || k.includes('mint') || k.includes('token'));
  assert.ok(hasNoForbidden && Array.isArray(r1.proof_gaps) && r1.proof_gaps.length > 0, 'proof-gap and still-blocked invariant boundary [DECLARED]');
});
