...
test("`dema eval layer2 prompts --json` exits 0 + emits rubric pack envelope", async () => {
  const { stdout } = await execFileAsync(
    ...
    { encoding: "utf8", env: { ...process.env, ...CLI_ENV }, maxBuffer: 8 * 1024 * 1024 }
  );
  ...
  assert.deepEqual(ids, ["actionability", "boundary_compliance", "truthfulness"]);
});
...
test("`dema eval layer2 verify` without path → exit 1 with helpful message", async () => {
  await assert.rejects(
    () =>
      execFileAsync(
        "node",
        [indexPath, "eval", "layer2", "verify"],
        { encoding: "utf8", env: { ...process.env, ...CLI_ENV }, maxBuffer: 4 * 1024 * 1024 }
      ),
    (err) => {
      assert.equal(err.code, 1);
...
test("`dema eval layer2 verify` on missing file → exit 1 with read-error message", async () => {
  await assert.rejects(
    () =>
      execFileAsync(
        "node",
        [indexPath, "eval", "layer2", "verify", "/nonexistent/path/to/verdict.json"],
        { encoding: "utf8", env: { ...process.env, ...CLI_ENV }, maxBuffer: 4 * 1024 * 1024 }
      ),
    (err) => {
      assert.equal(err.code, 1);
