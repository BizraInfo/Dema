...
const scriptPath = fileURLToPath(new URL("../scripts/review/non-generic-vocabulary-check.mjs", import.meta.url));

test("non-generic-vocabulary check passes on current user-facing surface", () => {
  const report = buildNonGenericVocabularyCheckReport();
  assert.equal(report.schema, "bizra.dema.review.non_generic_vocabulary_check.v0.1");
  assert.equal(report.mode, "READ_ONLY_AUDIT");
  assert.equal(report.ok, true);
  assert.ok(report.files_scanned > 0, "must discover at least one file to scan");
...
test("forbidden_phrases matches the UX Proof Harness criterion L verbatim", () => {
  ...
  assert.deepEqual([...report.forbidden_phrases].sort(), expected.slice().sort());
});
...
`.trim();
  ...
  assert.deepEqual(phrases, ["AI employee", "autonomous magic", "prompt runner"]);
});
