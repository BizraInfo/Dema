...
const scriptPath = fileURLToPath(new URL("../scripts/urp-shared-discovery.mjs", import.meta.url));
...
test("buildUrpSharedStateManifest is discovery-only with UKE not connected", () => {
  ...
  assert.equal(manifest.manifest_relative_path, URP_SHARED_MANIFEST_RELATIVE_PATH);
...
test("evaluateUrpSharedWriteBoundary refuses forbidden write kinds", () => {
  for (const kind of FORBIDDEN_WRITE_KINDS) {
    ...
    assert.ok(result.violations.some((v) => v.code === "forbidden_write_kind" || v.code === "discovery_only_no_persist"));
  }
});
...
test("evaluateUrpSharedWriteBoundary requires SAT pipeline_verified", () => {
  const result = evaluateUrpSharedWriteBoundary({
    ...
    candidate: { candidate_id: "x", tier: "URP_SHAREABLE", contains_private_pat_memory: false },
    sat_pipeline: { schema: "bizra.dema.orchestrator_verification_pipeline.v0.1", overall_verdict: "pipeline_violated", passed: false },
    consent_phrase: "GO: share x into URP soil",
    discovery_only: true
  });
  assert.equal(result.allowed, false);
  assert.ok(result.violations.some((v) => v.code === "sat_pipeline_not_verified"));
});

test("evaluateUrpSharedWriteBoundary blocks PAT private memory export", () => {
  const result = evaluateUrpSharedWriteBoundary({
    write_kind: "manifest_append_entry",
    candidate: { candidate_id: "leak", tier: "URP_SHAREABLE", contains_private_pat_memory: true },
    sat_pipeline: verifiedPipeline(),
    consent_phrase: "GO: share leak into URP soil",
    discovery_only: true
  });
  assert.equal(result.allowed, false);
  assert.ok(result.violations.some((v) => v.code === "pat_private_memory_blocked"));
});

test("evaluateUrpSharedWriteBoundary always refuses persist in discovery_only mode", () => {
  const result = evaluateUrpSharedWriteBoundary({
    write_kind: "manifest_append_entry",
    candidate: { candidate_id: "ok-1", tier: "URP_SHAREABLE", contains_private_pat_memory: false },
    sat_pipeline: verifiedPipeline(),
    consent_phrase: "GO: share ok-1 into URP soil",
    discovery_only: true
  });
  assert.equal(result.allowed, false);
  assert.ok(result.violations.some((v) => v.code === "discovery_only_no_persist"));
...
test("buildUrpSharedRuntimeDiscovery composes manifest and boundary sample", () => {
  const report = buildUrpSharedRuntimeDiscovery({ sat_pipeline: verifiedPipeline() });
  ...
  assert.equal(report.urp_discovery_flags.shared_urp_network_publish_performed, false);
  assert.ok(Object.isFrozen(report));
});

test("urp-shared-discovery script exits 0 with --json", async () => {
  const { stdout } = await execFileAsync(process.execPath, [scriptPath, "--json"], { encoding: "utf8" });
