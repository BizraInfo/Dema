...
test("buildDiagnosticsMissionPlan names the proactive checks and consent requirements", () => {
  ...
  assert.ok(
    plan.checks.some(
      (check) => check.command === "node scripts/node0-self-check.mjs --verify",
    ),
  );
  assert.ok(
    plan.consent_scope_preview.permissions.some(
      (permission) =>
...
test("buildDiagnosticsMissionPlan exposes micro harness compliance without authorization", () => {
  ...
  assert.equal(
    plan.proactive_harness.mode,
    "DETERMINISTIC_DIAGNOSTICS_POLICY_PREVIEW",
  );
  assert.equal(
    plan.proactive_harness.recommended_micro_action,
    "narrow_diagnostics_scope_then_request_exact_consent",
  );
  assert.equal(
    plan.proactive_harness.gates.find(
      (gate) => gate.gate === "all_effecting_checks_require_consent",
    ).pass,
    true,
  );
  assert.equal(
    plan.proactive_harness.gates.find(
      (gate) => gate.gate === "preview_boundary_closed",
    ).pass,
    true,
  );
  ...
  assert.equal(
    plan.micro_compliance.all_effecting_checks_require_consent,
    true,
  );
  assert.equal(plan.micro_compliance.no_policy_contradiction, true);
  assert.equal(
    plan.micro_consent.preview_scope,
    "diagnostics_plan_preview_only",
  );
  assert.equal(plan.micro_consent.status, "draft_only");
  assert.equal(
    plan.micro_consent.exact_consent_required_for_effecting_checks,
    true,
  );
  ...
  assert.equal(
    plan.self_critique.open_risk_count,
    plan.self_critique.gaps.length,
  );
});

test("formatDiagnosticsMissionPlan renders phases, critique, proof, and boundary", () => {
  const output = formatDiagnosticsMissionPlan(
    buildDiagnosticsMissionPlan({ now: fixedNow }),
  );
