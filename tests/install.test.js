...
test("install.sh --help describes unified installer + seed-pattern invariant", async () => {
  const { stdout } = await execFileAsync("bash", [INSTALL_SCRIPT, "--help"], {
    cwd: REPO_ROOT,
  });
...
test("install.sh candidate path REQUIRES paired-receipt-id and paired-receipt-hash", async () => {
  try {
    ...
    assert.match(
      err.stderr,
      /paired-receipt-id and --paired-receipt-hash required/,
    );
  }
});
...
test("install.sh --check reports state without writing", async () => {
  ...
  const { stdout } = await execFileAsync("bash", [INSTALL_SCRIPT, "--check"], {
    cwd: REPO_ROOT,
    env: { ...process.env, DEMA_HOME: safeHome },
  });
...
test("ADVERSARIAL: install.sh dry-run with custom DEMA_HOME does NOT create directory", async () => {
  ...
  await execFileAsync("bash", [INSTALL_SCRIPT, "--dry-run"], {
    cwd: REPO_ROOT,
    env: { ...process.env, DEMA_HOME: safeHome },
  });
  assert.equal(
...
test("install-unix.sh wrapper exists and delegates to install.sh", async () => {
  ...
  const { stdout } = await execFileAsync(
    "bash",
    [INSTALL_UNIX_SHIM, "--help"],
    { cwd: REPO_ROOT },
  );
  assert.match(
    stdout,
    /Unified Dema installer/,
    "wrapper must pass through to install.sh",
  );
});
...
test("install-unix.sh wrapper --dry-run produces same output as install.sh --dry-run", async () => {
  ...
  const a = await execFileAsync("bash", [INSTALL_UNIX_SHIM, "--dry-run"], {
    cwd: REPO_ROOT,
    env,
  });
  const b = await execFileAsync("bash", [INSTALL_SCRIPT, "--dry-run"], {
    cwd: REPO_ROOT,
    env,
  });
  assert.equal(
    a.stdout,
    b.stdout,
    "wrapper must produce byte-equal output to canonical script",
  );
});
