...
test("transactional: a later-atom failure auto-rolls-back the applied atoms to genesis", (t) => {
  const root = makeSandbox({ "a.txt": "aa", "b.txt": "bb" }); // no "ghost.txt"
  t.after(() => rmSync(root, { recursive: true, force: true }));

  // atom 0 (a.txt -> a.done.txt) succeeds; atom 1 (ghost.txt) fails. All-or-nothing
  // means atom 0 must be reversed — the sandbox returns to genesis, no partial state.
  const exec = sequenceExecuteStewardJob({
    sandboxRoot: root,
    atoms: [{ from: "a.txt", to: "a.done.txt" }, { from: "ghost.txt", to: "x.txt" }],
    consent: GO,
    fs,
    now: NOW,
  });
  ...
  assert.equal(exec.ok, false);
  assert.equal(exec.stopped_at.index, 1, "failed at atom 1");
  assert.equal(exec.rolled_back, true, "the applied atom must be reversed");
  assert.equal(exec.executed_count, 0, "net effect is zero after rollback");
  assert.equal(exec.applied_before_stop, 1, "one atom had been applied before the stop");
  // The whole user-file set is back at genesis — a.txt restored, a.done.txt gone.
  assert.deepEqual(measureStewardDirState({ fs, sandboxRoot: root }).files, ["a.txt", "b.txt"]);
});
