...
describe("rotateAuthorshipKey — contract", () => {
  ...
  it("activates self-consistent NEW pair via generation archive; no tmp leftovers", async () => {
    ...
    assert.equal(active.fingerprint, r.new_fingerprint);
    assert.equal(active.previous_generation, oldFp);
  });
  ...
  it("binds a FULL consent envelope + nonce (not the phrase); receipt carries no private material", async () => {
    ...
    assert.equal(
      readFileSync(r.receipt_path, "utf8").includes("PRIVATE KEY"),
      false,
    );
  });
  ...
  it("phrase-only consent (no envelope) BLOCKS mutation — no key/journal/generation written", async () => {
    ...
    const oldFp = await seedKey(home);
    const oldPriv = await loadPrivateKey(home);
    const gensBefore = readdirSync(join(keyPaths(home).dir, "generations")).sort();
    ...
    // Main's init already creates generations/ — assert rotate added nothing.
    assert.deepEqual(
      readdirSync(join(keyPaths(home).dir, "generations")).sort(),
      gensBefore,
    );
    ...
    const active = await loadActiveKeyPair(home);
    assert.equal(active.ok, true);
    assert.equal(active.fingerprint, oldFp);
  });
...
describe("rotateAuthorshipKey — failure injection (each ends in ONE explicit state)", () => {
  it("replayed consent nonce → refused, active key OLD_ACTIVE_UNCHANGED", async () => {
    ...
    assert.equal(await loadPrivateKey(home), afterFirst);
  });
  ...
  it("corrupt retirement registry → fail closed UPFRONT, old key unchanged", async () => {
    ...
    const active = await loadActiveKeyPair(home);
    assert.equal(active.ok, true);
    const oldPriv = active.private_key_pem;
    const genPrivPath = join(active.generation_path, "private.pem");
    writeFileSync(join(keyPaths(home).dir, "retired-registry.json"), "{ not json");
    const r = await rotate(home);
    assert.equal(r.rotated, false);
    assert.equal(r.error, "retired_registry_corrupt");
    // Flat privateKey does not exist on main's init — verify generation bytes.
    assert.equal(readFileSync(genPrivPath, "utf8"), oldPriv);
  });
  ...
  it("symlink substitution of the active key → fail closed, symlink target unchanged", async () => {
    ...
    const active = await loadActiveKeyPair(home);
    assert.equal(active.ok, true);
    const genPriv = join(active.generation_path, "private.pem");
    const realTarget = readFileSync(genPriv, "utf8");
    const decoy = join(keyPaths(home).dir, "decoy.pem");
    writeFileSync(decoy, realTarget);
    rmSync(genPriv);
    symlinkSync(decoy, genPriv);
    const r = await rotate(home);
    assert.equal(r.rotated, false);
    assert.equal(readFileSync(decoy, "utf8"), realTarget);
  });
  ...
  it("retired active fingerprint → HOT-PATH loader fails closed (never serves a retired key)", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        schema: "bizra.dema.retired_key_registry.v0.1",
        retired: [{ fingerprint: fp }],
      }),
    );
...
describe("rotateAuthorshipKey — branch coverage of failure/edge paths", () => {
  it("corrupt nonce ledger → nonce_ledger_unreadable (fail closed, old key unchanged)", async () => {
    const home = freshHome();
    const oldPriv = await (async () => {
      await seedKey(home);
      return loadPrivateKey(home);
    })();
  ...
  it("generation archive dir un-creatable → generation_archive_failed, old key unchanged", async () => {
    ...
    // Main's init already created generations/ as a directory — replace it
    // with a file so mkdir(generations/<fp>) / writeGeneration fails.
    rmSync(join(keyPaths(home).dir, "generations"), { recursive: true });
    writeFileSync(join(keyPaths(home).dir, "generations"), "not a dir");
    // Pointer still points at the removed generation → loader fails closed;
    // rotate must refuse without silent re-init. Prefer generation_archive_failed
    // when a loadable active pair existed; after nuking generations the active
    // pair is gone, so no_key_to_rotate is also admissible for this model.
    const r = await rotate(home);
    assert.equal(r.rotated, false);
    assert.ok(
      r.error === "generation_archive_failed" || r.error === "no_key_to_rotate",
      `unexpected error ${r.error}`,
    );
    // After destroying generations the hot path cannot serve the old key.
    assert.equal(await loadPrivateKey(home), null);
    void oldPriv;
  });
