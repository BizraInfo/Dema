...
test("buildAmbientBoundary declares ambient awareness without execution", () => {
  ...
  assert.equal(
    boundary.proof_of_truth.economic.status,
    "closed_until_verified_impact",
  );
});
...
test("buildAmbientAuditPreview captures SNR, SAPE, HHMM, and proof convergence without effects", () => {
  ...
  assert.equal(
    audit.hidden_flow_pattern,
    "intent -> micro_consent -> capability -> effect -> evidence -> impact",
  );
  assert.deepEqual(
    audit.sape_lenses.map((lens) => lens.id),
    ["security", "architecture", "performance", "ethics"],
  );
  ...
  assert.deepEqual(
    audit.agent_topology.sat.roles.map((role) => role.id),
    [
      "SAT-Orchestrator",
      "SAT-Policy",
      "SAT-QualityOps",
  ...
  assert.equal(
    audit.agent_topology.boundary,
    "SAT are not cloud PAT and do not live inside user nodes",
  );
  assert.equal(
    audit.agent_topology.invariant,
    "PAT may want success. SAT must require truth.",
  );
  assert.equal(
    audit.agent_topology.access.user_can_directly_command_sat,
    false,
  );
  assert.equal(
    audit.agent_topology.access.sat_reads_raw_private_pat_memory_by_default,
    false,
  );
  assert.equal(
    audit.agent_topology.access.urp_receives_raw_private_data,
    false,
  );
  assert.equal(
    audit.agent_topology.access.imp_from_pat_self_certification,
    false,
  );
...
test("formatAmbientAuditPreview renders the compliance spine and next implementation step", () => {
  ...
  assert.match(
    output,
    /SNR signal: EffectCap is the only legal side-effect path/,
  );
  assert.match(
    output,
    /Hidden flow: intent -> micro_consent -> capability -> effect -> evidence -> impact/,
  );
  assert.match(
    output,
    /PAT-7: local, user-aligned, user-node-only mission party/,
  );
  assert.match(output, /SAT-5: system-owned, system-aligned URP control plane/);
  assert.match(
    output,
    /SAT-Orchestrator: Global task sharding, mission routing, RSI scheduling/,
  );
...
test("dema ambient audit --json emits a schema-tagged non-executing audit", async () => {
  ...
  assert.equal(audit.agent_topology.sat.residence, "urp_control_plane_only");
  assert.equal(
    audit.agent_topology.ux_copy.pat,
    "Your PAT agents help shape and later execute your local mission.",
  );
  assert.equal(
    audit.agent_topology.ux_copy.sat,
    "SAT/URP validation is system-side and only applies after evidence or receipt handoff.",
  );
  assert.equal(audit.boundary.execution_enabled, false);
  assert.equal(
    audit.proof_of_truth.economic.status,
    "closed_until_verified_impact",
  );
});
