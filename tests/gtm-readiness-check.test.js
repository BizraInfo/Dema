...
test("buildGtmReadinessReport verifies current GTM docs and Lighthouse pack boundaries", async () => {
  ...
  try {
    ...
    assert.ok(report.open_operator_gates.every((gate) => gate.status.startsWith("open_")));
    ...
    assert.ok(report.checks.some((check) => check.name === "open_operator_gates_declared" && check.ok));
    assert.ok(report.checks.some((check) => check.name === "phase_milestone_gates_declared" && check.ok));
  } finally {
...
test("buildGtmReadinessReport exposes exact open operator gates without failing readiness", async () => {
  ...
  try {
    const report = await buildGtmReadinessReport({ lighthousePackDir: packDir });
    const gatesById = new Map(report.open_operator_gates.map((gate) => [gate.id, gate]));

    assert.equal(report.ok, true);
    assert.equal(gatesById.get("send_lighthouse_pack_ring1").phrase, "GO send pack to <name>");
    assert.equal(gatesById.get("author_poi_v0_1_test_plan").phrase, "GO author POI v0.1 test plan (no impl)");
    assert.equal(gatesById.get("sync_adr_013_status").phrase, "GO sync ADR-013 status to Accepted");
    ...
    assert.ok(report.open_operator_gates.every((gate) => gate.boundary_effect_performed === false));
    assert.deepEqual(report.phase_status.map((phase) => phase.id), ["phase_1", "phase_2", "phase_3"]);
    assert.equal(report.phase_status[0].status, "open_operator_and_external_evidence_required");
...
test("buildGtmReadinessReport scans missing Phase 1 evidence without failing readiness", async () => {
  ...
  try {
    const report = await buildGtmReadinessReport({ lighthousePackDir: packDir, demaHome });

    assert.equal(report.ok, true);
    assert.equal(report.phase1_evidence.status, "waiting_for_operator_send_and_reviewer_feedback");
    ...
    assert.equal(report.phase1_evidence.boundary.reviewer_identity_published, false);
    assert.match(report.phase1_evidence.next_safe_action, /GO send pack to <name>/);
    assert.ok(report.checks.some((check) => check.name === "phase1_evidence_scanned" && check.ok));
  } finally {
    await rm(packDir, { recursive: true, force: true });
...
test("buildGtmReadinessReport counts Phase 1 evidence metadata without reading private content", async () => {
  ...
  try {
    ...
    assert.equal(report.ok, true);
    assert.equal(report.phase1_evidence.status, "evidence_present_ready_for_operator_review");
    ...
    assert.deepEqual(report.phase1_evidence.send_receipts.map((entry) => entry.name), ["2026-05-22-ring1-001.md"]);
    assert.deepEqual(report.phase1_evidence.feedback_documents.map((entry) => entry.name), ["ring1-001-2026-Q2.md"]);
    assert.equal("content" in report.phase1_evidence.send_receipts[0], false);
    assert.equal("content" in report.phase1_evidence.feedback_documents[0], false);
    assert.match(report.phase1_evidence.next_safe_action, /GO author amendment ADR from <finding>/);
  } finally {
    await rm(packDir, { recursive: true, force: true });
    await rm(demaHome, { recursive: true, force: true });
  }
...
test("buildGtmReadinessReport exposes Phase 1 success criteria ledger", async () => {
  ...
  try {
    const report = await buildGtmReadinessReport({ lighthousePackDir: packDir, demaHome });
    const criteriaById = new Map(report.phase1_success_criteria.criteria.map((criterion) => [criterion.id, criterion]));

    assert.equal(report.phase1_success_criteria.schema, "bizra.dema.gtm.phase1_success_criteria.v0.1");
    ...
    assert.equal(criteriaById.get("ring1_feedback_on_record").status, "open_external_evidence_required");
    assert.equal(criteriaById.get("poi_gate_1_ring1_feedback_closed").status, "blocked_until_send_receipt_and_feedback_exist");
    assert.equal(criteriaById.get("poi_gate_4_test_plan_closed").exact_phrase, "GO author POI v0.1 test plan (no impl)");
    assert.equal(criteriaById.get("phase1_close_receipt_recorded").exact_phrase, "GO mint phase-1-close");
    assert.equal(criteriaById.get("reviewer_surprising_finding_memory").boundary_effect_performed, false);
    assert.ok(report.checks.some((check) => check.name === "phase1_success_criteria_tracked" && check.ok));
  } finally {
    await rm(packDir, { recursive: true, force: true });
    await rm(demaHome, { recursive: true, force: true });
  }
...
test("buildGtmReadinessReport updates Phase 1 success criteria from evidence metadata", async () => {
  ...
  try {
    ...
    await mkdir(sendDir, { recursive: true });
    await mkdir(feedbackDir, { recursive: true });
    await writeFile(join(sendDir, "2026-05-22-ring1-001.md"), "send metadata only\n", "utf8");
    await writeFile(join(feedbackDir, "ring1-001-2026-Q2.md"), "private feedback content\n", "utf8");
    ...
    assert.equal(criteriaById.get("ring1_feedback_on_record").status, "satisfied");
    assert.equal(criteriaById.get("ring1_feedback_on_record").evidence[0], "1 private feedback document(s) counted metadata-only");
    assert.equal(criteriaById.get("poi_gate_1_ring1_feedback_closed").status, "evidence_present_operator_review_required");
    assert.equal(criteriaById.get("poi_gate_1_ring1_feedback_closed").satisfied, false);
  } finally {
    await rm(packDir, { recursive: true, force: true });
    await rm(demaHome, { recursive: true, force: true });
  }
...
test("formatGtmReadinessReport renders concise human output", async () => {
  ...
  try {
    ...
    assert.match(output, /phase_1: open_operator_and_external_evidence_required/);
    ...
    assert.match(output, /ring1_feedback_on_record: open_external_evidence_required/);
