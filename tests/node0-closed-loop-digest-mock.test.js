...
test('creates a local Node0 digest envelope with sha256 node0_digest_boundary_id', () => {
  ...
  const env = createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, input);
  assert.ok(env.node0_digest_boundary_id && env.node0_digest_boundary_id.startsWith('sha256:'), 'creates envelope with sha256 node0_digest_boundary_id [DECLARED]');
});

// 2. requires exact consent
test('requires exact consent', () => {
  const input = loadExampleNode0ClosedLoopDigestInput();
  assert.throws(() => createMockNode0ClosedLoopDigest({ requireConsent: 'WRONG' }, input), /CONSENT_REQUIRED/, 'rejects missing exact consent [DECLARED]');
});
...
test('requires digest_scope', () => {
  ...
  assert.throws(() => createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad), /digest_scope/, 'requires digest_scope [DECLARED]');
});
...
test('requires receipt_review_id sha256 reference', () => {
  ...
  assert.throws(() => createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad), /receipt_review_id/, 'requires receipt_review_id sha256: [DECLARED]');
});
...
test('requires local_writer_result_id sha256 reference', () => {
  ...
  assert.throws(() => createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad), /local_writer_result_id/, 'requires local_writer_result_id sha256: [DECLARED]');
});
...
test('requires air_id sha256 reference', () => {
  ...
  assert.throws(() => createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad), /air_id/, 'requires air_id sha256: [DECLARED]');
});
...
test('requires mission_state_id sha256 reference', () => {
  ...
  assert.throws(() => createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad), /mission_state_id/, 'requires mission_state_id sha256: [DECLARED]');
});
...
test('requires alignment_boundary_id and hybrid_knowledge_boundary_id sha256 references', () => {
  ...
  assert.throws(() => createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad), /alignment_boundary_id/, 'requires alignment_boundary_id and hybrid_knowledge_boundary_id sha256: [DECLARED]');
});
...
test('declares proof-chain expectation without digest runtime, writer, aggregator, or closed-loop execution', () => {
  const input = loadExampleNode0ClosedLoopDigestInput();
  const env = createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, input);
  const hasChain = env.proof_chain_expectation &&
  ...
  assert.ok(hasChain, 'declares proof-chain expectation without runtime/writer/aggregator/closed-loop [DECLARED]');
});
...
test('rejects forbidden digest/runtime/economic/public fields', () => {
  ...
  assert.throws(() => createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad), /FORBIDDEN/, 'rejects forbidden fields [DECLARED]');
});
...
test('deterministic node0_digest_boundary_id for same semantic input excluding created_at', () => {
  const input = loadExampleNode0ClosedLoopDigestInput();
  const r1 = createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, input);
  const r2 = createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, input);
  assert.strictEqual(r1.node0_digest_boundary_id, r2.node0_digest_boundary_id, 'deterministic node0_digest_boundary_id (excl created_at) [DECLARED]');
});
