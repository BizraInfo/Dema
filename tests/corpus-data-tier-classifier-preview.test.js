...
const modulePath = new URL(
  "../packages/core/src/corpus-data-tier-classifier-preview.js",
  import.meta.url,
);
...
test("buildCorpusDataTierClassifierPreview emits schema-tagged D0-D4 metadata classifications", () => {
  ...
  const byId = new Map(
    preview.classifications.map((classification) => [
...
test("classifier allows D0-D2 metadata design and quarantines D3-D4 metadata", () => {
  const preview = buildCorpusDataTierClassifierPreview();
  const allowed = preview.classifications.filter(
    (classification) => classification.allowed_in_preview,
  );
  const quarantined = preview.classifications.filter(
    (classification) => !classification.allowed_in_preview,
  );

  assert.deepEqual(
    allowed.map((classification) => classification.tier),
    ["D0", "D1", "D2"],
  );
  assert.deepEqual(
    quarantined.map((classification) => classification.tier),
    ["D3", "D4"],
  );
  assert.equal(
    quarantined[0].disposition,
    "quarantine_private_strategy_metadata",
  );
  assert.equal(
    quarantined[1].disposition,
    "quarantine_secret_identity_or_high_sensitivity_metadata",
  );
...
test("mixed metadata signals classify by highest-risk declared tier", () => {
  ...
  assert.deepEqual(preview.classifications[0].matched_signals, [
    "architecture",
    "credential",
    "documentation",
  ]);
...
test("malformed or raw-content inputs fail closed without echoing observed text", () => {
  ...
  assert.doesNotMatch(
    JSON.stringify(raw),
    /raw-secret-transcript-should-not-appear/,
  );
...
test("preview emits self-proactive harness, self-critique, micro-compliance, and micro-consent", () => {
  ...
  assert.equal(
    ...
    "DETERMINISTIC_DATA_TIER_CLASSIFIER_PREVIEW",
  );
  assert.equal(
    preview.self_proactive_harness.recommended_micro_action,
    "build_corpus_redaction_fixture_preview",
  );
  assert.equal(
    preview.self_proactive_harness.gates.find(
      (gate) => gate.gate === "metadata_only_inputs",
    ).pass,
    true,
  );
  ...
  assert.equal(
    ...
    "color_label_not_container_opening",
  );
});
