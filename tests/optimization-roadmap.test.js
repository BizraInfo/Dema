...
const modulePath = fileURLToPath(new URL("../packages/core/src/optimization-roadmap.js", import.meta.url));
...
test("buildOptimizationRoadmapPreview keeps proof pillars canonical and non-certifying", () => {
  ...
  assert.ok(Object.values(report.proof_of_truth_convergence).every((pillar) => (
...
test("roadmap items are advisory, ordered, and dependency-safe", () => {
  ...
  const edgeKeys = new Set(report.risk_graph.edges.map((edge) => `${edge.from}->${edge.to}`));

  assert.equal(ids.size, report.roadmap_items.length);
  assert.deepEqual(report.roadmap_items.map((item) => item.priority), [1, 2, 3, 4, 5, 6, 7, 8]);
  assert.ok(report.roadmap_items.every((item) => item.effect_class === "advisory_only"));
  assert.ok(report.roadmap_items.every((item) => !/\b(implement|deploy|execute|mint|federate)\b/i.test(item.title)));
  assert.ok(report.roadmap_items.every((item) => item.blueprint_focus));
  assert.ok(report.roadmap_items.every((item) => item.management_knowledge_area));
  ...
  assert.equal(
    ...
    report.roadmap_items.reduce((count, item) => count + item.depends_on.length, 0)
  );
...
test("roadmap lenses and gates remain evidence-tagged and non-enforcing", () => {
  const report = buildOptimizationRoadmapPreview();
  const allowedGateStatuses = new Set(["already_local_practice", "advisory_read_only", "not_enforced"]);

  assert.ok(report.pmbok_lenses.every((lens) => lens.canon_source));
  assert.ok(report.management_body_of_knowledge.every((lens) => lens.canon_source));
  assert.ok(report.snr_lenses.every((lens) => lens.canon_source));
  assert.ok(report.sape_lenses.every((lens) => lens.canon_source));
  assert.ok(report.proposed_gates.some((gate) => gate.status === "not_enforced"));
  assert.ok(report.proposed_gates.every((gate) => allowedGateStatuses.has(gate.status)));
  assert.ok(report.proposed_gates.every((gate) => gate.enforcement_owner));
  assert.ok(report.proposed_gates.every((gate) => gate.effect_class === "advisory_only"));
...
test("roadmap covers CI/CD, pipeline automation, quality, performance, and ethics without effects", () => {
  const report = buildOptimizationRoadmapPreview();
  const blueprintDomains = new Set(report.blueprint_coverage.map((entry) => entry.domain));
  const itemWorkstreams = new Set(report.roadmap_items.map((item) => item.workstream));
  const itemBlueprints = new Set(report.roadmap_items.map((item) => item.blueprint_focus));
  const gateBlueprints = new Set(report.proposed_gates.map((gate) => gate.blueprint_domain));
  ...
  for (const domain of [
    ...
    assert.ok(blueprintDomains.has(domain), `${domain} missing from blueprint coverage`);
    assert.ok(itemBlueprints.has(domain), `${domain} missing from roadmap items`);
    assert.ok(gateBlueprints.has(domain) || domain === "devops", `${domain} missing from proposed gates`);
  }
  ...
  assert.ok(report.blueprint_coverage.every((entry) => entry.non_goal.startsWith("no ")));
});
...
test("formatOptimizationRoadmapPreview renders lenses, gates, proof, and boundary", () => {
  const output = formatOptimizationRoadmapPreview(buildOptimizationRoadmapPreview());
  ...
  assert.match(output, /Boundary: preview-only; no execution; no mutation; no gates enforced/);
});
...
test("dema roadmap preview prints a human-readable advisory preview", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "roadmap", "preview"]);
...
test("dema roadmap preview --json emits the schema-tagged roadmap", async () => {
  const { stdout } = await execFileAsync("node", [cliPath, "roadmap", "preview", "--json"]);
  ...
  assert.ok(report.blueprint_coverage.some((entry) => entry.domain === "ci_cd"));
});
