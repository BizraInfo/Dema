...
export function createLocalProposalEnvelope(proposal, consent) {
  ...
  if (
    !proposal.claimLabel ||
    typeof proposal.claimLabel !== "string" ||
    proposal.claimLabel.trim().length === 0
  ) {
    throw new Error(
      "VALIDATION_FAILED: claim label field required (non-empty string)",
    );
  }
  if (
    !proposal.source ||
  ...
  if (
    !proposal.evidence ||
    (typeof proposal.evidence !== "string" && !Array.isArray(proposal.evidence))
  ) {
    throw new Error(
      "VALIDATION_FAILED: claim evidence required (string or array)",
    );
  }
  ...
  for (const term of FORBIDDEN_PROMOTION_TERMS) {
    if (proposalText.includes(term)) {
      throw new Error(
        `FORBIDDEN_PROMOTION: detected "${term}" - rejected per boundary`,
      );
    }
  }
  ...
  if (consent !== REQUIRED_CONSENT) {
    throw new Error(
      `CONSENT_REQUIRED: exact "${REQUIRED_CONSENT}" marker required`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      "REVIEW_BOUNDARY: proposal must not contain review/decision/score fields",
    );
  }
...
export function loadExampleProposal() {
  return {
    claimLabel:
      "Local test proposal for Impact Launchpad MVP boundary (post-G8R)",
    source: "ADR-020 + R3 CI gate repair",
    evidence: [
      "G8R success on 2150ff5 (4 rails)",
      "R1F/R2/R3 classifier patches",
      "local proofs 7/7 + mu 104/104",
    ],
    description:
      "Minimal local envelope only - no contracts, scoring, token, reward, marketplace, public copy, Node1, URP, Shariah claim",
  };
}
