...
export const MISSION_CENTRIC_STATE_ECOSYSTEM_MOCK_CONSENT =
  "GO: MISSION-CENTRIC STATE ECOSYSTEM MOCK";
...
export function createMockMissionCentricStateEcosystem(
  { requireConsent },
  input = loadExampleMissionCentricStateInput(),
) {
  if (requireConsent !== MISSION_CENTRIC_STATE_ECOSYSTEM_MOCK_CONSENT) {
    throw new Error(
      'CONSENT_REQUIRED: exact "GO: MISSION-CENTRIC STATE ECOSYSTEM MOCK" required',
    );
  }
  ...
  if (
    !input.state_transition_id ||
    !input.state_transition_id.startsWith("sha256:")
  ) {
    throw new Error(
      "VALIDATION_FAILED: state_transition_id must start with sha256:",
    );
  }
  if (
  ...
  const mission_state_id =
    "sha256:" +
    createHash("sha256")
      .update(canonical + MISSION_CENTRIC_STATE_ECOSYSTEM_MOCK_CONSENT)
      .digest("hex");
...
export function loadExampleMissionCentricStateInput() {
  return {
    ...
    proof_gaps: [
      "GAP_MISSION_STATE_NOT_YET_PERSISTED",
...
if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(
    "--- BIZRA G39: MISSION-CENTRIC STATE ECOSYSTEM MOCK SELF-TEST ---",
  );
  try {
    ...
    console.log(
      "1. creates local envelope: mission_state_id=",
      (r1.mission_state_id || "").substring(0, 30) + "... current=",
      r1.current_state,
    );

    // 2. requires exact consent
    try {
      createMockMissionCentricStateEcosystem({ requireConsent: "WRONG" }, base);
    ...
    console.log(
      "4. includes air_ref and state_transition_ref:",
      !!r1.air_ref && !!r1.state_transition_ref,
    );

    // 5. environment re-check expectation
    const hasEnv =
      r1.environment_recheck_result &&
      r1.environment_recheck_result.placeholder === true &&
      r1.environment_recheck_result.runtime_implemented === false;
    console.log(
    ...
    console.log(
      "6. declares stale-belief policy without opaque compression:",
      hasStale,
    );
    ...
    const hasHhmm =
      r1.hhmm_state &&
      r1.hhmm_state.placeholder === true &&
      r1.hhmm_state.engine_implemented === false;
    ...
    console.log(
      ...
      r1.writer_ref && r1.writer_ref.startsWith("sha256:"),
    );

    // 9. AgentFold expectation
    const hasAgent =
      r1.agentfold_expectation &&
      r1.agentfold_expectation.placeholder === true &&
      r1.agentfold_expectation.agentfold_l3_implemented === false;
    console.log(
    ...
    const hasDl =
      r1.datalake_alignment_expectation &&
      r1.datalake_alignment_expectation.placeholder === true &&
      r1.datalake_alignment_expectation.datalake_sync_implemented === false;
    console.log("10. declares Data Lake alignment without sync:", hasDl);

    // 11. URP expectation
    const hasUrp =
      r1.urp_expectation &&
      r1.urp_expectation.placeholder === true &&
      r1.urp_expectation.urp_sync_implemented === false &&
      r1.urp_expectation.public_publication === false;
    console.log(
    ...
    const r13a = createMockMissionCentricStateEcosystem(
      { requireConsent: MISSION_CENTRIC_STATE_ECOSYSTEM_MOCK_CONSENT },
      base,
    );
    const r13b = createMockMissionCentricStateEcosystem(
      { requireConsent: MISSION_CENTRIC_STATE_ECOSYSTEM_MOCK_CONSENT },
      base,
    );
    console.log(
      "13. deterministic mission_state_id (excl created_at):",
      r13a.mission_state_id === r13b.mission_state_id,
    );

    console.log(
      "G39 self-test PASS (local mock, consented, required fields, placeholders, deterministic, no forbidden).",
    );
    process.exit(0);
  } catch (e) {
