...
export const REWARD_ELIGIBILITY_MOCK_CONSENT =
  "GO: MOCK REWARD ELIGIBILITY REVIEW FOR ADR-024";
...
export function createMockRewardEligibilityReview(
  ...
  if (requireConsent !== REWARD_ELIGIBILITY_MOCK_CONSENT) {
    throw new Error(
      `CONSENT_REQUIRED: exact "${REWARD_ELIGIBILITY_MOCK_CONSENT}" marker required`,
    );
  }
  ...
  for (const key of Object.keys(input)) {
    if (!ALLOWED_INPUT_FIELDS.has(key)) {
      throw new Error(
        `FORBIDDEN_INPUT: field "${key}" not in allowed set per ADR-024`,
      );
    }
  }
  ...
  const scenario =
    input.local_context && input.local_context.prototype_scenario;
  const eligibilityStatus =
    ELIGIBILITY_STATUS_BY_SCENARIO[scenario] ||
    ELIGIBILITY_STATUS_BY_SCENARIO.needs_more_evidence;

  const proofGaps =
    Array.isArray(input.proof_gaps) && input.proof_gaps.length > 0
      ? [...input.proof_gaps]
      : [
...
export function loadExampleRewardEligibilityInput() {
  return {
    ...
    claim_label:
      "Local review candidate state only [PROTOTYPE] [DESIGNED_NOT_LIVE]",
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    console.error(
      "G20 reward eligibility mock local prototype FAIL:",
      error.message,
    );
    process.exit(1);
  }
