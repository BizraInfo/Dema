...
export const DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT =
  "GO: DEMA DATA-LAKE ALIGNMENT MOCK";
...
const FORBIDDEN_OUTPUT_KEYS = [
  ...
  "pat_runtime_invoked",
  "sat_runtime_invoked",
  "fate_runtime_executed",
];

export function createMockDemaDataLakeAlignment(
  { requireConsent },
  input = loadExampleDemaDataLakeAlignmentInput(),
) {
  if (requireConsent !== DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT) {
    throw new Error(
      'CONSENT_REQUIRED: exact "GO: DEMA DATA-LAKE ALIGNMENT MOCK" required',
    );
  }
  ...
  const alignment_boundary_id =
    "sha256:" +
    createHash("sha256")
      .update(canonical + DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT)
      .digest("hex");
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    ...
    console.log(
      "1. creates local alignment envelope: alignment_boundary_id=",
      (r1.alignment_boundary_id || "").substring(0, 30) + "... status=",
      r1.face_body_alignment_status,
    );

    // 2. requires exact consent
    try {
      createMockDemaDataLakeAlignment({ requireConsent: "WRONG" }, base);
    ...
    try {
      ...
      if (!e.message.includes("dema_artifact_ref")) throw e;
      console.log("3. requires dema_artifact_ref");
    }

    // 4. declares REFERENCE_EXPECTATION_ONLY face/body status
    console.log(
      "4. declares face/body alignment status REFERENCE_EXPECTATION_ONLY:",
      r1.face_body_alignment_status === "REFERENCE_EXPECTATION_ONLY",
    );

    // 5. declares PAT-7 expectation without runtime
    const hasPat =
      r1.pat7_expectation &&
      r1.pat7_expectation.placeholder === true &&
      r1.pat7_expectation.runtime_implemented === false;
    console.log(
      "5. declares PAT-7 expectation without runtime invocation:",
      hasPat,
    );
    ...
    const hasSat =
      r1.sat5_expectation &&
      r1.sat5_expectation.placeholder === true &&
      r1.sat5_expectation.runtime_implemented === false;
    console.log(
      "6. declares SAT-5 expectation without runtime invocation:",
      hasSat,
    );
    ...
    const hasFate =
      r1.fate_expectation &&
      r1.fate_expectation.placeholder === true &&
      r1.fate_expectation.runtime_implemented === false;
    console.log(
      "7. declares FATE expectation without runtime invocation:",
      hasFate,
    );
    ...
    console.log(
      "8. declares URP expectation non-claim (no sync/public):",
      hasUrp,
    );
    ...
    console.log(
      "9. includes proof_gaps and prototype_posture:",
      hasGaps && hasPosture,
    );
    ...
    console.log(
      "10. never returns forbidden fields (sync/mutation/bridge/runtime):",
      hasNoForbidden,
    );

    // 11. deterministic excluding created_at
    const r11a = createMockDemaDataLakeAlignment(
      { requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT },
      base,
    );
    const r11b = createMockDemaDataLakeAlignment(
      { requireConsent: DEMA_DATALAKE_ALIGNMENT_MOCK_CONSENT },
      base,
    );
    console.log(
      "11. deterministic alignment_boundary_id (excl created_at):",
      r11a.alignment_boundary_id === r11b.alignment_boundary_id,
    );

    console.log(
      "G43 self-test PASS (local mock, consented, required refs, 10 boundary categories as placeholders, deterministic, no forbidden).",
    );
    process.exit(0);
  } catch (e) {
