...
export const REWARD_RECEIPT_LOCAL_WRITER_CONSENT = 'GO: REWARD RECEIPT LOCAL WRITER PROTOTYPE';
...
export async function writeLocalRewardReceipt({ requireConsent, demaHome }, writePlan) {
  if (requireConsent !== REWARD_RECEIPT_LOCAL_WRITER_CONSENT) {
    throw new Error('CONSENT_REQUIRED: exact "GO: REWARD RECEIPT LOCAL WRITER PROTOTYPE" required');
  }
  ...
  const proposed = writePlan.proposed_path || 'receipts/reward-receipt-local.json';
  ...
  if (!Array.isArray(writePlan.proof_gaps) || writePlan.proof_gaps.length === 0) {
    return {
      schema: 'bizra.reward.receipt.local_writer_result.v0.1',
      local_writer_result_id: null,
      local_write_plan_id: writePlan.local_write_plan_id,
  ...
  const readCanonical = JSON.stringify(readBackObj, Object.keys(readBackObj).sort());
  const integrity_hash = 'sha256:' + createHash('sha256').update(readCanonical).digest('hex');
...
export function loadExampleLocalWriterInput() {
  return {
    ...
    content: {
      receipt_review_id: 'ex-receipt-review-001',
      claim_label: 'Local writer prototype test only [PROTOTYPE] [DESIGNED_NOT_LIVE]'
    },
...
if (import.meta.url === `file://${process.argv[1]}`) {
  console.log('--- BIZRA G31: REWARD RECEIPT LOCAL WRITER PROTOTYPE SELF-TEST ---');
  (async () => {
    try {
      ...
      const r1 = await writeLocalRewardReceipt({ requireConsent: REWARD_RECEIPT_LOCAL_WRITER_CONSENT, demaHome: tempRoot }, base);
      console.log('1. write under temp DEMA_HOME:', r1.write_result_status, 'path safe:', !r1.final_local_path.includes('..'));
      console.log('   read_back_verified:', r1.read_back_verified);

      // 2. consent
      try {
        await writeLocalRewardReceipt({ requireConsent: 'WRONG', demaHome: tempRoot }, base);
      ...
      const r3 = await writeLocalRewardReceipt({ requireConsent: REWARD_RECEIPT_LOCAL_WRITER_CONSENT, demaHome: tempRoot }, badPath);
      console.log('3. rejects unsafe traversal:', r3.write_result_status);

      // 4. absolute
      const absPath = { ...base, proposed_path: '/tmp/evil.json' };
      const r4 = await writeLocalRewardReceipt({ requireConsent: REWARD_RECEIPT_LOCAL_WRITER_CONSENT, demaHome: tempRoot }, absPath);
      ...
      const r8a = await writeLocalRewardReceipt({ requireConsent: REWARD_RECEIPT_LOCAL_WRITER_CONSENT, demaHome: tempRoot }, base);
      const r8b = await writeLocalRewardReceipt({ requireConsent: REWARD_RECEIPT_LOCAL_WRITER_CONSENT, demaHome: tempRoot }, base);
      console.log('8. deterministic id (excl created_at):', r8a.local_writer_result_id === r8b.local_writer_result_id);

      // 9. file mode expectation
      console.log('9. file_mode_expected 0o600:', r1.file_mode_expected === '0o600');

      // 10. refuses empty proof_gaps
      const noGaps = { ...base, proof_gaps: [] };
      const r10 = await writeLocalRewardReceipt({ requireConsent: REWARD_RECEIPT_LOCAL_WRITER_CONSENT, demaHome: tempRoot }, noGaps);
      console.log('10. refuses empty proof_gaps:', r10.write_result_status);

      console.log('G31 self-test PASS (local-only writer, atomic, verified, deterministic, no side effects).');
