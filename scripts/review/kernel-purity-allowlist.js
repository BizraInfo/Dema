...
export const NONCORE_IO_TIER_ALLOWLIST = Object.freeze({
  ...
  "packages/genesis/src/node0-genesis-witness.js":
    "NODE0-GENESIS-ANCHOR-1A. mkdir/open/link/readFile/unlink for ONE file held deliberately OUTSIDE DEMA_HOME (env DEMA_GENESIS_WITNESS, else XDG config root) plus a read of the canonical root record it checks. The out-of-band ceremony pin: create-once via wx tmp + link (EEXIST = already witnessed), never rewritten, never repaired from DEMA_HOME. It carries a fingerprint and a commitment and REFUSES key material outright, so it can veto a substituted root but can never nominate one. No network/child_process, no clock (the caller supplies it), never a private key byte.",
