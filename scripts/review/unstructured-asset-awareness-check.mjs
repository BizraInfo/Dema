#!/usr/bin/env node
// UNSTRUCTURED-ASSET-AWARENESS-GATE-1A — read-only unstructured asset awareness verifier.
...
export function runUnstructuredAssetAwarenessGateCheck() {
  return runUnstructuredAssetAwarenessGate();
}

if (process.argv[1] && import.meta.url === `file://${process.argv[1]}`) {
  const result = runUnstructuredAssetAwarenessGateCheck();
  ...
  if (JSON_MODE) {
    ...
    console.log(`  fixture assets: ${UNSTRUCTURED_FIXTURE_ASSETS.length}`);
