...
function cliCommandFromArgs(args, helpCommands) {
  ...
  for (const command of candidates) {
    ...
    if (tokens.every((token, index) => cliArgs[index] === token))
      return command;
  }
  return null;
}
...
export async function buildIntegrationCheckReport({
  root = process.cwd(),
} = {}) {
  const checks = [];
  const missingFiles = [
  ...
  checks.push(
    makeCheck("required_files_present", missingFiles.length === 0, {
  ...
  if (cliSource) {
    ...
    checks.push(
      makeCheck("help_commands_discovered", helpCommands.length > 0, {
  ...
  if (architectureSource && helpCommands.length) {
    ...
    checks.push(
      makeCheck("help_commands_in_architecture_map", missing.length === 0, {
        missing,
      }),
    );
  }
  ...
  if (smokeCommands) {
    ...
    checks.push(
      makeCheck("smoke_cli_commands_match_help", unknown.length === 0, {
        unknown,
      }),
    );
    ...
    checks.push(
      makeCheck("smoke_commands_documented", missingSmokeDocs.length === 0, {
        missing: missingSmokeDocs,
      }),
    );
  } else {
    checks.push(
      makeCheck("check_commands_exported", false, {
        missing: [CHECK_PATH],
      }),
    );
  }
  ...
  checks.push(
    makeCheck("test_files_documented", missingTestRows.length === 0, {
