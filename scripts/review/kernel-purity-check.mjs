...
export const IO_TIER_ALLOWLIST = Object.freeze({
  ...
  "node0-runtime-write-surface-adapter.js":
    "NODE0-RUNTIME-WRITE-SURFACE-1A: two bounded reads and nothing else \u2014 one recorded write-surface artefact under DEMA_HOME||~/.dema, and the classification kernel's own bytes. Reader tier by design: `remote_write` is the write surface of the RUNNING host, so the PRODUCER runs the read-only probes and THIS module only reads their recorded verdict \u2014 which is what lets the review gate keep declaring execution_allowed:false honestly. No write, no spawn, no network, no clock.",
  "node0-history-replay-adapter.js":
    "NODE0-HISTORY-REPLAY-1A: two bounded reads and nothing else \u2014 one recorded replay artefact under DEMA_HOME||~/.dema, and the classification kernel's own bytes. Reader tier by design: `full_history_replayable` cannot be measured without spending the producing process, so the producer spawns and dies and THIS module only reads \u2014 which is what lets the review gate keep declaring execution_allowed:false honestly. No write, no spawn, no network, no clock.",
