...
if (!proof.ok) throw new Error("U1 proof artifacts do not verify byte-for-byte.");

const selfCheck = await verifySelfCheckReports();
if (!selfCheck.ok) throw new Error("U1 self-check reports do not verify byte-for-byte.");

for (const file of ARTIFACT_FILES) {
  const artifact = await readJson(file);
  if (artifact.content_sha256 !== contentHash(artifact)) {
    throw new Error(`${file} content_sha256 does not match canonical content hash.`);
  }
  assertFalse(artifact.token_value_claim, `${file} token_value_claim`);
  assertFalse(artifact.node1_handshake, `${file} node1_handshake`);
  assertFalse(artifact.raw_private_data_included, `${file} raw_private_data_included`);
  assertFalse(artifact.federation_claim, `${file} federation_claim`);
}

const status = await readJson("node0_local_urp_status.json");
if (status.truth_label !== "URP_LOCAL_ACTIVE") throw new Error("truth_label must be URP_LOCAL_ACTIVE.");
...
if (!sat.roles.every((role) => role.verdict_authority === "placeholder_only_never_permit")) {
...
assertFalse(criticReport.boundary.sat_permit_claimed, "critic sat_permit_claimed");
