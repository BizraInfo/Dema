...
export const REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT =
  "GO: REWARD RECEIPT LOCAL WRITE PLAN MOCK";
...
export function createMockRewardReceiptLocalWritePlan(
  { requireConsent },
  input = loadExampleRewardReceiptLocalWriteInput(),
) {
  if (requireConsent !== REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT) {
    throw new Error(
      'CONSENT_REQUIRED: exact "GO: REWARD RECEIPT LOCAL WRITE PLAN MOCK" required',
    );
  }
  ...
  const proposedPath =
    input.proposed_path || "receipts/reward-receipt-local-plan.json";
  if (proposedPath.includes("..") || proposedPath.startsWith("/")) {
    throw new Error(
      "UNSAFE_PATH: proposed_path must not traverse parents or be absolute",
    );
  }
  ...
  const idx =
    input.receipt_context && input.receipt_context.index
      ? input.receipt_context.index % 4
      : 0;
  ...
  const body = {
    ...
    content_hash: createHash("sha256")
      .update(
        JSON.stringify(input.content || input.claim_label || "plan-content"),
      )
      .digest("hex"),
  ...
  const local_write_plan_id =
    "sha256:" +
    createHash("sha256")
      .update(canonical + REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT)
      .digest("hex");
...
export function loadExampleRewardReceiptLocalWriteInput() {
  return {
    ...
    claim_label:
      "Local persistence plan for receipt review only [PROTOTYPE] [DESIGNED_NOT_LIVE]",
    content: { review: "minimal-receipt-review" },
    proposed_path: "receipts/receipt-local-plan.json",
    proof_gaps: [
      "GAP_LOCAL_PERSISTENCE_NOT_YET_EXECUTED",
      "GAP_HUMAN_REVIEW_PENDING",
    ],
    receipt_context: { node: "Node0", phase: "G27" },
    timestamp: Date.now(),
    local_context: { node: "Node0", phase: "G27" },
    description:
      "Minimal local persistence plan only — after G26R. Excludes all actual writing, minting, publishing, bridging, reward logic, token, contracts, marketplace, Node1, URP, Shariah claim [DECLARED]",
  };
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(
    "--- BIZRA G27: REWARD RECEIPT LOCAL WRITE PLAN MOCK SELF-TEST ---",
  );
  try {
    const base = loadExampleRewardReceiptLocalWriteInput();

    // 1. accepts valid
    const p1 = createMockRewardReceiptLocalWritePlan(
      { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
      base,
    );
    console.log(
      "1. accepts valid input: local_write_plan_id=",
      p1.local_write_plan_id.substring(0, 20) + "...",
    );

    // 2. rejects missing consent
    try {
      createMockRewardReceiptLocalWritePlan({ requireConsent: "WRONG" }, base);
    ...
    try {
      createMockRewardReceiptLocalWritePlan(
        { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
        { ...base, receipt_context: { note: "expected mint" } },
      );
      throw new Error("should have thrown");
    } catch (e) {
      if (!e.message.includes("FORBIDDEN_PROMOTION")) throw e;
      console.log("3. rejects forbidden mint/write/publish/bridge language");
    }
    ...
    try {
      createMockRewardReceiptLocalWritePlan(
        { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
        { ...base, proposed_path: "../../etc/passwd" },
      );
      throw new Error("should have thrown");
    } catch (e) {
      if (!e.message.includes("UNSAFE_PATH")) throw e;
      console.log("4. rejects unsafe path traversal");
    }

    // 5+6. returns allowed fields only, never forbidden
    const p5 = createMockRewardReceiptLocalWritePlan(
      { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
      base,
    );
    const allowed = [
    ...
    const hasNoForbidden = !(
      "file_written" in p5 ||
      "receipt_minted" in p5 ||
      "reward_authorized" in p5 ||
      "token_amount" in p5 ||
      "contract_call" in p5
    );
    console.log(
      "5+6. returns allowed fields only, never writes or mints:",
      hasOnlyAllowed && hasNoForbidden,
    );
    ...
    try {
      ...
      if (!e.message.includes("proof_gaps")) throw e;
      console.log("7. requires proof_gaps");
    }

    // 8. deterministic id
    const p8a = createMockRewardReceiptLocalWritePlan(
      { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
      base,
    );
    const p8b = createMockRewardReceiptLocalWritePlan(
      { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
      base,
    );
    console.log(
      "8. deterministic local_write_plan_id:",
      p8a.local_write_plan_id === p8b.local_write_plan_id,
    );
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const p = createMockRewardReceiptLocalWritePlan(
        { requireConsent: REWARD_RECEIPT_LOCAL_WRITE_PLAN_CONSENT },
        inp,
      );
    ...
    console.log(
      "G27 self-test PASS (local write plan, consented, gapped, deterministic, all statuses, no filesystem side effects).",
    );
    process.exit(0);
  } catch (e) {
