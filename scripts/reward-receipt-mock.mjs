...
export const REWARD_RECEIPT_MOCK_CONSENT =
  "GO: REWARD RECEIPT MOCK LOCAL PROTOTYPE";
...
export function createMockRewardReceiptReview(
  { requireConsent },
  input = loadExampleRewardReceiptInput(),
) {
  if (requireConsent !== REWARD_RECEIPT_MOCK_CONSENT) {
    throw new Error(
      'CONSENT_REQUIRED: exact "GO: REWARD RECEIPT MOCK LOCAL PROTOTYPE" required',
    );
  }
  ...
  const receipt_review_id =
    "sha256:" +
    createHash("sha256")
      .update(canonical + REWARD_RECEIPT_MOCK_CONSENT)
      .digest("hex");
...
export function loadExampleRewardReceiptInput() {
  return {
    ...
    claim_label:
      "Minimal local review receipt mock test boundary only [PROTOTYPE] [DESIGNED_NOT_LIVE]",
    ...
    description:
      "Minimal local mock receipt review only — after G22R. Excludes all receipt implementation, minting, writing, publishing, bridging, reward logic, token, contracts, marketplace, Node1, URP, Shariah claim [DECLARED]",
  };
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(
    "--- BIZRA G23: REWARD RECEIPT MOCK LOCAL PROTOTYPE SELF-TEST ---",
  );
  try {
    const base = loadExampleRewardReceiptInput();

    // 1. accepts valid
    const r1 = createMockRewardReceiptReview(
      { requireConsent: REWARD_RECEIPT_MOCK_CONSENT },
      base,
    );
    console.log(
      "1. accepts valid input: receipt_review_id=",
      r1.receipt_review_id.substring(0, 20) + "...",
    );

    // 2. rejects missing consent
    try {
      createMockRewardReceiptReview({ requireConsent: "WRONG" }, base);
    ...
    const r4 = createMockRewardReceiptReview(
      { requireConsent: REWARD_RECEIPT_MOCK_CONSENT },
      base,
    );
    const allowed = [
    ...
    const hasNoForbidden = !(
      "receipt_written" in r4 ||
      "receipt_minted" in r4 ||
      "reward_authorized" in r4 ||
      "token_amount" in r4 ||
      "contract_call" in r4
    );
    console.log(
      "4+5. returns allowed fields only, never mint/write/reward/token/contract:",
    ...
    try {
      ...
      console.log("6. requires proof_gaps");
    }

    // 7. deterministic id
    const r7a = createMockRewardReceiptReview(
      { requireConsent: REWARD_RECEIPT_MOCK_CONSENT },
      base,
    );
    const r7b = createMockRewardReceiptReview(
      { requireConsent: REWARD_RECEIPT_MOCK_CONSENT },
      base,
    );
    console.log(
      "7. deterministic receipt_review_id:",
      r7a.receipt_review_id === r7b.receipt_review_id,
    );
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const r = createMockRewardReceiptReview(
        { requireConsent: REWARD_RECEIPT_MOCK_CONSENT },
        inp,
      );
    ...
    console.log(
      "8. exercises all four receipt_status:",
      statuses.size === 4,
      Array.from(statuses),
    );

    console.log(
      "G23 self-test PASS (local mock, consented, gapped, deterministic, all statuses, no forbidden output).",
    );
    process.exit(0);
  } catch (e) {
