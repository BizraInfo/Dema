...
export function createMockImpactScore({ requireConsent }, context = loadExampleScoringContext()) {
  ...
  for (const term of FORBIDDEN_PROMOTION_TERMS) {
    if (serialized.includes(term)) {
      throw new Error(`FORBIDDEN_PROMOTION: detected "${term}" in scoring context`);
    }
  }
  ...
  const canonical = JSON.stringify({ context, proof }, (k, v) => (typeof v === 'bigint' ? v.toString() : v));
  ...
  return {
    ...
    mockScore: {
      ...context,
      proof,
      value: 'MOCK_ONLY [PROTOTYPE] [DESIGNED_NOT_LIVE] — NO REWARD ELIGIBILITY — TEST BOUNDARY ONLY',
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    const mock = createMockImpactScore({ requireConsent: "GO: MOCK SCORING FOR ADR-021" }, ctx);

    console.log('Peak ultra micro mock scoring created (post scoring boundary G3 proof).');
    console.log('ID prefix:', mock.id.slice(0, 16));
    console.log('claim_label present:', !!mock.mockScore.proof.claim_label);
    console.log('anti_gaming_enforced:', mock.mockScore.proof.anti_gaming_enforced);
    ...
    console.log('receipt_expectation schema:', mock.mockScore.proof.receipt_expectation.schema);
    console.log('value (MOCK_ONLY, no eligibility):', mock.mockScore.value);

    // Assertions for the 5 markers (parallel to proposal)
    if (!mock.mockScore.proof.claim_label) throw new Error('claim_label missing');
    if (!mock.mockScore.proof.anti_gaming_enforced) throw new Error('anti_gaming_enforced false');
    if (!mock.mockScore.proof.consent_required) throw new Error('consent_required false');
    if (!mock.mockScore.proof.review_boundary) throw new Error('review_boundary false');
    if (!mock.mockScore.proof.receipt_expectation || !mock.mockScore.proof.receipt_expectation.placeholder) throw new Error('receipt_expectation incomplete');

    console.log('All 5 markers asserted. Mock scoring boundary respected. [PROTOTYPE] [DESIGNED_NOT_LIVE]');
    process.exit(0);
  } catch (e) {
