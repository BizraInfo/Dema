...
export const NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT = 'GO: NODE0 CLOSED-LOOP DIGEST MOCK';
...
const FORBIDDEN_TERMS = new Set([
  ...
  'fate_runtime', 'digest_written', 'digest_published', 'digest_runtime_active',
  'digest_aggregated', 'node1_sync', 'urp_publication'
]);
...
export function createMockNode0ClosedLoopDigest({ requireConsent }, input = loadExampleNode0ClosedLoopDigestInput()) {
  if (requireConsent !== NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT) {
    throw new Error('CONSENT_REQUIRED: exact "GO: NODE0 CLOSED-LOOP DIGEST MOCK" required');
  }
  ...
  if (!input.receipt_review_id || !input.receipt_review_id.startsWith('sha256:')) {
    throw new Error('VALIDATION_FAILED: receipt_review_id must start with sha256:');
  }
  ...
  if (!input.mission_state_id || !input.mission_state_id.startsWith('sha256:')) {
    throw new Error('VALIDATION_FAILED: mission_state_id must start with sha256:');
  }
  if (!input.alignment_boundary_id || !input.alignment_boundary_id.startsWith('sha256:')) {
    throw new Error('VALIDATION_FAILED: alignment_boundary_id must start with sha256:');
  }
  if (!input.hybrid_knowledge_boundary_id || !input.hybrid_knowledge_boundary_id.startsWith('sha256:')) {
    throw new Error('VALIDATION_FAILED: hybrid_knowledge_boundary_id must start with sha256:');
  }
  ...
  const allowedInput = [
    'digest_scope', 'receipt_review_id', 'local_writer_result_id', 'air_id',
    'state_transition_id', 'mission_state_id', 'alignment_boundary_id',
    'hybrid_knowledge_boundary_id', 'dema_ref', 'datalake_ref', 'mission_ref',
    'proof_gaps', 'still_blocked_invariants', 'consent_status', 'review_status',
    'prototype_posture'
  ];
  ...
  const body = {
    ...
    digest_scope: input.digest_scope || 'NODE0_CLOSED_LOOP_REFERENCE_EXPECTATION',
  ...
  const node0_digest_boundary_id = 'sha256:' + createHash('sha256').update(canonical + NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT).digest('hex');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    ...
    const r1 = createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, base);
    console.log('1. creates local envelope: node0_digest_boundary_id=', (r1.node0_digest_boundary_id || '').substring(0, 30) + '...');

    // 2. requires exact consent
    try { createMockNode0ClosedLoopDigest({ requireConsent: 'WRONG' }, base); throw new Error('should have thrown'); } catch (e) { if (!e.message.includes('CONSENT_REQUIRED')) throw e; console.log('2. rejects missing exact consent'); }

    // 3. requires digest_scope
    try { const bad = { ...base }; delete bad.digest_scope; createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad); throw new Error('should have thrown'); } catch (e) { if (!e.message.includes('digest_scope')) throw e; console.log('3. requires digest_scope'); }

    // 4. requires receipt_review_id sha256 reference
    try { const bad = { ...base }; bad.receipt_review_id = 'not-sha256'; createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad); throw new Error('should have thrown'); } catch (e) { if (!e.message.includes('receipt_review_id')) throw e; console.log('4. requires receipt_review_id sha256 reference'); }

    // 5. requires local_writer_result_id sha256 reference
    try { const bad = { ...base }; bad.local_writer_result_id = 'not-sha256'; createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad); throw new Error('should have thrown'); } catch (e) { if (!e.message.includes('local_writer_result_id')) throw e; console.log('5. requires local_writer_result_id sha256 reference'); }

    // 6. requires air_id sha256 reference
    try { const bad = { ...base }; bad.air_id = 'not-sha256'; createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad); throw new Error('should have thrown'); } catch (e) { if (!e.message.includes('air_id')) throw e; console.log('6. requires air_id sha256 reference'); }

    // 7. requires mission_state_id sha256 reference
    try { const bad = { ...base }; bad.mission_state_id = 'not-sha256'; createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad); throw new Error('should have thrown'); } catch (e) { if (!e.message.includes('mission_state_id')) throw e; console.log('7. requires mission_state_id sha256 reference'); }

    // 8. requires alignment_boundary_id and hybrid_knowledge_boundary_id sha256 references
    try { const bad = { ...base }; bad.alignment_boundary_id = 'not-sha256'; createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, bad); throw new Error('should have thrown'); } catch (e) { if (!e.message.includes('alignment_boundary_id')) throw e; console.log('8. requires alignment + hybrid sha256 references'); }

    // 9. declares proof-chain expectation without digest runtime, writer, aggregator, or closed-loop execution
    const hasChain = r1.proof_chain_expectation &&
    ...
    console.log('9. declares proof-chain expectation without runtime/writer/aggregator/closed-loop:', hasChain);
    ...
    try { createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, badForbidden); throw new Error('should have thrown'); } catch (e) { if (!e.message.includes('FORBIDDEN')) throw e; console.log('11. rejects forbidden digest/runtime/economic/public fields'); }

    // 12. deterministic node0_digest_boundary_id for same semantic input excluding created_at
    const r12a = createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, base);
    const r12b = createMockNode0ClosedLoopDigest({ requireConsent: NODE0_CLOSED_LOOP_DIGEST_MOCK_CONSENT }, base);
    console.log('12. deterministic node0_digest_boundary_id (excl created_at):', r12a.node0_digest_boundary_id === r12b.node0_digest_boundary_id);

    console.log('G51 self-test PASS (local mock, consented, required refs, placeholders, deterministic, no forbidden).');
    process.exit(0);
  } catch (e) {
