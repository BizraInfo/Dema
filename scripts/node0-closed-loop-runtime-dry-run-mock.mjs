...
export function createMockNode0ClosedLoopRuntimeDryRun(
  ...
  if (requireConsent !== NODE0_CLOSED_LOOP_RUNTIME_DRY_RUN_MOCK_CONSENT) {
    throw new Error(
      'CONSENT_REQUIRED: exact "GO: NODE0 CLOSED-LOOP RUNTIME DRY-RUN MOCK" required',
    );
  }

  validateInput(input);

  const runtime_trace_id =
    input.runtime_trace_id ||
    hashObject({
  ...
  const body = {
    ...
    dry_run_only: true,
    state_sequence: Object.freeze(
      STATE_SEQUENCE.map((name) =>
        Object.freeze({
          name,
          dry_run_only: true,
    ...
    still_blocked_invariants: Object.freeze([
      ...input.still_blocked_invariants,
    ]),
...
function validateInput(input) {
  ...
  if (
    ...
    throw new Error(
      "VALIDATION_FAILED: runtime_trace_id must be null or sha256:",
    );
  }
  ...
  if (input.operator_approval_status !== "required_not_collected") {
    throw new Error(
      "VALIDATION_FAILED: operator_approval_status must equal required_not_collected",
    );
  }
