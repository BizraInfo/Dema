...
function containsAll(text, tokens) {
  ...
  return tokens.filter(
    (token) => !normalizedText.includes(token.toLowerCase()),
  );
}
...
export function buildLlmGuidanceReport({ root = defaultRoot } = {}) {
  ...
  if (missingFiles.length === 0) {
    ...
    checks.push(
      makeCheck(
        "canonical_flow_invariants_present",
        missingFlowTokens.length === 0,
        {
    ...
    for (const path of agentFiles) {
      ...
      checks.push(
        makeCheck(
          `${path}_links_canonical_flow`,
          text.includes("docs/LLM_SYSTEM_FLOW.md"),
          {
            path,
          },
        ),
      ...
      checks.push(
        makeCheck(`${path}_is_thin_router`, text.split("\n").length <= 80, {
          path,
        }),
      );
    }
    ...
    checks.push(
      makeCheck(
        "historical_noise_classified",
        missingIndexTokens.length === 0,
        {
