...
function resolveBaselinePath(arg) {
  ...
  const inBaselineDir = join(
    root,
    "docs",
    "baselines",
    `dema-baseline-l1-${arg}.json`,
  );
  if (existsSync(inBaselineDir)) return inBaselineDir;
  throw new Error(
    `baseline not found: ${arg} (tried direct path and docs/baselines/dema-baseline-l1-${arg}.json)`,
  );
}
...
function parseArgs() {
  if (argv.includes("--help") || argv.length < 2) {
    process.stderr.write(
      "usage: baseline-l1-diff [--files] <before> <after>\n",
    );
    process.exit(2);
  }
  ...
  return {
    before: resolveBaselinePath(pair[0]),
    after: resolveBaselinePath(pair[1]),
  };
}
...
function loadBaseline(path) {
  ...
  if (data.schema !== "bizra.dema.baseline_l1.v0.1") {
    throw new Error(
      `${path}: not a baseline_l1.v0.1 (got schema='${data.schema}')`,
    );
  }
  return data;
}
...
function computeAsymmetryScore(beforeSrc, afterSrc) {
  ...
  const packagesGrowth = pctGrowth(
    beforeSrc.packages_loc,
    afterSrc.packages_loc,
  );
...
const delta = Object.freeze({
  ...
  source_state_delta: Object.freeze({
    packages_loc: deltaInt(
      before.source_state?.packages_loc,
      after.source_state?.packages_loc,
    ),
    packages_files: deltaInt(
      before.source_state?.packages_files,
      after.source_state?.packages_files,
    ),
    tests_loc: deltaInt(
      before.source_state?.tests_loc,
      after.source_state?.tests_loc,
    ),
    tests_files: deltaInt(
      before.source_state?.tests_files,
      after.source_state?.tests_files,
    ),
    scripts_loc: deltaInt(
      before.source_state?.scripts_loc,
      after.source_state?.scripts_loc,
    ),
    scripts_files: deltaInt(
      before.source_state?.scripts_files,
      after.source_state?.scripts_files,
    ),
    apps_loc: deltaInt(
      before.source_state?.apps_loc,
      after.source_state?.apps_loc,
    ),
    schemas_declared_unique: deltaInt(
      before.source_state?.schemas_declared_unique,
      after.source_state?.schemas_declared_unique,
    ),
    cli_commands_in_help: deltaInt(
      before.source_state?.cli_commands_in_help,
      after.source_state?.cli_commands_in_help,
    ),
  }),
  ...
  verify_before_assert_trend: computeAsymmetryScore(
    before.source_state ?? {},
    after.source_state ?? {},
  ),
  boundary: Object.freeze(
    Object.fromEntries(PREVIEW_BOUNDARY_CANONICAL_KEYS.map((k) => [k, false])),
  ),
  notes: Object.freeze([
