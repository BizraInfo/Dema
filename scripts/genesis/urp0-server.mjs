...
export const DEMA_BRIDGE_ALLOWED_ROUTES = Object.freeze([
  "GET /readyz",
  "GET /api/realm",
...
export function createUrp0Server({
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      if (demaBridge && !DEMA_BRIDGE_ALLOWED_ROUTES.includes(`${req.method} ${path}`)) {
        return json(res, 403, { ok: false, blocked_by: ["dema_bridge_route_not_exposed"] }, origin, allowedOrigins);
      }
      ...
      if (req.method === "POST" && path === "/api/consent-card") {
        const body = await readBody(req);
        const out = missionConsentCard(stateRootDir, {
          root: body.root,
