  ...
  runPrePushProofSealScript()
    .then((report) => {
      if (JSON_MODE) {
        ...
        console.log(
          `  git:     ${(report.git.working_tree_status ?? (report.git.working_tree_clean ? "CLEAN" : "DIRTY")).toLowerCase()} · ahead ${report.git.upstream_counts?.ahead ?? "?"} · behind ${report.git.upstream_counts?.behind ?? "?"}`,
        );
