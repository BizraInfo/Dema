...
export const ATOMIC_IMPACT_RECEIPT_LIFECYCLE_MOCK_CONSENT =
  "GO: ATOMIC IMPACT RECEIPT LIFECYCLE MOCK";
...
export function createMockAtomicImpactReceiptLifecycle(
  { requireConsent },
  input = loadExampleAtomicImpactReceiptLifecycleInput(),
) {
  if (requireConsent !== ATOMIC_IMPACT_RECEIPT_LIFECYCLE_MOCK_CONSENT) {
    throw new Error(
      'CONSENT_REQUIRED: exact "GO: ATOMIC IMPACT RECEIPT LIFECYCLE MOCK" required',
    );
  }
  ...
  const body = {
    ...
    local_writer_result_id:
      input.local_writer_result_id || "sha256:example-writer-ref-from-g31",
    claim_label:
      input.claim_label ||
      "Atomic Impact Receipt Lifecycle mock — local envelope only [PROTOTYPE] [DESIGNED_NOT_LIVE]",
  ...
  const air_id =
    "sha256:" +
    createHash("sha256")
      .update(canonical + ATOMIC_IMPACT_RECEIPT_LIFECYCLE_MOCK_CONSENT)
      .digest("hex");
  const state_transition_id =
    "sha256:" +
    createHash("sha256")
      .update("PERSISTED->READY_FOR_REVIEW:" + air_id)
      .digest("hex");
...
export function loadExampleAtomicImpactReceiptLifecycleInput() {
  return {
    ...
    claim_label:
      "Atomic Impact Receipt Lifecycle mock — local envelope only [PROTOTYPE] [DESIGNED_NOT_LIVE]",
    proof_gaps: [
      "GAP_AIR_LIFECYCLE_NOT_YET_SEALED",
      "GAP_HUMAN_REVIEW_PENDING_FOR_READY_STATE",
    ],
...
if (import.meta.url === `file://${process.argv[1]}`) {
  console.log(
    "--- BIZRA G35: ATOMIC IMPACT RECEIPT LIFECYCLE MOCK SELF-TEST ---",
  );
  try {
    ...
    console.log(
      "1. creates local AIR envelope: air_id=",
      (r1.air_id || "").substring(0, 30) + "... lifecycle_state=",
      r1.lifecycle_state,
    );

    // 2. requires exact consent
    try {
      createMockAtomicImpactReceiptLifecycle({ requireConsent: "WRONG" }, base);
    ...
    const hasMcp =
      r1.mcp_expectation &&
      r1.mcp_expectation.placeholder === true &&
      r1.mcp_expectation.runtime_implemented === false;
    console.log("4. declares MCP expectation without runtime:", hasMcp);

    // 5. A2A
    const hasA2a =
      r1.a2a_expectation &&
      r1.a2a_expectation.placeholder === true &&
      r1.a2a_expectation.pat_sat_bridge_runtime_implemented === false;
    console.log(
    ...
    const hasHhmm =
      r1.hhmm_expectation &&
      r1.hhmm_expectation.placeholder === true &&
      r1.hhmm_expectation.engine_implemented === false;
    console.log("6. declares HHMM expectation without engine:", hasHhmm);

    // 7. Seal
    const hasSeal =
      r1.seal_expectation &&
      r1.seal_expectation.placeholder === true &&
      r1.seal_expectation.agentfold_l3_implemented === false;
    console.log(
      "7. declares AgentFold seal expectation without implementation:",
      hasSeal,
    );
    ...
    console.log(
      "9. never returns forbidden economic/public fields:",
      hasNoForbidden,
    );

    // 10. deterministic excluding created_at
    const r10a = createMockAtomicImpactReceiptLifecycle(
      { requireConsent: ATOMIC_IMPACT_RECEIPT_LIFECYCLE_MOCK_CONSENT },
      base,
    );
    const r10b = createMockAtomicImpactReceiptLifecycle(
      { requireConsent: ATOMIC_IMPACT_RECEIPT_LIFECYCLE_MOCK_CONSENT },
      base,
    );
    console.log(
      "10. deterministic air_id (excl created_at):",
      r10a.air_id === r10b.air_id,
    );

    console.log(
      "G35 self-test PASS (local mock envelope, consented, gapped, expectations as placeholders only, deterministic, no forbidden).",
    );
    process.exit(0);
  } catch (e) {
