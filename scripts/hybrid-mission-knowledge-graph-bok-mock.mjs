...
export const HYBRID_MISSION_KNOWLEDGE_GRAPH_BOK_MOCK_CONSENT = 'GO: HYBRID MISSION KNOWLEDGE GRAPH BOK MOCK';
...
const FORBIDDEN_OUTPUT_KEYS = [
  'vector_memory_runtime_active', 'autonomous_retrieval_active',
  'opaque_compression_active', 'global_state_store_active',
  'context_rewrite_performed', 'datalake_synced',
  'cross_repo_write_performed', 'runtime_bridge_active',
  'node1_sync', 'urp_publication', 'token_minted',
  'reward_authorized', 'contract_call', 'marketplace_signal',
  'public_receipt_url', 'shariah_compliant'
];

export function createMockHybridMissionKnowledgeGraphBok({ requireConsent }, input = loadExampleHybridMissionKnowledgeGraphBokInput()) {
  if (requireConsent !== HYBRID_MISSION_KNOWLEDGE_GRAPH_BOK_MOCK_CONSENT) {
    throw new Error('CONSENT_REQUIRED: exact "GO: HYBRID MISSION KNOWLEDGE GRAPH BOK MOCK" required');
  }
  ...
  const allowedInput = [
    'mission_id', 'mission_state_id', 'air_id', 'alignment_boundary_id',
    'dema_ref', 'datalake_ref', 'mission_tree_ref',
    'knowledge_node_ref', 'knowledge_edge_ref', 'bok_pattern_ref',
    'environment_refs', 'expected_hashes', 'proof_gaps',
  ...
  const hybrid_knowledge_boundary_id = 'sha256:' + createHash('sha256').update(canonical + HYBRID_MISSION_KNOWLEDGE_GRAPH_BOK_MOCK_CONSENT).digest('hex');
...
export function loadExampleHybridMissionKnowledgeGraphBokInput() {
  return {
    ...
    proof_gaps: ['GAP_HYBRID_KNOWLEDGE_NOT_YET_IMPLEMENTED', 'GAP_REFERENCE_EXPECTATION_ONLY'],
...
if (import.meta.url === `file://${process.argv[1]}`) {
  console.log('--- BIZRA G47: HYBRID MISSION KNOWLEDGE GRAPH BOK MOCK SELF-TEST ---');
  try {
    const base = loadExampleHybridMissionKnowledgeGraphBokInput();

    // 1. creates with sha256 hybrid_knowledge_boundary_id
    const r1 = createMockHybridMissionKnowledgeGraphBok({ requireConsent: HYBRID_MISSION_KNOWLEDGE_GRAPH_BOK_MOCK_CONSENT }, base);
    console.log('1. creates local envelope: hybrid_knowledge_boundary_id=', (r1.hybrid_knowledge_boundary_id || '').substring(0,30)+'...');

    // 2. requires exact consent
    try { createMockHybridMissionKnowledgeGraphBok({ requireConsent: 'WRONG' }, base); throw new Error('should have thrown'); } catch(e){ if(!e.message.includes('CONSENT_REQUIRED')) throw e; console.log('2. rejects missing exact consent'); }

    // 3. requires mission_id
    try { const bad = {...base}; delete bad.mission_id; createMockHybridMissionKnowledgeGraphBok({ requireConsent: HYBRID_MISSION_KNOWLEDGE_GRAPH_BOK_MOCK_CONSENT }, bad); throw new Error('should have thrown'); } catch(e){ if(!e.message.includes('mission_id')) throw e; console.log('3. requires mission_id'); }

    // 4. includes mission_state_ref and alignment_ref
    console.log('4. includes mission_state_ref and alignment_ref sha256:', r1.mission_state_ref.startsWith('sha256:') && r1.alignment_ref.startsWith('sha256:'));

    // 5. declares mission tree expectation without runtime
    const hasTree = r1.mission_tree_expectation && r1.mission_tree_expectation.placeholder === true && r1.mission_tree_expectation.mission_tree_runtime_implemented === false;
    console.log('5. declares mission tree expectation without runtime:', hasTree);

    // 6. declares knowledge graph expectation without graph runtime or autonomous retrieval
    const hasGraph = r1.knowledge_graph_expectation && r1.knowledge_graph_expectation.placeholder === true && r1.knowledge_graph_expectation.graph_runtime_implemented === false && r1.knowledge_graph_expectation.autonomous_retrieval_enabled === false;
    console.log('6. declares knowledge graph expectation without runtime/autonomous retrieval:', hasGraph);

    // 7. declares BoK expectation without runtime or automatic promotion
    const hasBok = r1.bok_expectation && r1.bok_expectation.placeholder === true && r1.bok_expectation.bok_runtime_implemented === false && r1.bok_expectation.automatic_pattern_promotion === false;
    console.log('7. declares BoK expectation without runtime/automatic promotion:', hasBok);

    // 8. declares environment re-check expectation
    const hasEnv = r1.environment_recheck_expectation && r1.environment_recheck_expectation.placeholder === true && r1.environment_recheck_expectation.required_before_knowledge_update === true && r1.environment_recheck_expectation.runtime_implemented === false;
    console.log('8. declares environment re-check expectation before knowledge update:', hasEnv);

    // 9. declares stale-belief policy without opaque compression
    const hasStale = r1.stale_belief_policy && r1.stale_belief_policy.placeholder === true && r1.stale_belief_policy.invalidation_required === true && r1.stale_belief_policy.opaque_compression_forbidden === true;
    console.log('9. declares stale-belief policy without opaque compression:', hasStale);
    ...
    console.log('10. includes proof_gaps and prototype_posture:', hasGaps && hasPosture);
    ...
    const r12a = createMockHybridMissionKnowledgeGraphBok({ requireConsent: HYBRID_MISSION_KNOWLEDGE_GRAPH_BOK_MOCK_CONSENT }, base);
    const r12b = createMockHybridMissionKnowledgeGraphBok({ requireConsent: HYBRID_MISSION_KNOWLEDGE_GRAPH_BOK_MOCK_CONSENT }, base);
    console.log('12. deterministic hybrid_knowledge_boundary_id (excl created_at):', r12a.hybrid_knowledge_boundary_id === r12b.hybrid_knowledge_boundary_id);

    console.log('G47 self-test PASS (local mock, consented, required fields, placeholders, deterministic, no forbidden).');
    process.exit(0);
  } catch (e) {
