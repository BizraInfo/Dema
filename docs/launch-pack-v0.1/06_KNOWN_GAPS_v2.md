...
| 9   | **Operator-side env-hygiene check for DEMA\_\* environment variables** (`dema doctor --env-hygiene`) | Issue #56 (operator-side · `~/.bashrc` leak workaround)                  | ❌                                                                                                 | Phase 1 (Days 1-30 · operator-side)      |
...
| 3   | **SAT-5 schema canon drift** between `bizra-omega/.../genesis.rs` (Validator/Oracle/Mediator/Archivist/Sentinel) and Dema `dema ambient audit` output (SAT-Orchestrator/Policy/QualityOps/Resource/GlobalVerifier) | Both schemas in source · neither superseded · per-runtime vocabulary canon allows this                   | RESOLVED 2026-07-02: canonical SAT-5 = Validator/Oracle(FROZEN)/Mediator/Archivist/Sentinel (topology canon). Mapping: Orchestrator→Validator, Policy→Oracle, QualityOps→Mediator, Resource→Archivist, GlobalVerifier→Sentinel. Ambient/registry role names are historical variants; code reconciliation deferred (don't-refactor). |
