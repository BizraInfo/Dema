...
- [x] Public bridge spec exists before any public connection. (`docs/NODE0_PUBLIC_BRIDGE_SPEC_v0_1.md`, operator ruling 2026-08-03: v0.1 public surface is **read-only proof artifacts only** — content hashes, receipts, witness script — and **no queryable API**. Written while nothing is connected, which is what "before any public connection" requires. The spec forbids publishing environment-bound values as constants, and forbids presenting Node0 identity as clean while the 2026-07-21 signing key remains unrotated.)
- [x] Node1 handshake preview exists before Node1. (`docs/NODE1_HANDSHAKE_PREVIEW_v0_1.md`, operator ruling 2026-08-03: **Node1 is a second sovereign human** — one human, one node; the operator's second machine is estate, never a node. Four-step preview, consent-gated on both sides, `PREVIEW_ONLY`. Non-federation is the default and requires its own separate consent: acknowledge, then decide. No handshake is implemented and none may be attempted before TASK-029.)
