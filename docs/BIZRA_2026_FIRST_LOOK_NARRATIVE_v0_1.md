# BIZRA 2026 First Look Narrative v0.1

> "The 2026 First Look Narrative is not the launch of the full BIZRA forest. It is the first public-facing narrative blueprint for showing the seed honestly: what began, what was built, what is proven, what is designed, and what remains future work."

> "The purpose of this document is to make the first public face beautiful without allowing beauty to outrun truth."

## 1. Purpose

This document is the **claim-governed narrative blueprint** for BIZRA's first public face in 2026. It is not a campaign launch. It is not a video. It is not Canva work. It is the canonical text that every future First-Look artifact (deck, video script, landing page, lighthouse invitation, social opener, Canva slide, visual-emulator caption) **must** consult and obey.

The intended reader is any future contributor — designer, narrator, marketer, copywriter, video producer — who will turn the seven-pillar canon already on `main` into a public-facing artifact. This doc tells them **what they may say, what they must show, what they may hint, and what is forbidden**.

## 2. Truth Label

```text
DECLARED_BIZRA_2026_FIRST_LOOK_NARRATIVE_v0_1
```

This label means: the scenes, evidence anchors, allowed wording, forbidden mutations, and verification checklist are declared and consistent with the seven-pillar canon. **No artifact derived from this doc is automatically public-ready** — every artifact must run the Section 29 Verification Checklist before production.

## 3. Scope

In scope:

- A six-scene narrative arc (Scene 0 → Scene 6, seven scenes total).
- Public-wording boundaries inherited from `CLAIM_REGISTER §9`, Root Source §22, Origin Video §21, Founder Proof §26.
- Visual / audio / on-screen-text direction at the **canon** level (not the production level).
- A verification checklist that any First-Look artifact must satisfy before it leaves Node0.
- The relationship between First Look and the future Visual Emulator + GTM slices.

Out of scope:

- Canva slide production (canon governs; production is a separate slice).
- Video generation (no media binaries committed by this slice).
- Market analysis (separate future slice).
- GTM campaign plan (separate future slice).
- Live token, URP, UKE, PoI, or federation activation.
- Any claim not traceable to the seven-pillar canon.

## 4. Operating Law

```text
Root papers preserve the origin. Origin Video preserves the emotional receipt.
Founder Proof preserves what one seed became. First Look must inherit all three
without exaggerating any of them.
```

```text
No marketing line may outrun the Claim Register.
```

Every paragraph, frame, line, and caption derived from this canon must satisfy both clauses. A beautiful sentence that contradicts a Claim Register row is a defect, not a feature.

## 5. Relationship to Seven-Pillar Canon
...
Every First Look claim **must inherit** from one or more of:
...
- `THREE_REPO_PRODUCT_STACK_CANON_v0_1.md`
- `DELIVERY_SPINE_v0_1.md`

If a candidate claim is **not supported** by these docs, it must be labeled one of:

- `SOURCE_PENDING` — assemblable later; not allowed in the First Look artifact at v0.1
- `DESIGNED_NOT_LIVE` — design exists; may appear only with the qualifier visible
- `FORBIDDEN` — must not appear under any wording

The narrative spine in Section 10 obeys this rule beat by beat.

## 7. Correct Origin Chain

```text
الرسالة + البذرة
→ Arabic root-paper phase
→ July 2023 Drive/screenshot/hash witnesses
→ prompt-engineering exploration
→ parallel crypto/smart-contract research tracks
→ Origin Video 001
→ GitHub / Node0 / Dema
→ future chat-history reconstruction
```

Bound rules:

- The First Look **must not skip** الرسالة + البذرة. The Arabic root-paper phase comes first; the cinematic / GitHub / product surfaces come later.
- Origin Video 001 may be used as **Scene 0's opening bridge** or as Scene 2's anchor — but it does **not** replace the Arabic root papers.
- GitHub / Node0 / Dema are later **engineering proof surfaces**, not the beginning of BIZRA.
- Future chat-history reconstruction is acknowledged as deferred — First Look may say "more is being assembled" but may not show that reconstruction at v0.1.

## 8. What First Look Means

First Look means a specific bounded thing:

- The first public-facing narrative that shows BIZRA honestly, with each element labeled per the seven-pillar canon.
- A narrative that opens with humanity's eternal question (per Origin Video 001's transcript opening), threads through the Arabic root-paper phase, arrives at Founder Proof, and ends with an invitation to choose.
- A blueprint that every later First-Look artifact (deck, video, landing, lighthouse invitation) consults before its first sentence is written.

## 9. What First Look Does Not Mean

First Look does **not** mean:

- A launch announcement.
- A product release.
- A token sale, IDO, IEO, or any economic event.
- A claim that the full BIZRA forest exists today.
- A founder-worship piece.
- A "world's first" or "centralized AI obsolete" pitch.
- A scenario-as-measurement showcase (1M/100M/1B nodes shown as deployed reality).
- A Canva asset, a video file, or any media binary in this repo.

A First-Look artifact that drifts into any of the above fails the Section 29 Verification Checklist.

## 10. Narrative Spine

The First-Look narrative arc is a concise 10-beat sequence inherited from the seven-pillar canon:
...
7. PAT remains local as the user's personal think tank and task force.
8. SAT / UKE / URP are the designed forest/soil layer, **not yet public-live runtime**.
9. The invitation is not "believe the founder." The invitation is **"inspect, test, improve, surpass."**
10. Every human may choose to become a node/seed without surrendering dignity, consent, or memory.

This spine is the load-bearing structure under every scene below. A First-Look artifact that loses any of these beats has drifted from the canon.
...
- The founder's `BizraInfo` GitHub user account holds 156 repositories (142 public + 14 private) as of 2026-05-21.
- The account was created 2024-01-04.
- Dema is the public face / operator doorway of BIZRA Node0.
- All local Node0 state stays under `~/.dema` on the operator's own machine.
- Every consequential action is recorded in a hash-chained local receipt.
- The PAT / SAT / UKE / URP / PoI layers are **designed architecture**, not live runtime.
- The founder is the **first** proof path. The invitation is to inspect, test, improve, surpass.
- Every human may choose to become a node without surrendering dignity, consent, or memory.

A First-Look claim outside this set is treated as `SOURCE_PENDING` until added to this list or the underlying canon.

## 20. Forbidden Claims

Forbidden in every First-Look artifact:

- Do not claim BIZRA is complete.
- Do not claim the full public URP is live.
- Do not claim UKE shared runtime is live.
- Do not claim PoI rewards or token economy are live.
- Do not claim 15,000+ hours as `VERIFIED` unless evidence-bound.
- Do not claim 600 GB+ R&D as `VERIFIED` unless measured.
- Do not claim 5,000+ conversations as `VERIFIED` unless inventory-bound.
- Do not claim exact Ramadan 2023 / June 2023 root-paper dates as `VERIFIED` beyond current evidence labels.
- Do not claim Origin Video 001 proves Node0 implementation.
- Do not claim Founder Proof means founder worship.
- Do not claim "world's first" or "centralized AI obsolete" in public copy.
- Do not claim 1M / 100M / 1B nodes as current reality.
- Do not imply early crypto / smart-contract research means live BIZRA token / economy.

A First-Look artifact containing any of these is a release-blocking defect, not a marketing choice.

## 21. Designed-Not-Live Boundaries

The First Look may **describe** the following only with the `DESIGNED_NOT_LIVE` qualifier visible:

- Public URP — not live; gated behind pilot proof.
- UKE shared runtime — not live; spec only.
- Proof-of-Impact rewards — not live.
- BIZRA dual-token economy — not live; `RESEARCH_QUARANTINE` for value language.
- 1M / 100M / 1B node scenarios — **future emulation / projection**, not current deployment.
...
- **Sacred but not religiously coercive** — the work has spiritual roots (الرسالة, البذرة, Islamic finance principles as design constraints); the visual treatment must honor that without proselytizing or implying religious authority that does not exist.
- **Cinematic but evidence-bounded** — beauty allowed; beauty must serve the proof, not replace it.
- **Futuristic but local-first** — show the home base before the forest; the operator's machine before the network.
- **Human-first, not token-first** — humans appear before any economic surface; tokens (when shown at all) appear only as `DESIGNED_NOT_LIVE` framing.
- **Seed / home base / soil / forest metaphor** — the canonical visual lexicon. A seed grows into a home base; many home bases share soil; the soil supports a forest; the forest is the future state.
- **MMORPG / home-base atmosphere only as explanatory metaphor**, not literal game claim. A "home base" visual aid is allowed; a "BIZRA the game" framing is forbidden.

## 23. Audio / Voice Direction

- **Calm, reflective, cinematic opening** — matches Origin Video 001's tone (per its `§10 Known Audio Direction`).
- **Gradual transition from origin to proof** — no abrupt "and now the product" pivot.
...
## 25. Public Wording Blocks

Three drafts. All labeled `DECLARED_PUBLIC_WORDING_DRAFT`. **Not final marketing copy.** Each must run the Section 29 Verification Checklist before any external use.
...
- Calling the founder "the visionary," "the genius," or any other final-authority noun. The founder is the **first** proof path; that wording is non-negotiable.

## 26. Visual Emulator Relationship

The First Look may **later feed** the Visual Emulator (`docs/public/BIZRA_VISUAL_EMULATOR_SPEC_v0_1.md`, future slice). Bound rules:

- The visual emulator must display **verified current facts** (Node0 Founder Proof state, single human, single node, hash-bound artifacts) **separately from future projections** (5-node demo, 1M / 100M / 1B node scenarios).
- Current Node0 proof, 5-node future demo, and 1M / 100M / 1B node scenarios **must be visually separated** by truth labels (`VERIFIED` / `DESIGNED_NOT_LIVE` / `SCENARIO`).
- Projection screens **must say** "projection" or "simulation" or "scenario" — never "deployment" or "live" or "currently running."

When the visual emulator spec lands, this section becomes its content anchor.

## 27. GTM Relationship

The future GTM master plan (`docs/market/BIZRA_GTM_MASTER_PLAN_v0_1.md`) inherits the First Look's:

- Narrative spine (Section 10).
- Allowed claims (Section 19).
- Forbidden claims (Section 20).
- Public wording blocks (Section 25).

A GTM artifact that violates any of these is revised before any external audience sees it. The GTM doc is not a redo of First Look; it is the **distribution layer** that carries First-Look material into channels.

## 28. Canva / Media Production Boundary

- This document **does not** create Canva assets.
- This document **does not** generate video.
- This document **does not** commit media binaries.
- This document **is** the narrative canon that future Canva / video / image work must obey.

Any Canva or video production slice must:

1. Inherit the Claim Register's truth labels (§8 of the Register).
2. Inherit this doc's allowed wording (Section 19) and forbidden wording (Section 20).
3. Run the Section 29 Verification Checklist before each artifact ships.
4. Cite the specific scene (0-6) and the evidence anchor (Section 18) used.

Media production lives in a separate slice; canon lives here.

## 29. Verification Checklist Before Production

Before any First-Look artifact ships externally, the producer (operator or designated reviewer) confirms each of the following:

- [ ] Every claim in the artifact traces to a row in the Section 18 Evidence Anchor Map.
...
- [ ] The artifact's source paragraphs cite their canon docs in a producer-side notes file (not necessarily visible on-screen).
- [ ] The operator has typed explicit consent for the artifact to ship externally (per ADR-005 + Delivery Spine release receipt template).

A First-Look artifact that fails any item is revised or held; never shipped with an unchecked box.

## 30. Deferred Work

Named and deferred, per `feedback_incremental_evidence_binding`:

- **`docs/public/BIZRA_VISUAL_EMULATOR_SPEC_v0_1.md`** — the emulator spec that this doc's Section 26 anchors.
- **`docs/market/BIZRA_GTM_MASTER_PLAN_v0_1.md`** — the distribution plan that inherits this doc's narrative spine.
- **`docs/public/BIZRA_2026_FIRST_LOOK_NARRATIVE_v0_2_PRODUCTION_RECEIPT.md`** — a future v0.2 that, when actual First-Look artifacts ship, records which canon paragraphs each artifact used + the typed consent receipt.
- **Canva / media production** — outside this slice entirely; future slices may produce specific media artifacts under this doc's discipline.
- **Visual Emulator scenes for 5-node / 1M-node / 100M-node / 1B-node scenarios** — each `SCENARIO`-labeled; framework deferred to the emulator spec slice.
- **Lighthouse-pilot invitation copy** — may reuse Section 25 blocks; the invitation slice itself is separate.
- **Founder-proof v0.2+** — when corpus-consolidation evidence binds the hours / GB / conversation figures, this doc's Allowed Claims (Section 19) will be updated to reflect the promoted labels.

Each deferred item consults this doc before its first sentence is written.

## 31. Next Canon Slices

This First Look Narrative depends on and points forward to:

- `docs/THREE_REPO_PRODUCT_STACK_CANON_v0_1.md` — repo authority.
- `docs/NODE0_DEMA_COMPLETE_COMPONENT_DNA_v0_1.md` — component status labels (every `DESIGNED_NOT_LIVE` row binds this doc's Designed-Not-Live boundaries).
...
- `docs/BIZRA_ORIGIN_VIDEO_001_CANON_v0_1.md` — Scene 0 and Scene 2 anchor.
...
- Future `docs/market/BIZRA_GTM_MASTER_PLAN_v0_1.md`.
- Future Lighthouse-pilot invitation slice.

The load-bearing surfaces of this canon are Section 10 (Narrative Spine), Sections 11-17 (the seven scenes), Section 18 (Evidence Anchor Map), Section 20 (Forbidden Claims), Section 25 (Public Wording Blocks), and Section 29 (Verification Checklist). The other sections explain and bound them.

When any pillar in the seven-pillar canon changes, this doc is re-read for drift. When this doc changes, every deferred First-Look artifact in Section 30 is re-read for drift.
