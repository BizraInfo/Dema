...
- [02-architecture/NODE0_OSTREE_TAD_v0_1.md](02-architecture/NODE0_OSTREE_TAD_v0_1.md) — [DECLARED] OSTree-model technical architecture: maps the content-addressed signed receipt chain / Block0 to OSTree (object store, commit, fsck, deployment), then proposes the stdlib composition manifest before any libostree adoption.
...
- [canon/BIZRA_CONSTITUTIONAL_POSTURE_ON_TRUTH_SERVICE_AND_CHOICE.md](canon/BIZRA_CONSTITUTIONAL_POSTURE_ON_TRUTH_SERVICE_AND_CHOICE.md) — Binding (declared constitutional doctrine, truth class `CONSTITUTIONAL_DECLARATION`; repository lifecycle externalized — derive promotion state from Git history, GitHub metadata, and the latest event receipt) — posture on truth, service and choice: principled fallibilism, ten invariants, law of separation, exit materiality, conflict-of-interest & reward law, per-clause enforcement records (base state + scope + evidence + known gap). Not runtime-loaded by Dema (`DEMA_RUNTIME_AWARE = NO`).
- [00-product-thesis/NODE0_IGNITION_1A_IGNITION_PACK.md](00-product-thesis/NODE0_IGNITION_1A_IGNITION_PACK.md) — Working artifact (`DESIGNED_NOT_LIVE / PLANNING_ONLY`) — G0 Genesis Convergence ignition pack: 12 role cards, service cards, mission/permit schema, Genesis Twin Slice, bounded absence-batch contract, four receipt schemas. References the posture by path + content hash; the implementation is unbuilt and execution requires a separate exact human GO. No runtime loads this document.
...
- [demo/bizra-doxology-gate-proof-cockpit.html](demo/bizra-doxology-gate-proof-cockpit.html) — Current local demo artifact: single-file offline cockpit for exact micro-consent, executable doxology gates, SHA-256 receipt chaining, and `READY_LOCAL` demonstration. No runtime, no network, no persistence, no live mint.
...
- [receipts/BIZRA_GENESIS_CONVERGENCE_CANON_0A.md](receipts/BIZRA_GENESIS_CONVERGENCE_CANON_0A.md) — Historical (event receipt) — preservation of the posture + ignition pack as canon candidates; evidence sources labeled `UNVERIFIED_CONTEXTUAL_INPUT`. Records events, not runtime capability.
...
- [audits/BIZRA_GENESIS_NODE_TECHNICAL_ASSESSMENT_2026_07_16.md](audits/BIZRA_GENESIS_NODE_TECHNICAL_ASSESSMENT_2026_07_16.md) — Current evidence-based architecture, security, reliability, performance, documentation, testing, dependency, and delivery assessment; records the FDE boundary-precedence hardening and the P0 consent-nonce/check-exit blockers.
