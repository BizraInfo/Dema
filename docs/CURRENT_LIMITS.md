...
| Local LLM adapter — localhost-only Ollama invocation (model-broker-local-invocation v0.1) | `packages/core/src/llm-adapter.js` `invokeLocalLLM()` — invokes a localhost-only Ollama model at `http://localhost:11434/api/generate` under exact-string consent · 6 sequential gates (localhost · model-whitelist · prompt-empty · prompt-too-long · consent-exact-string · consent-freshness · inbound-Layer-1-scan) · bidirectional Layer 1 safety scan (inbound prompt + outbound response with REDACTED preview on hit) · `AbortController` 60s default timeout · canonical 16-key **runtime-emission** sibling boundary distinct from preview-boundary (6 keys may be `true` only on MEASURED truth_label) · `verdict_role: "suggestion"` enforced by schema · monotonic per-process replay-resistant consent. Result envelope `bizra.dema.llm_invocation_result.v0.1` (structurally validated by Layer 1 envelope-schema-validator). ADR-018 records the design. Tests: `tests/llm-adapter.test.js` (36 total · +8 new for the ADR-018 surface) · `tests/preview-boundary.test.js` (16 · runtime-emission-boundary sibling covered). |
