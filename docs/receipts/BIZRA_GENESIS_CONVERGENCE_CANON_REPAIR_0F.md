...
| Repair-attestation commit (0F, this receipt) | not self-claimed (next commit on the branch) |
...
3. **Enforcement labels registered** — `ENFORCED_AND_MEASURED_VIA_CONSUMER_TESTS` and `ENFORCED_AND_MEASURED_WITHIN_SANDBOX_BOUNDARY` are defined in the legend as scope-qualified (strictly narrower) states; the slash-pair `A / B` convention is defined. (Copilot ×2 · CodeRabbit Major)
