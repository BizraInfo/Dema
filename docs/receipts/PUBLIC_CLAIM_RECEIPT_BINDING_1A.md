...
| Error handling/observability | Invalid manifests exit `1`; valid-but-open closure exits `2` under `--require-closed`. A stale observation is routed through the *invalid* axis, so once past the declared bound the command exits `1`, not `2` | Distinguishes defect from honest blocker | No continuous monitor; staleness and malformation share one exit code, so callers must read `violation` to tell them apart | Add scheduled read-only scan only after authority |
...
3. **A dedicated non-zero closure command is the operator spearpoint.**
   `npm run claims:receipt-binding:require-closed` refuses closure and names the
   blockers. It exits `2` on the claim axis while the observation is fresh, and
   `1` on the staleness axis once the observation passes the declared bound —
   non-zero either way, which is the property the spearpoint depends on.
...
| Command | Result |
| --- | --- |
| `node --test tests/public-claim-receipt-binding.test.js` | 17/17 pass (`2026-08-09`; was 8/8 before the observation-freshness tests were added) |
| `node --test tests/public-claim-receipt-binding.test.js tests/integration-check.test.js tests/check-exit-integrity-adversarial.test.js` | 41/41 pass (`2026-08-09`; was 32/32 for the same reason) |
| `npm run claims:receipt-binding` | Exit 0; manifest valid, closure blocked |
| `npm run claims:receipt-binding:require-closed` | **Exit code is a function of observation age, not a constant.** Exit `2` (`closure_ready=false`, five unbound IDs) while the observation sits inside the declared `--max-age-hours` bound; exit `1` (`stale_observation`, which sets `manifest_valid=false` and short-circuits before the closure check) once it is beyond. `2026-08-09` re-run: exit `1` at `110.2h` against the shipped `24h` bound. The `2026-08-04` reading of exit `2` was correct at `0h` and is not reproducible today by design. |
| `npm test` | Clean extraction of this slice alone: `8555` tests, `8553` pass, `2` fail — both `EROFS` writing under a read-only `$HOME`, environmental. The operator sandbox additionally fails `21` git-dependent tests because `.git/config.worktree` is permission-denied. **Failures attributable to this slice: `0`** (control vs treatment extraction of `53e636c`: `0` new, `0` gone, `+17` tests all passing). |
