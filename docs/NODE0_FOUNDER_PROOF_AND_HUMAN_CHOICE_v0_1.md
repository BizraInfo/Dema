# Node0 Founder Proof and Human Choice v0.1

> "The Node0 Founder Proof does not claim that BIZRA is complete. It shows that one human seed has become inspectable, testable, extendable, and ready for others to improve."

> "The purpose of this document is to preserve the proof of human choice without turning the founder into the final authority."

## 1. Purpose

This document is the canonical **founder-proof** record for Node0. It captures what one human seed has already made inspectable today, with each claim carrying the truth label its evidence path actually supports. It is the public-facing answer to "is there a real human behind BIZRA?" — answered without overclaim, without inflation, and without surrendering the founder's voice to hype.

The intended reader is any third party — a Lighthouse pilot operator, a reviewer, an investor of attention, a fellow builder — who needs to confirm:

- a human is here, not a marketing front,
- the work is inspectable, not narrated,
- the evidence chain is honest about what is verified vs. attested vs. pending,
- the founder is the **first** proof path, not the **final** authority.

## 2. Truth Label

```text
DECLARED_NODE0_FOUNDER_PROOF_AND_HUMAN_CHOICE_v0_1
```

This label means the v0.1 evidence binding is declared and consistent with the four architect-locked evidence laws (see Section 10). v0.2+ slices will extend the evidence chain as the home-base corpus is consolidated by Dema and as additional cross-provider witnesses are bound.

## 3. Scope

In scope:

- Public-facing founder-proof claims, each bound to a truth label per its current evidence.
- The GitHub account evidence (`BizraInfo`).
- The local cloud-archive corpus evidence (size + 2023-dated counts from disk probe).
- Reference to Origin Video 001 as the pre-technical artifact already canon-bound.
- The explicit bounding language for "15k+ hours", "600 GB+ R&D", "5k+ AI conversations", "no founder token minted", and "no formal technical background before Ramadan 2023".
- The named v0.2+ verification roadmap.

Out of scope (deferred — see Section 25):

- Manual consolidation of the home-base corpus across 11 cloud mirrors (~507 GB). That is a Dema feature, not a documentation slice.
- Full normalized AI-conversation export across providers.
- Full deduplicated R&D corpus measurement.
- Ledger absence-proof for the no-founder-token claim.
- Education / employment history evidence for the no-formal-technical-background claim.

## 4. Operating Law

```text
One human can start a seed. The forest belongs to everyone who chooses to nurture it.
```

```text
Founder proof is not founder worship. It is the first inspectable path others may test, reject, improve, or surpass.
```

Every claim in this doc obeys both clauses. A claim that elevates the founder to final authority is a defect. A claim that diminishes the inspectability of what has actually been built is also a defect.

## 5. Relationship to Root Source of Truth

`docs/BIZRA_ROOT_SOURCE_OF_TRUTH_v0_1.md` Section 11 lists "Node0 Founder Proof" as one element of the BIZRA origin chain. This doc is the binding **expansion** of that Section 11.

- **Root Source** says what Node0 Founder Proof *means* (the founder is the first proof path, not the final authority).
- **This doc** binds the *current evidence* behind that meaning with explicit truth labels.

When Root Source's framing or wording changes, this doc is re-read for drift. When this doc's evidence labels are promoted (v0.2+), Root Source's Section 11 is updated to match.

## 6. Relationship to Claim Register

`docs/CLAIM_REGISTER_v0_1.md` §9 carries the **Founder proof** row in the Public Claim Table. This doc is the binding expansion of that row. §17 of the Claim Register sets the **dual-label discipline for founder-proof claims**: numeric figures (hours, repos, GB, conversations) are `DERIVED` if a repo-bound evidence path is named, `OPERATOR_ATTESTED` / `SOURCE_PENDING` otherwise.

This doc honors that discipline strictly: every numeric or attestational claim below carries a precise label per its current evidence path.

When this doc's labels change in v0.2+, the Claim Register §9 row is updated to match.

## 7. Relationship to Origin Video 001

`docs/BIZRA_ORIGIN_VIDEO_001_CANON_v0_1.md` (with v0.2 evidence binding) preserves the pre-technical voice of BIZRA from **2023-08-31** (filename-embedded ms timestamp, `DATE_DERIVED_SINGLE_WITNESS`). It bridges the gap between operator-attested 2023 work and the 2024-01-04 GitHub account creation.

Origin Video 001 is the **earliest hard-dated** evidence of BIZRA intent on disk. Founder Proof v0.1 references it as the pre-GitHub anchor without duplicating its binding.

## 8. What Founder Proof Means

Node0 Founder Proof means a specific bounded thing:

- A single human, on a single machine, with a single transparent codebase, has carried the BIZRA seed far enough that it is **inspectable** (any third party can read the code, the docs, and the receipts), **testable** (any third party can clone, run gates, and verify), and **extendable** (any third party can fork, propose, contribute).
- It does **not** mean BIZRA is complete.
- It does **not** mean Node0 proves the future forest.
- It does **not** mean the founder is the final authority.
- It does **not** mean the work is finished.

Founder Proof is the **floor**, not the ceiling. The floor exists. The ceiling is for the forest to decide.

## 9. What Human Choice Means

Every human reading this document is invited to **choose**:

- to inspect the proof,
- to reject it,
- to improve it,
- to become a node themselves on their own home base,
- to extend the seed into a forest under their own consent, on their own receipts, with their own memory.

Human choice means **no surrender of dignity, no surrender of consent, no surrender of memory** when entering the BIZRA design. The forest is opt-in. The seed is given freely.

## 10. Evidence Label Method

Every claim in this doc carries exactly one of:

| Label | Meaning |
|---|---|
| `VERIFIED` | Verified by mechanism reachable from this repo or its external witnesses (GitHub API, disk probe, hash binding). Anyone can reproduce. |
| `DERIVED` | Logically follows from a `VERIFIED` witness with the derivation made explicit. |
| `OPERATOR_ATTESTED` | The founder has stated it; no third-party-verifiable evidence path is named at v0.1. Reusable in public language only when paired with the label. |
| `SOURCE_PENDING` | Known to exist or be assemblable but not yet bound to inspected evidence in this repo. A future slice will bind it. |
| `DESIGNED_NOT_LIVE` | Spec / design exists; no runtime. Used here only when the claim depends on a `DESIGNED_NOT_LIVE` component layer (per Component DNA). |
| `FORBIDDEN` | A claim that may not be made anywhere on public surfaces under any wording. |

This doc binds to four architect-locked evidence laws preserved in memory canon:

- `feedback_no_invented_evidence_source` — no convenience witnesses
- `feedback_incremental_evidence_binding` — canon grows v0.1 → v0.N; never collapse "provable later" into "proven now"
- `feedback_post_ramadan_2023_default_relevance` — every operator-mirror file dated 2023-03-22 onwards is presumed BIZRA-relevant by default
- `project_2026_05_21_home_base_consolidation_deferred_to_dema` — corpus consolidation is a Dema feature, not a manual slice

## 11. GitHub Account Evidence

Captured via `gh api users/BizraInfo` and the operator-provided 2026-05-21 profile screenshot. Two independent witnesses agree on every field below — both citable.

| Fact | Value | Label |
|---|---|---|
| Account login | `BizraInfo` | `VERIFIED` |
| Account type | **User** (not Organization — common-misread point; see Section 24) | `VERIFIED` |
| Account name | `BIZRA` | `VERIFIED` |
| Account `created_at` | **2024-01-04T15:57:05Z** | `VERIFIED` |
| Public bio / tagline | *"Every human is a node, and every node is a seed, and every seed has infinite potential."* | `VERIFIED` |
| Total repositories | **156** | `VERIFIED` |
| Public repositories | **142** | `VERIFIED` (anyone can confirm via `gh api users/BizraInfo` or browser) |
| Private repositories | **14** | `VERIFIED` (requires authenticated view; cross-witness with operator screenshot) |
| Followers | 15 | `VERIFIED` |
| Following | 70 | `VERIFIED` |
| Contributions in the last year | 2,260 (per profile screenshot graph) | `VERIFIED` from screenshot witness; `DERIVED` until re-confirmed via `gh api` events |
| Activity composition | ~89% Commits / 3% Code review / 1% Issues / remainder other (per profile screenshot) | `DERIVED` from screenshot witness |
| Earliest repo on this account | `open-interpreter` at 2024-01-18 (a **fork**, not an original BIZRA repo) | `VERIFIED` |
| Latest repo on this account | `the-verifier-agent` at 2026-05-05 | `VERIFIED` |

**Important precision**:

- GitHub activity on `BizraInfo` begins **January 2024**. The broader BIZRA evidence chain reaches back into 2023 through Origin Video 001 (2023-08-31 hard date from filename ms timestamp) and through ~110,000 files dated 2023 in operator-owned cloud mirrors (see Section 12).
- **Do not** say "3 years on GitHub." Say: *GitHub activity on this account begins in January 2024, while the broader BIZRA evidence chain reaches back into 2023.*

## 12. Local Corpus Evidence

Captured 2026-05-21 by a `find ... -newermt '2023-01-01' ! -newermt '2024-01-01'` probe across operator-owned cloud-archive mirrors at `/data/bizra/cloud-archive/`. These counts are `DERIVED` from the probe; they are not a final audited corpus measurement.

| Mirror | 2023-dated files | Label |
|---|---|---|
| `onedrive-beshr000` | **72,023** | `DERIVED` (from disk probe; reproducible by re-running the same `find` command) |
| `gdrive-beshr000` | **24,361** | `DERIVED` |
| `onedrive-wizard` | **13,350** | `DERIVED` |
| `gdrive-bizra-info` | **503** | `DERIVED` |
| Other operator mirrors (small subsets) | ~3 + ~3 (gdrive-beshr0000, beshr000-duplicates) | `DERIVED` |
| **Sampled total** | **~110,237 files dated 2023 across these mirrors** | `DERIVED` |
| Total cross-mirror disk size (all dates) | ~507 GB across 11 mirrors | `DERIVED` (from `du -sh`) |

> *These counts support the existence of a large 2023 local/cloud research corpus. They do not yet prove final deduplicated corpus size, exact total R&D GB, or exact conversation count.*

Specific 2023 witnesses already identified in the probe:

- `Screenshot 2023-11-05 025643.png` — hard 2023 date in filename.
- `chatGPT data/` folder in `gdrive-beshr000/Colab Notebooks/` — AI-conversation archive material.
- `البذرة.pdf` mirrored in `onedrive-beshr000` — the foundational seed PDF (also canon-bound at the repo root).
- `bizra-normalizers/normalizers/chatgpt.py` in repo tooling — the parser the founder has already built to consume cross-provider chat history.

## 13. Origin Artifact Evidence

The pre-GitHub evidence anchor is **Origin Video 001**, already preserved and evidence-bound in `docs/BIZRA_ORIGIN_VIDEO_001_CANON_v0_1.md` with v0.2 evidence binding:

- Primary artifact candidate is hash-bound (`sha256: 8b89b6dd…3d5af`, ~143 MB, located in the `gdrive-wizard` local mirror).
- Filename-embedded ms timestamp `1693522543490` decodes to **2023-08-31 22:55:43 UTC**. Carries label `DATE_DERIVED_SINGLE_WITNESS`.
- Production tool: Google AI Studio (label `TOOL_PROVENANCE_DERIVED_STRONG`).
- Pristine-original status remains `PRISTINE_ORIGINAL_SOURCE_PENDING`.

This anchors the pre-2024-01 portion of the founder evidence chain. Founder Proof v0.1 does not re-bind these claims; it cites the Origin Video canon as the authoritative source.

## 14. Time and Effort Claims

| Claim | v0.1 label | Allowed wording |
|---|---|---|
| "3 years solo work" | `DERIVED` for the broad project-origin window (Ramadan 2023 → today is ~3 years 2 months elapsed, anchored by Origin Video 001 + local 2023 corpus + 2024+ GitHub activity) | "BIZRA's development arc spans the period from Ramadan 2023 to today, carried by a single human, with witnesses graduated by period: pre-2023-08 = operator-attested + cloud-mirror dating; 2023-08+ = Origin Video 001 hard date; 2024-01+ = `BizraInfo` GitHub account." |
| "15,000+ hours" | `OPERATOR_ATTESTED` | "The founder attests to ~15,000+ hours of solo work; precise hours are operator-attested and not yet derived from time logs, calendar records, or commit-time aggregation." |
| Precise hours figure as `VERIFIED` | **forbidden in v0.1** | — |

A future v0.2+ slice may promote the hours claim from `OPERATOR_ATTESTED` toward `DERIVED` via commit-time aggregation across the BizraInfo account, calendar-export evidence, or activity-aggregation tooling.

## 15. Repo and Code Surface Claims

| Claim | v0.1 label | Allowed wording |
|---|---|---|
| "156 total repositories under `BizraInfo` GitHub account" | `VERIFIED` | "The founder's `BizraInfo` GitHub account holds 156 total repositories (142 public, 14 private) as of 2026-05-21." |
| "142 public repositories" | `VERIFIED` (anyone-viewable) | "142 of those repositories are public and inspectable by any third party." |
| "155+ repos" (per architect's queued draft) | `VERIFIED` if paired with the 156-repo binding | "155+ repos" is acceptable shorthand only when tied to the underlying 156-repo evidence. |
| "All repos are production-ready" | **forbidden** | — |
| "All repos are active runtime authority" | **forbidden** | — |

## 16. R&D Data Claims

| Claim | v0.1 label | Allowed wording |
|---|---|---|
| "600 GB+ of R&D data" | `OPERATOR_ATTESTED` / `SOURCE_PENDING` | "The founder's home-base cloud-archive mirrors total ~507 GB of operator data across 11 source mirrors (per `du -sh` probe 2026-05-21). The post-Ramadan-2023 subset (BIZRA-relevant by default per `feedback_post_ramadan_2023_default_relevance`) is the candidate for a `~600 GB+ R&D` claim; final deduplicated, audited size remains source-pending until Dema's home-base consolidation feature ships." |
| Exact deduplicated R&D total as `VERIFIED` | **forbidden in v0.1** | — |

## 17. AI Conversation Claims

| Claim | v0.1 label | Allowed wording |
|---|---|---|
| "5,000+ AI conversations across providers" | `OPERATOR_ATTESTED` / `SOURCE_PENDING` | "The founder attests to 5,000+ AI conversations across providers (ChatGPT, Claude, Google AI Studio, others) spanning multiple email accounts. Existing repo tooling (`bizra-normalizers/normalizers/chatgpt.py`) provides the parser infrastructure for a future evidence-binding slice. Full normalized inventory is deferred to a Dema home-base consolidation slice." |
| Exact conversation count as `VERIFIED` | **forbidden in v0.1** | — |

Per `feedback_post_ramadan_2023_default_relevance`, **every** AI conversation in operator-owned exports from 2023-03-22 onwards is presumed BIZRA-relevant by default — keyword filtering for "bizra" understates the true corpus.

## 18. No-Token Claim Boundary

| Claim | v0.1 label | Allowed wording |
|---|---|---|
| "The founder has not minted a token for himself" | `OPERATOR_ATTESTED` | "The founder attests that no founder-allocation token has been minted, and that BIZRA has no live token at this stage. Verification requires a ledger absence-proof (a public on-chain or off-chain audit showing no founder mint occurred) and is source-pending." |
| Live tokenomics, founder mint, or any token economic claim | **forbidden** per `CLAIM_REGISTER §13` (Economic and Token Claims) | — |

## 19. No-Formal-Technical-Background Claim Boundary

| Claim | v0.1 label | Allowed wording |
|---|---|---|
| "No formal technical background before Ramadan 2023" | `OPERATOR_ATTESTED` | "The founder attests to having no formal technical background prior to Ramadan 2023 (Gregorian 2023-03-22 onwards). Verification would require education / employment history evidence and is source-pending." |
| Precise prior background claim as `VERIFIED` | **forbidden in v0.1** | — |

This is part of the founder narrative arc — that the seed grew from scratch under one human's choice — but it is **not** to be cited as verified fact without supporting evidence.

## 20. What Is Verified

`VERIFIED` today, with evidence reproducible from this repo or its external witnesses:

- All GitHub account facts in Section 11 (account login, type, name, `created_at`, bio, repo counts, follower/following, earliest/latest repo).
- Origin Video 001's `FILE_IDENTITY_VERIFIED` row (path, size, sha256) — already bound in `BIZRA_ORIGIN_VIDEO_001_CANON_v0_1.md §22 Evidence Binding v0.2`.
- This canon document exists at `docs/NODE0_FOUNDER_PROOF_AND_HUMAN_CHOICE_v0_1.md` and is bound by the Delivery Spine release gates.
- The four architect-locked evidence laws exist in memory canon (`feedback_no_invented_evidence_source.md`, `feedback_incremental_evidence_binding.md`, `feedback_post_ramadan_2023_default_relevance.md`, `project_2026_05_21_home_base_consolidation_deferred_to_dema.md`).

## 21. What Is Derived

`DERIVED` today from a named `VERIFIED` witness with the derivation made explicit:

- "BIZRA development arc spans Ramadan 2023 → today" (~3 years 2 months) — derived from Origin Video 001 (`DATE_DERIVED_SINGLE_WITNESS`) + 2023-dated mirror probe + 2024-01-04 GitHub account creation.
- All 2023-dated file counts in Section 12 — derived from the disk-probe `find` command.
- "~507 GB total cloud-archive volume" — derived from `du -sh`.
- "Solo-ship pattern" (89% Commits / 3% Code review / 1% Issues) — derived from GitHub profile activity overview.

## 22. What Is Operator-Attested

`OPERATOR_ATTESTED` today — stated by the founder, not yet verified by independent witness:

- "15,000+ hours of solo work."
- "5,000+ AI conversations across providers."
- "600 GB+ R&D data" (the architect-queued figure; current measured volume is ~507 GB total cross-mirror, of which the post-Ramadan-2023 BIZRA-relevant subset is the candidate).
- "No founder-allocation token has been minted."
- "No formal technical background before Ramadan 2023."
- "All operator-mirror files dated 2023-03-22 onwards are BIZRA-relevant by default" (this is itself architect-locked in `feedback_post_ramadan_2023_default_relevance`; the inversion is operator-decreed).

Each of these is reusable in public language only when paired with the `OPERATOR_ATTESTED` label or equivalent qualifier (e.g., "the founder attests …").

## 23. What Is Source-Pending

`SOURCE_PENDING` today — assemblable, but the assembly work has not yet been done in this repo:

- Time-log / calendar / commit-time aggregation for the hours claim.
- Full deduplicated post-Ramadan-2023 corpus size (the `~600 GB+ R&D` precise number).
- Full normalized AI-conversation count across providers.
- Ledger absence-proof for the no-founder-token claim.
- Education / employment history evidence for the no-formal-technical-background claim.
- Cross-provider chat history binding via `bizra-normalizers/normalizers/chatgpt.py` (and equivalent parsers for Claude, Google AI Studio).
- Full deduplicated home-base corpus tree (deferred to Dema's consolidation feature).

Each `SOURCE_PENDING` item is named in Section 27's Verification Roadmap v0.2+.

## 24. What Must Not Be Claimed Yet

Forbidden in this canon and in every public artifact derived from it:

- "BIZRA is complete."
- "Node0 proves the entire future forest."
- "15,000+ hours" cited as `VERIFIED` at v0.1.
- "600 GB+ deduplicated corpus" cited as `VERIFIED` at v0.1.
- "5,000+ AI conversations" cited as `VERIFIED` at v0.1.
- "No founder token minted" cited as `VERIFIED` at v0.1 (without ledger absence-proof).
- "All repositories are production-ready."
- "BizraInfo is a GitHub organization" — it is a **user account**.
- "The founder is the final authority" — the founder is the **first** proof path.
- "3 years on GitHub" — GitHub account begins 2024-01-04; the broader chain reaches earlier via Origin Video + local mirrors.
- Live public URP, UKE shared runtime, Proof-of-Impact rewards, or token economy.

A surface that contains any of these is a release-blocking defect.

## 25. Dema Home-Base Consolidation Deferred

> *The home-base corpus should eventually be cleaned, indexed, deduplicated, typed, and made searchable by Dema itself as a Node0-local capability. This document does not manually consolidate that corpus.*

Per `project_2026_05_21_home_base_consolidation_deferred_to_dema` (memory canon, architect-locked):

- This Linux machine is BIZRA's home base local space.
- ~507 GB / ~110K 2023-files corpus is mirrored locally across 11 source mirrors (gdrive-* and onedrive-* under operator email accounts).
- **Consolidation is deferred** until Dema itself can perform it as a Node0 operator capability.
- Reason: don't manually rake what the tool should do; a manual snapshot would drift and would compete with Dema's eventual receipt-bound consolidation.

Future Dema feature stub (to be picked up in a Dema feature slice, not in this canon slice):

```text
dema consolidate home-base
  → preview: enumerate operator-owned mirrors
  → preview: report dup rates, type distribution, size totals
  → typed consent → execute: produce a single deduplicated, type-sorted, indexed tree
  → mint a receipt: every consolidation run produces a hash-chained receipt of inputs + outputs + dedup decisions
  → idempotent on re-run; appends to receipt chain on incremental adds
  Boundary: read-only on source mirrors; writes only to a designated home-base directory; honors PAT / SAT / EffectCap discipline.
```

Founder Proof v0.2+ corpus-volume claims are bound to ship **after** that Dema feature ships.

## 26. Public-Facing Founder Proof Wording

The following short paragraph is `DECLARED_PUBLIC_WORDING` (not final marketing copy; reusable on README, lighthouse invitation, deck opening, 2026 First Look bridge, with the Claim Review Gate from `CLAIM_REGISTER §20` applied each time):

> **One human carried the first BIZRA seed far enough to become inspectable. The proof is not perfection. The invitation is for better, wiser, more capable humans to test what is here, reject what is wrong, improve what is partial, and surpass what is current. Every human may choose to become a node — a seed of their own — without surrendering dignity, consent, or memory.** *(`DECLARED_PUBLIC_WORDING`)*

Permitted variants (same label, same boundary):

- Replace "human" with "person" — both true to operating law.
- Replace "node — a seed of their own —" with "node, with their own seed" — both true.
- Replace "current" with "imperfect" — both true.

Forbidden mutations:

- Removing "the proof is not perfection" or any synonym (this is the anti-worship clause).
- Replacing "every human may choose" with "every human must" or "every human will" (sovereignty is the point).
- Inserting any token, economic, or reward language.
- Inserting "complete", "ready", "final", "perfect" anywhere in this paragraph.
- Inserting any number that is `OPERATOR_ATTESTED` without its label (hours, GB, conversation count).

## 27. Verification Roadmap v0.2+

Named and deferred, per `feedback_incremental_evidence_binding`:

| Slice | Will bind | Target label promotion |
|---|---|---|
| **v0.2 — hours derivation** | Commit-time aggregation across `BizraInfo` repos + activity-aggregation evidence | "15k+ hours" `OPERATOR_ATTESTED` → `DERIVED` |
| **v0.3 — chat-history single-provider binding** | Sample export from ChatGPT data folder + run through `bizra-normalizers/normalizers/chatgpt.py` to produce a structured inventory | Subset of "5k+ AI conversations" `OPERATOR_ATTESTED` → `DERIVED` |
| **v0.4 — cross-provider chat-history binding** | Equivalent normalizers for Claude, Google AI Studio, others | Full "5k+ AI conversations" claim promoted toward `DERIVED` / `VERIFIED` |
| **v0.5 — Dema home-base consolidation** | Awaits `dema consolidate home-base` feature shipping | "600 GB+ R&D" promoted from `OPERATOR_ATTESTED` to `DERIVED` with measured number |
| **v0.6 — ledger absence-proof** | Public on-chain or off-chain audit showing no founder-allocation token mint | "No founder token" `OPERATOR_ATTESTED` → `VERIFIED` |
| **v0.7 — background claim** | Education / employment history evidence (if and only if the operator chooses to publish it) | "No formal technical background" `OPERATOR_ATTESTED` → `DERIVED` or `VERIFIED` (or kept `OPERATOR_ATTESTED` by choice) |

Each slice is small, scoped, and adds binding without revisiting prior bindings.

## 28. Explicit Non-Goals

This slice does **not**:

- manually consolidate the home-base corpus,
- run any `find` or `du` exhaustive traversal beyond the small sample probe already cited,
- normalize any AI conversation export,
- search for a founder-allocation token on any chain,
- write education / employment history claims,
- write market analysis,
- write GTM campaign content,
- create Canva or visual assets,
- mint any receipt,
- activate any token, URP, UKE, PoI, or federation surface,
- create any ADR,
- edit any runtime code, script, package file, or GitHub Actions workflow,
- create a manual `/data/bizra/home-base/` directory (that is Dema's job).

## 29. Next Canon Slices

This Founder Proof depends on and points forward to:

- `docs/BIZRA_ROOT_SOURCE_OF_TRUTH_v0_1.md` — origin chain (this doc is the expansion of its Section 11).
- `docs/CLAIM_REGISTER_v0_1.md` — public-claim authority + §17 Founder Proof Claims dual-label discipline.
- `docs/DELIVERY_SPINE_v0_1.md` — release gates that bind any GTM reuse of this canon.
- `docs/NODE0_DEMA_COMPLETE_COMPONENT_DNA_v0_1.md` — component status; Home Base Consolidation belongs under `Dema Product Face` (Layer 8, `ACTIVE` layer) as a future feature.
- `docs/THREE_REPO_PRODUCT_STACK_CANON_v0_1.md` — repo authority.
- `docs/BIZRA_ORIGIN_VIDEO_001_CANON_v0_1.md` — pre-GitHub anchor (already evidence-bound at v0.2).
- Memory canon: `feedback_no_invented_evidence_source.md`, `feedback_incremental_evidence_binding.md`, `feedback_post_ramadan_2023_default_relevance.md`, `project_2026_05_21_home_base_consolidation_deferred_to_dema.md`.
- Future `docs/public/BIZRA_2026_FIRST_LOOK_NARRATIVE_v0_1.md` — the modern cinematic narrative will combine Origin Video 001's emotional spine with this Founder Proof's evidence floor.
- Future Dema feature slice for `dema consolidate home-base` (Component DNA Layer 8 capability).

The load-bearing surfaces of this canon are Section 11 (GitHub Account Evidence), Section 12 (Local Corpus Evidence), Section 20–23 (the four label inventories), Section 24 (What Must Not Be Claimed Yet), and Section 26 (Public-Facing Founder Proof Wording). The other sections explain and bound them.

When this canon's labels are promoted in v0.2+ slices, the Root Source's Section 11 row and the Claim Register's §9 + §17 rows are updated to match.

---

> **Founder proof is not founder worship. It is the first inspectable path others may test, reject, improve, or surpass.**
> *(Operating-law clause, also fit for public copy when paired with `DECLARED_PUBLIC_WORDING`.)*
