...
|---|---|---|
| **Dema Mission Router** | Mission decomposition + role dispatch; carries the founder's voice; runs the daily stand | Current main anchor: `packages/mission/src/mission-corridor.js` (contract + journal, PREVIEW_ONLY — merged via the reconciled candidate PR #386) + `apps/cli` |
...
| **Checkpoint Service** | Restart, continuation, recovery | Current main anchor: Mission Corridor restart/resume derivation (`packages/mission/src/mission-corridor.js`, PREVIEW_ONLY — merged via PR #386). The Checkpoint *Service* wrapper itself remains `DESIGNED_NOT_LIVE` (S5 build) |
...
**Present half (2026):** recent conversations around Mission Corridor / FATE / receipts / Node0 / Vanishing-Agent / the inference fabric + **one current implementation module** (current-tree candidates, both path-verified: `packages/mission/src/mission-corridor.js` — merged via PR #386 — or `packages/core/src/dema-fde-dual-diagnostic.js`) + its tests + its receipt + one external paper/repo + one bounded reproduction benchmark.
...
**Reuse (exists on disk in the current tree, measured/preview — each path verified):** FATE consent (`fate.js`, MEASURED) · continuity/checkpoint (`packages/mission/src/mission-corridor.js`, PREVIEW_ONLY — merged via PR #386; root-bound consent on every corridor write) · mission replay (`node0-mission-replay-preview.js`) · SAT five-pass judge (`sat5-constitutional-verifier-set-preview.js`, PREVIEW) · reversible action + undo (`node0-reversible-execute-gate.js`, sandbox-MEASURED) · one live local model call (`invokeLocalLLM()`, MEASURED, suggestion-only) · append-only journal (`event-log.js`) · signing crypto (`authorship-signature.js`, ephemeral keys).
