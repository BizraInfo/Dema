...
| **kebab** | `consent-card`, `mission-loop`, `evidence-event`, `node-registry`, `onboarding-lifecycle`, `skill-growth-governor`, `project-status`, `craftsmanship-witness`, `llm-router`, `process-mining`, `key-maker-check`, `llm-invoke` | 12 |
...
2. No new kebab commands beyond the 12-entry legacy allowlist.
