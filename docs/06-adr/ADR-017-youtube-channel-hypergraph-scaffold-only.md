...
But after producing the plan, the operator pivoted: per the 4-source convergence on **model-broker promotion as the flagship bottleneck**, the youtube-hypergraph slice is reclassified as *adjacent capability*, not *load-bearing-on-the-north-star*. Implementation is therefore explicitly deferred.
...
4. **Hypergraph as authority temptation** — Multi-hop query results could be cited as truth. Per ADR-015, the hypergraph is a *suggestion structure*; queries return *advisory* relational context.
