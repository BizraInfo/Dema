...
**Naming precheck (one step, no invention):** run `grep -rn "librarian\|scribe\|lanes" packages/core/src/council-seat-pat-dispatch.js packages/adk/src/ 2>/dev/null | head -20`. If existing PAT/SAT lane names are found, REUSE them verbatim for `role_id`s. If not found, use these defaults — PAT: `pat-librarian`, `pat-researcher`, `pat-planner`, `pat-builder`, `pat-scribe`, `pat-navigator`, `pat-steward`; SAT: `sat-boundary-judge`, `sat-consent-judge`, `sat-overclaim-judge`, `sat-provenance-judge`, `sat-economy-judge`.
