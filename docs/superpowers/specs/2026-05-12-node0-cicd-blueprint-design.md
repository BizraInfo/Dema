...
self_digest:     "<hex string>"         # sha256 of canonicalized payload
...
producer_identity: "<dotted name>"      # MANDATORY for new receipts (e.g. "dema.kernel.assurance.preflight")
...
jobs:
  ...
  consolidate:
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4
        with: { name: ci_drift_report, path: ci_drift_report.json, retention-days: 90 }
```
