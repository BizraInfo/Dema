...
| `dema node-registry` | `packages/core/src/node-registry-preview.js` | Emits Node Registry Preview v0.1e (`bizra.dema.node_registry_preview.v0.1`): the schema-tagged registry that makes the Node ordinal law (canonized 2026-05-18, `docs/canon/BIZRA_TOPOLOGY_CANON.md` §"Node ordinal law") operational. Surfaces accepted nodes + ghost-preview candidates + `next_available_ordinal` + `highest_assigned_ordinal` + `forbidden_ordinals` (3, 4 per canon_registry). Refuses (as data) every ordinal-law violation: forbidden ordinals, duplicates, would-skip-ordinal, ghost-without-candidate-name, active-as-ghost-status, self-referencing `companion_of`, non-integer ordinals. `ordinal_claim_phrase` template `GO accept Node<N> ordinal` enforces ADR-005 exact-string consent (no fuzzy/case-insensitive/prefix match). 8-entry `primary_refusals` taxonomy + `blocked_effects` (federation, raw_data_sharing, public_broadcast, economic_activation, node_connection, ordinal-assignment guards). Canonical 16-key boundary all false. Receipt-shape-ready. 32 tests (16 base + 16 adversarial). |
