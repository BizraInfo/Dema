# BIZRA 2026 First Look Production Brief v0.1
...
This document is the **expression-layer brief** between the eight-pillar canon and any media artifact (Canva slide, video frame, landing page, social teaser, visual emulator screen, lighthouse invitation, founder-story piece).

It exists so that any future contributor — designer, animator, narrator, copywriter, video producer, web builder, social poster — can transform the canonical text into a public artifact **without breaking truth**. Every prompt, every visual instruction, every wording block in this brief inherits from a specific row in the canon and carries a specific truth label.

This brief does **not** produce assets. It produces the **instructions** that any future asset-producing slice must obey.

## 2. Truth Label

```text
DECLARED_BIZRA_2026_FIRST_LOOK_PRODUCTION_BRIEF_v0_1
```

This label means the asset-family briefs (§6-12), the matrices (§13-14), the forbidden lists (§15-16), the truth-label requirements (§17), and the three prompt packs (§19-21) are **declared** and consistent with the eight-pillar canon as of 2026-05-21. No artifact derived from this brief is automatically public-ready — every artifact must run the Production Checklist (§22) and the Review Gate (§23) before leaving Node0.
...
```text
Canon defines truth.
Narrative defines story.
Production brief defines expression.
Media generation comes only after expression is bounded.
```

```text
No asset may show future URP, UKE, PoI, token economy, 1M nodes,
100M nodes, or 1B nodes as live reality.
...
Both laws bind every asset, every prompt, every frame, every caption. A beautiful asset that violates either is a defect, not a feature.
...
## 6. First-Look Video Brief

**Purpose**: a 60-180 second cinematic piece carrying the 7-scene narrative arc from `BIZRA_2026_FIRST_LOOK_NARRATIVE_v0_1.md` §11-17.

**Length tiers**: 30s teaser (Scene 0 + Scene 6 only) · 90s first-look (all 7 scenes compressed) · 3-minute founder-proof (all 7 scenes + extended Scene 3).

**Visual mood**: cinematic, contemplative, scaled-to-human. No Silicon-Valley quick-cuts. No urgency manufactured by edit pace.

**Voice direction**: per First Look §23 — calm, reflective. End with choice, never with CTA-button.
...
**Forbidden**: any video frame showing a "live" forest, a token UI, a node-network heatmap presented as deployed, a price chart, an APY chart, a "Buy now" CTA. Any closing without the choice element.
...
- 4:5 carousel slide
- 1200×630 OG image
- 1500×500 banner (LinkedIn, X)

**Visual lexicon**: seed → home base → soil → forest (per First Look §22). MMORPG / home-base atmosphere only as explanatory metaphor, never as literal game claim.

**Color tone**: sacred but not religiously coercive. Earth tones acceptable. No neon-crypto color palette (no green-candle / red-candle motifs).

**Typography**: Arabic + Latin script support required for any artifact referencing `الرسالة` or `البذرة`. The Arabic text must be reproduced verbatim, never transliterated as substitute.

**Required truth label on any asset that touches a `DESIGNED_NOT_LIVE` element**: visible caption "designed, not live" or "scenario" — Production Checklist §22 enforces this.

**Forbidden**: any asset showing the token economy, URP marketplace, UKE shared memory, PoI rewards, or scale projections **without** the on-screen qualifier. Any asset that omits the choice framing in its closing slide of a series.

## 8. Visual Emulator Brief

**Purpose**: a sandboxed visual that lets a viewer see Node0 + future-forest behavior without a live network. Reference: First Look §26 + Component DNA Layer 18.
...
**Required**: any user-interactive emulator must include an "evidence panel" that names the canon doc + section behind every visible claim.

## 9. Website / Landing Page Brief
...
**Forbidden**: any "Buy / Invest / Join the Token" CTA. Any number presented without its truth label. Any embedded chart that is not a `SCENARIO`-captioned projection.

## 10. Social Teaser Brief

**Purpose**: short-format hooks that lead to the website / landing / video — not standalone product pitches.

**Platforms in scope**: X (Twitter), LinkedIn, Instagram, TikTok (text overlay only — no live token / financial overlay), YouTube Shorts.
...
- No teaser asks the audience to "join" or "follow" anything that doesn't actually exist (no fake mailing list, no fake DAO).

**Forbidden**: hashtag-bombing crypto tags (no `#bizratoken`, `#bizraICO`, `#bizraDAO` unless those exist on canon — none currently do). No FOMO countdowns. No engagement-bait questions disconnected from the canon.

## 11. Private Lighthouse Invitation Brief

**Purpose**: the artifact handed (privately) to a vetted external operator who is being invited to run a Node0 on their own machine as a Lighthouse pilot participant.

**Audience**: single human, known to the founder, who has agreed in principle to operate their own Node0 under the same discipline (PAT/SAT/EffectCap/receipts).
...
- The verification path (how the pilot operator can inspect their own receipts, run the same gates, verify nothing is hidden).
- A typed-consent block for the operator to enter their explicit "GO" before installing.

**Forbidden**: any framing of the pilot as a "token allocation event", a "founders' round", an "early access program with rewards", or any economic framing. The pilot is **operator discipline**, not a product.

## 12. Founder Story Brief

**Purpose**: the long-form piece (text, audio, or video) telling the human story behind BIZRA without crossing into founder worship.
...
4. The solo work arc (143-MB primary artifact + ~110K 2023 files + GitHub Jan 2024 onward)
5. The current home base (Dema, Node0, receipt chain)
...
**Forbidden framings**: "the visionary", "the genius", "the only one in the world", "the future is here because of one person". Use the bounded language from Founder Proof §17.

## 13. Scene-to-Asset Matrix
...
## 15. Forbidden Visuals

Visuals that may **not** appear in any First-Look-program asset at v0.1:

- Live token-trading UI · price charts · candle charts · APY / yield charts · countdown timers · "Buy now" / "Pre-sale" / "IDO" buttons
- Network heatmaps presented as "live" deployment (1M / 100M / 1B nodes shown without `SCENARIO` caption)
- PAT / SAT / UKE / URP / PoI runtime visuals without the visible "designed, not live" caption
- Sharia / halal certification badges, scholar endorsements, or religious-authority seals (none have been earned at v0.1)
- "World's first" badges, "Awarded X" badges, press-mention badges that haven't actually occurred
- Founder portrait with hagiographic framing (excessive lighting, halo composition, hero-shot crop, "the visionary" label)
- Crypto-asset logos used as if BIZRA has a tradable asset
- "Founder allocation %" pie charts (no token, no allocation exists)
- "Roadmap to mainnet" timelines that present `DESIGNED_NOT_LIVE` components on dated quarters
- Heatmaps of "users by country" that imply a deployed user base when none exists at v0.1
- Stock-photo "diverse team meeting" implying a team that doesn't exist (BIZRA is one human at v0.1)

## 16. Forbidden Wording

Wording that may **not** appear in any First-Look-program asset at v0.1:

Inheriting from `CLAIM_REGISTER §10`, `BIZRA_ROOT_SOURCE §18`, `NODE0_FOUNDER_PROOF §24`, `BIZRA_2026_FIRST_LOOK §20`:

- "BIZRA is complete" · "Node0 proves the entire forest" · "The full BIZRA is live"
- "Public URP is live" · "UKE shared runtime is live" · "PoI rewards are live"
- "Token live" · "Token launched" · "Token allocation" · "Founder allocation" · "Pre-sale" · "IDO" · "Listing" · "Yield" · "APY" · "Buyback" · "Burn" (as economic claim) · "Rebate"
...
- "1M nodes" / "100M nodes" / "1B nodes" stated as current reality
- "15,000+ hours" / "5,000+ AI conversations" / "600 GB+ R&D" stated as `VERIFIED`
- "Ramadan 2023" / "June 2023" stated as `VERIFIED` exact dates
- "World's first sovereign AI" · "Centralized AI is obsolete"
- "Founder allocation" · "Founder's share" · "Founder reward"
- "The visionary" · "The genius" · "The only one in the world" · "The founder is the final authority"
- "Free / Earn / Stake" tied to BIZRA in any economic framing
- "Production-ready" applied to any repository or layer except `Dema` (which is `ACTIVE` per Component DNA Layer 8)

Quoting these phrases in this brief, in CLAIM_REGISTER, or in any other canon doc **for the purpose of labeling them forbidden** is explicit and allowed; using them in produced assets is forbidden.

## 17. Required Truth Labels

Every produced asset must carry, in a designated metadata field or producer-side notes file, the following:
...
## 18. Evidence Anchor Requirements

Every claim in every asset traces to a row in `BIZRA_2026_FIRST_LOOK_NARRATIVE_v0_1.md §18 (Evidence Anchor Map)`. The asset's metadata (§17 above) cites the row.
...
## 19. Canva Prompt Pack v0.1

Text-only prompt blocks for future Canva work. **No Canva call is made by this brief.** Each prompt block carries the required truth labels inline.
...
>
> **Forbidden in this asset**: no BizraInfo account numbers; no founder face; no token visuals; no scale projections.
...
>
> **Forbidden**: founder face / portrait. "The visionary" or any final-authority noun. Any number cited as `VERIFIED` that the canon labels `OPERATOR_ATTESTED`.
...
>
> **Forbidden**: any cloud-service visual; any "always on / 24-7" framing that implies a daemon; any "AI agent doing things for you while you sleep" framing (Dema previews; the human consents).

### 19.5 — Seed / soil / forest visual (1:1 square)

> **Brief**: a botanical metaphor — one seed → one home-base soil patch → many seeds → future forest. **Each transition must carry a truth label**: seed = `ACTIVE`; home-base = `ACTIVE`; many-seeds = `SCENARIO` (with assumption set named in the caption); forest = `FUTURE_FOREST`.
>
> **Evidence anchors**: Component DNA Table §4 (label inheritance) + Root Source §13 (PAT/SAT/UKE/URP root meaning).
>
> **Forbidden**: showing the forest as "now"; showing many-seeds without the `SCENARIO` caption.
...
>
> **Forbidden**: any scenario ring shown without its truth-band label. Any token icon. Any "live deployment" framing.

## 20. Video Prompt Pack v0.1

Storyboard / production prompts per scene. **No video generation is performed by this brief.** Each scene below carries: visual mood, narration intent, evidence anchor, allowed wording, forbidden wording.
...
Prompt blocks for the future emulator's distinct screens. Each screen carries its truth band per §8.
...
> **Brief**: same pattern as 21.3, scaled. **Mandatory**: every projection screen carries the `SCENARIO` caption + assumption set. No projection screen says "live", "deployed", "current network state".
...
Before any asset ships externally, the producer (operator or designated reviewer) confirms each item:

- [ ] The asset's metadata file (per §17) is complete and saved alongside the asset.
- [ ] Every claim traces to a row in §18 / First Look §18 Evidence Anchor Map.
- [ ] No claim is labeled `VERIFIED` if its underlying canon row carries `DERIVED`, `OPERATOR_ATTESTED`, or `SOURCE_PENDING`.
- [ ] No phrase from §16 (Forbidden Wording) appears in the asset (text, captions, alt-text, audio).
...
- [ ] The First Look §29 Verification Checklist has been applied (this checklist is a superset, not a replacement).
- [ ] The operator has typed explicit consent for the asset to ship externally.

A produced asset that fails any item is revised or held; never shipped with an unchecked box.

## 23. Review Gate

Before publication, the asset passes through this gate:

1. **Producer self-review** — Production Checklist (§22) signed off.
2. **Canon cross-check** — every claim row in the asset's metadata matches a current row in the eight pillars (no drift since the eight-pillar canon was published).
3. **First Look §29 Verification Checklist** — applied independently.
4. **Operator typed consent** — exact string per ADR-005, with timestamp.
5. **Asset Receipt drafted** — a release receipt per Delivery Spine §24 template, scoped to the asset (`bizra.dema.asset_release_receipt.v0.1`).
6. **If asset is for external audience** — second reviewer (operator or trusted lighthouse pilot operator) confirms the Forbidden lists (§15, §16) before publication.
7. **If asset claims any number or date** — the citation matches the canon to the character.
8. **If asset touches token / economy / Sharia / scale-claim language** — automatic hold; must escalate to canon-level review.

The gate is **sequential**. An asset that fails any step is revised or held.

## 24. Deferred Work

Named and deferred (per `feedback_incremental_evidence_binding`):

- **Actual Canva production runs** — a separate slice that takes a §19 prompt block, runs Canva, produces an asset file, and records its metadata + receipt. Not in scope here.
- **Actual video production / storyboard generation** — a separate slice using the §20 prompt pack.
- **Visual emulator implementation** — currently `DESIGNED_NOT_LIVE`; a future slice will scope a static-prototype build (per Component DNA Layer 18 = `PILOT_REQUIRED`).
- **Website / landing build** — separate slice; uses §9 brief as input.
- **Social teaser production schedule** — separate slice; uses §10 brief.
- **Private Lighthouse invitation packaging** — separate slice; uses §11 brief and the operator-consent flow from ADR-005.
- **Founder Story long-form** — separate slice; uses §12 brief.
- **Asset Receipt schema formalization** — `bizra.dema.asset_release_receipt.v0.1` should be added to the Delivery Spine's receipt-template catalog in a future Delivery Spine v0.2 slice.
- **Canva → asset → receipt pipeline automation** — long-deferred; lives at the intersection of Dema product face (Component DNA Layer 8) and Delivery Spine §28 Deferred Automation.

Each deferred item consults this brief before its first produced asset.

## 25. Next Canon Slices

This Production Brief depends on and points forward to:

- `docs/THREE_REPO_PRODUCT_STACK_CANON_v0_1.md` — repo authority.
- `docs/NODE0_DEMA_COMPLETE_COMPONENT_DNA_v0_1.md` — component status (every `DESIGNED_NOT_LIVE` row binds asset visuals).
- `docs/DELIVERY_SPINE_v0_1.md` — release gates (the asset-release receipt template).
- `docs/CLAIM_REGISTER_v0_1.md` — public-claim authority.
- `docs/BIZRA_ROOT_SOURCE_OF_TRUTH_v0_1.md` — public meaning.
- `docs/BIZRA_ORIGIN_VIDEO_001_CANON_v0_1.md` — Scene 0 + Scene 2 anchor; transcript inviolate.
- `docs/NODE0_FOUNDER_PROOF_AND_HUMAN_CHOICE_v0_1.md` — Scene 3 anchor; founder-proof discipline.
- `docs/BIZRA_2026_FIRST_LOOK_NARRATIVE_v0_1.md` — narrative spine + 7 scenes + Evidence Anchor Map + 3 Public Wording Blocks.
...
- Future `docs/public/BIZRA_VISUAL_EMULATOR_SPEC_v0_1.md` (the spec this brief's §8 will anchor).
- Future `docs/market/BIZRA_GTM_MASTER_PLAN_v0_1.md` (the GTM plan inherits this brief's asset families).
- Future `docs/public/BIZRA_LIGHTHOUSE_INVITATION_v0_1.md` (uses this brief's §11).

The load-bearing surfaces of this brief are §13 (Scene-to-Asset Matrix), §14 (Claim-to-Visual Matrix), §15 (Forbidden Visuals), §16 (Forbidden Wording), §19-21 (the three Prompt Packs), §22 (Production Checklist), and §23 (Review Gate). The other sections explain and bound them.

When any pillar in the eight-pillar canon changes, this brief is re-read for drift. When this brief changes, every deferred production slice (§24) is re-read for drift.
