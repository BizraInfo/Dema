...
| **v0.3.5** Install-script harden | IN PROGRESS | **Skeletons exist on disk** at `scripts/install/install-unix.sh` and `scripts/install/install-windows.ps1` (~44 LOC each); v0.3.5 adds `--dry-run` and `--check` flags, sibling uninstall scripts gated by exact-string consent, SHA-256 hash table, and clearer doctor copy. Canonical install endpoint will be `https://install.bizra.ai/dema` (under owned domain). Publishing the endpoint and signature verification remain L5 acts, halt-gated. |
